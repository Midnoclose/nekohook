 ...
 * GNU General Public Liense for more details.
 *
...
std::string NameSteal() {
    ...
    for (int i = 0; i < ent_count; i++) {
        ...
        if (!entity) continue;
        if (entity == local_ent) continue;
        if (entity->GetType() != Entity::Type::kPlayer) continue;
        if (entity->GetEnemy()) continue;

        std::string_view ent_name = GetName(entity);
