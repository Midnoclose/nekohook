...
void internal_fonts_init() { memset(loaded_fonts, 0, sizeof(loaded_fonts)); }
...
glez_font_t glez_font_load(const char *path, float size) {
    ...
    if (result.atlas == NULL) return GLEZ_FONT_INVALID;
...
void glez_font_string_size(glez_font_t font, const char *string, float *out_x,
    ...
    for (size_t i = 0; i < strlen(string); ++i) {
        ...
        if (pen_x > size_x) size_x = pen_x;

        if (glyph->height > size_y) size_y = glyph->height;
    }
    if (out_x) *out_x = size_x;
