...
void glez_begin() { ds_pre_render(); }

void glez_end() { ds_post_render(); }

void glez_resize(int width, int height) { shader_screen_size(width, height); }
...
void draw_string_internal(float x, float y, const char *string,
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        // pen_x = (int) pen_x + 1;
        if (glyph->height > size_y) size_y = glyph->height;

        vertex_buffer_push_back(program.buffer, vertices, 4, indices, 6);
    }

    if (out_x) *out_x = pen_x - x;
    if (out_y) *out_y = size_y;
}
...
void glez_string_with_outline(float x, float y, const char *string,
                              ...
                              float *out_y) {
    if (adjust_outline_alpha) outline_color.a = color.a;
...
void glez_circle(float x, float y, float radius, glez_rgba_t color,
    ...
    for (int i = 0; i < steps; i++) {
        ...
        if (!i) ang = 2 * M_PI * ((float)(steps - 1) / steps);
        if (i)
