...
class Slider : public Panel {
   ...
   public:
    ...
    virtual void SetRange(
        int min, int max);  // set to max and min range of rows to display
    ...
    // This allows the slider to behave like it's larger than what's actually
    // being drawn
    virtual void SetSliderThumbSubRange(bool bEnable, int nMin = 0,
                                        int nMax = 100);
   ...
   protected:
    ...
    int _range[2];
    int _subrange[2];
    int _value;  // the position of the Slider, in coordinates as specified by
                 // SetRange/SetRangeWindow
    int _buttonOffset;
