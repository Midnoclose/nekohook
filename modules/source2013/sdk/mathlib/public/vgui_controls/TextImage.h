...
class CColorChangeListLess {
   public:
    bool Less(const label_colorchange_t &src1, const label_colorchange_t &src2,
              void *pCtx) {
        if (src1.textStreamIndex < src2.textStreamIndex) return true;
...
class TextImage : public Image {
   ...
   public:
    // takes the string and looks it up in the localization file to convert it
    // to unicode
    ...
    void SetColorChangeStream(
        CUtlSortVector<label_colorchange_t, CColorChangeListLess>
            *pUtlVecStream);
   ...
   private:
    ...
    int _drawWidth;  // this is the width of the window we are drawing into.
                     // if there is not enough room truncate the txt	and add
                     // an elipsis

    StringIndex_t _unlocalizedTextSymbol;  // store off the unlocalized text
                                           // index for build mode
    ...
    bool m_bAllCaps : 1;
    CUtlVector<wchar_t *> m_LineBreaks;  // an array that holds the index in the
                                         // buffer to wrap lines at

    bool m_bWrapCenter;  // Separate from m_bWrap to ensure it doesn't break
                         // legacy code.
    CUtlVector<int>
        m_LineXIndent;  // For centered word wrap. The X indent for each line.
