...
class EditablePanel : public Panel {
   ...
   public:
    ...
    // Load the control settings - should be done after all the children are
    // added If you pass in pPreloadedKeyValues, it won't actually load the
    // file. That way, you can cache the keyvalues outside of here if you want
    // to prevent file accesses in the middle of the game.
    ...
    virtual void LoadControlSettingsAndUserConfig(
        const char *dialogResourceName, int dialogID = 0);
    ...
    // Shortcut function to set data in child controls
    virtual void SetControlString(const char *controlName,
    ...
    // static buffer
    virtual void GetControlString(const char *controlName, char *buf,
    ...
    /* INFO HANDLING
            "BuildDialog"
                    input:
                            "BuildGroupPtr" - pointer to the panel/dialog to
       edit returns: "PanelPtr" - pointer to a new BuildModeDialog()

            "ControlFactory"
                    input:
                            "ControlName" - class name of the control to create
                    returns:
                            "PanelPtr" - pointer to the newly created panel, or
   ...
   protected:
    ...
    // immediately after Paint() disabled for now virtual bool
    // ShouldFlushText();
