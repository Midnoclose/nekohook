...
class ListPanel : public Panel {
   ...
   public:
    ...
    enum ColumnFlags_e {
        ...
        COLUMN_IMAGE = 0x04,  // set if the column data is not text, but instead
    ...
    virtual int AddItem(
        ...
        bool bSortOnAdd);  // Takes a copy of the data for use in the table.
                           // Returns the index the item is at.
    void SetItemDragData(
        int itemID, const KeyValues *
                        data);  // Makes a copy of the keyvalues to store in the
                                // table. Used when dragging from the table.
                                // Only used if the caller enables drag support
    ...
    virtual void ApplyItemChanges(
        int itemID);  // applies any changes to the data, performed by modifying
                      // the return of GetItem() above
    virtual void RemoveItem(
        int itemID);  // removes an item from the table (changing the indices of
                      // all following items)
    ...
    virtual void GetCellText(
        int itemID, int column,
        OUT_Z_BYTECAP(bufferSizeInBytes) wchar_t *buffer,
        int bufferSizeInBytes);  // returns the data held by a specific cell
    virtual IImage *GetCellImage(
        int itemID, int column);  //, ImagePanel *&buffer); // returns the image
                                  //held by a specific cell
    ...
    // it is removed from the row list when it becomes invisible, but stays in
    // the indexes this is much faster than a normal remove
    ...
    virtual bool GetCellAtPos(
        int x, int y, int &row,
        int &column);  // returns true if any found, row and column are filled
                       // out. x, y are in screen space
    ...
    // relayout the scroll bar in response to changing the items in the list
    // panel do this if you RemoveAll()
    void ResetScrollBar();
   ...
   public:
    virtual void SetSortColumnEx(int iPrimarySortColumn,
   ...
   private:
    ...
    // Purpose:
    void UpdateSelection(vgui::MouseCode code, int x, int y, int row,
                         int column);
    ...
    int m_iColumnDraggerMoved;  // which column dragger was moved->which header
                                // to resize
    int m_lastBarWidth;
