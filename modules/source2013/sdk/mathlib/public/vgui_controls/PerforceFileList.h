...
class PerforceFileList : public vgui::ListPanel {
   ...
   public:
    // The context keyvalues are added to all messages sent by this dialog if
    // they are specified
    ...
    // and it may exist in perforce or not. It's specified as a full path on
    // disk though. In the case where a file doesn't exist on disk, but it does
    // exist in perforce specify where that file would appear on disk. This
    // function returns the itemID of the added file If you already know the
    // file exists or is a directory (or not), specify that in the call. -1
    // means autodetect whether the file exists or is a directory
    int AddFile(const char *pFullPath, int nFileExists = -1,
                int nIsDirectory = -1);
    ...
    void RefreshPerforceState(int nItemID, bool bFileExists,
                              P4File_t *pFileInfo);
   ...
   protected:
    ...
    void AddItemToDirectoryList(const char *pFullPath, int nItemID,
                                bool bIsDirectory);
