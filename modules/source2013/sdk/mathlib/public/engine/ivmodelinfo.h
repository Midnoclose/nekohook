...
abstract_class IModelLoadCallback {
   ...
   protected:
    ...
    // Automatically unregisters if the callback is destroyed while still
    // pending.
    ~IModelLoadCallback();
};
...
// If index < -1, then the model is DYNAMIC and has a DYNAMIC INDEX of (-2 -
// index)
...
abstract_class IVModelInfo {
   public:
    ...
    // Returns model_t* pointer for a model given a precached or dynamic model
    // index.
    ...
    virtual const studiohdr_t *FindModel(const studiohdr_t *pStudioHdr,
                                         void **cache, const char *modelname)
        const = 0;
    virtual const studiohdr_t *FindModel(void *cache) const = 0;
    virtual virtualmodel_t *GetVirtualModel(const studiohdr_t *pStudioHdr)
        const = 0;
    virtual byte *GetAnimBlock(const studiohdr_t *pStudioHdr, int iBlock)
        const = 0;
    ...
    virtual void GetIlluminationPoint(
        const model_t *model, IClientRenderable *pRenderable,
    ...
    virtual unsigned char ComputeLevelScreenFade(
        const Vector &vecAbsOrigin, float flRadius, float flFadeScale)
        const = 0;
    virtual unsigned char ComputeViewScreenFade(
        const Vector &vecAbsOrigin, float flRadius, float flFadeScale)
        const = 0;
    ...
    // Obsolete methods. These are left in to maintain binary compatibility with
    // clients using the IVModelInfo old version.
    ...
    // Returns index of model by name, dynamically registered if not already
    // known.
    ...
    virtual bool RegisterModelLoadCallback(
        int modelindex, IModelLoadCallback *pCallback,
        bool bCallImmediatelyIfLoaded = true) = 0;
    virtual void UnregisterModelLoadCallback(int modelindex,
                                             IModelLoadCallback *pCallback) = 0;
};
...
abstract_class IVModelInfoClient : public IVModelInfo {
   public:
    virtual void OnDynamicModelsStringTableChange(
        int nStringIndex, const char *pString, const void *pData) = 0;
...
struct virtualterrainparams_t {
    // UNDONE: Add grouping here, specified in BSP file? (test grouping to see
