...
abstract_class ITextureRegenerator {
   public:
    ...
    // (erics): This should have a virtual destructor, but would be ABI breaking
    // (non-versioned interface implemented
    //          by the game)
...
abstract_class ITexture {
   public:
    ...
    virtual void *GetResourceData(uint32 eDataType, size_t * pNumBytes)
        const = 0;
    ...
    // Uses for stats. . .get the approximate size of the texture in it's
    // current format.
    ...
    // Copy this texture, which must be a render target or a renderable texture,
    // to the destination texture, which must have been created with the STAGING
    // bit.
