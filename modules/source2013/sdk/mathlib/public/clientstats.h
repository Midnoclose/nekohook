...
struct IClientStatsTextDisplay {
    ...
    virtual void SetDrawColor(unsigned char r, unsigned char g,
                              unsigned char b) = 0;
...
abstract_class IClientStats {
   public:
    ...
    // ---------------------------------------------------------------
    // All this stuff is used to prop stats for gathering r_speeds data during
    // timedemo.
    // ---------------------------------------------------------------
...
template <int timedStatCount, int countedStatCount>
void CBaseClientStats<timedStatCount, countedStatCount>::Init(
    IClientStatsTime *pTime) {
    Assert(pTime);
    m_pTime = pTime;
}
...
void CBaseClientStats<timedStatCount, countedStatCount>::BeginRun() {
    ...
    for (i = 0; i < timedStatCount; ++i) m_TotalStatTime[i] = 0.0;
}
...
void CBaseClientStats<timedStatCount, countedStatCount>::BeginFrame() {
    int i;
    for (i = 0; i < timedStatCount; ++i) m_StatFrameTime[i] = 0.0;
}
...
template <int timedStatCount, int countedStatCount>
void CBaseClientStats<timedStatCount, countedStatCount>::BeginTimedStat(
    int stat) {
    if (m_pTime) m_StatStartTime[stat] = m_pTime->GetTime();
}

template <int timedStatCount, int countedStatCount>
void CBaseClientStats<timedStatCount, countedStatCount>::EndTimedStat(
    int stat) {
    if (m_pTime)
