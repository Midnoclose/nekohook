...
//index 		type, which usually should be an unsigned short or smaller. 		This list
   ...
   protected:
    ...
    void ResetDbgInfo() {
        ...
        if (m_Memory.NumAllocated() > 0) {
            if (!m_pElementList) {
                m_pElementList =
                    (I*)malloc(m_Memory.NumAllocated() * sizeof(I));
            } else {
                m_pElementList = (I*)realloc(
                    m_pElementList, m_Memory.NumAllocated() * sizeof(I));
            }
        }
#endif
    }
...
inline bool CUtlMultiList<T, I>::IndexInRange(int index)  // Static method
{
    ...
    // === unsigned short', in which case case CUtlMemory will have
    // 'InvalidIndex == (int)-1' (which casts to 65535 in I), and will happily
    // return elements at index 65535 and above.

    // Do a couple of static checks here: the invalid index should be (I)~0
    // given how we use m_MaxElementIndex, and 'I' should be unsigned (to avoid
    // signed arithmetic errors for plausibly exhaustible ranges).
...
I CUtlMultiList<T, I>::Alloc() {
    ...
    if (m_FirstFree == InvalidIndex()) {
        ...
        // Since nothing is in the free list, m_TotalElements == total # of
