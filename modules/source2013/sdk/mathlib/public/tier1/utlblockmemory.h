...
// If you would like to use this, add -fpermissive to your options as well as
// define fpermissive. Cathook will NOT compile if fpermissive isnt used while
// this is.
#ifdef fpermissive
...
void CUtlBlockMemory<T, I>::Init(int nGrowSize /* = 0 */,
    ...
    if (nGrowSize == 0) {
        // default grow size is smallest size s.t. c++ allocation overhead is
        // ~6% of block size
...
void CUtlBlockMemory<T, I>::ChangeSize(int nBlocks) {
    ...
    UTLBLOCKMEMORY_TRACK_ALLOC();  // this must stay after the recalculation of
                                   // m_nBlocks, since it implicitly uses the
                                   // new value

    if (m_pMemory) {
        // free old blocks if shrinking
        // Only possible if m_pMemory is non-NULL (and avoids PVS-Studio
        // warning)
