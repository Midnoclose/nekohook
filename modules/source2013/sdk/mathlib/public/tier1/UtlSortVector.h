...
class CUtlSortVector : public BaseVector {
   ...
   public:
    ...
    /// inserts (copy constructs) an element in sorted order into the list if it
    /// isn't already in the list
    int InsertIfNotFound(const T& src);

    /// Finds an element within the list using a binary search. These are
    /// templatized based upon the key in which case the less function must
    /// handle the Less function for key, T and T, key
    ...
    /// Note that you can only use this index until sorting is redone with
    /// RedoSort!!!
...
template <class T, class LessFunc, class BaseVector>
int CUtlSortVector<T, LessFunc, BaseVector>::InsertAfter(int nIndex,
                                                         const T& src) {
    ...
    if (nInsertedIndex > 0) {
        Assert(
            less.Less(this->Element(nInsertedIndex - 1), src, m_pLessContext));
    }
    if (nInsertedIndex < BaseClass::Count() - 1) {
        Assert(
            less.Less(src, this->Element(nInsertedIndex + 1), m_pLessContext));
    }
#endif
...
template <class T, class LessFunc, class BaseVector>
template <typename TKey>
int CUtlSortVector<T, LessFunc, BaseVector>::FindUnsorted(
    const TKey& src) const {
    LessFunc less;
    int nCount = this->Count();
    for (int i = 0; i < nCount; ++i) {
        if (less.Less(this->Element(i), src, m_pLessContext)) continue;
        if (less.Less(src, this->Element(i), m_pLessContext)) continue;
        return i;
    }
    return -1;
}
...
int CUtlSortVector<T, LessFunc, BaseVector>::FindLessOrEqual(
    const TKey& src, bool* pFound) const {
    AssertFatal(!m_bNeedsSort);
