...
class CLZMA {
   public:
    static unsigned int Uncompress(unsigned char *pInput,
                                   unsigned char *pOutput);
    static bool IsCompressed(unsigned char *pInput);
    static unsigned int GetActualSize(unsigned char *pInput);
};

// For files besides the implementation, we forward declare a dummy struct. We
// can't unconditionally forward declare this because LzmaEnc.h typedefs this
// directly to an unnamed struct :-/
#ifndef CLzmaDec_t
struct _CLzmaDec_t;
...
class CLZMAStream {
   public:
    ...
    // Initialize a stream to read data from a LZMA style zip file, passing the
    // original size from the zip headers. Streams with a source-engine style
    // header (lzma_header_t) do not need an init call.
    void InitZIPHeader(unsigned int nCompressedSize,
                       unsigned int nOriginalSize);

    // Attempt to read up to nMaxInputBytes from the compressed stream, writing
    // up to nMaxOutputBytes to pOutput. Makes progress until blocked on input
    // or output. Returns false if read stops due to an error or if called at
    // EOF (GetExpectedBytesRemaining == 0)
    ...
    // Get the expected uncompressed bytes yet to be read from this stream.
    // Returns false if not yet known, such as before being fed the header.
   ...
   private:
    ...
    eHeaderParse TryParseHeader(unsigned char *pInput,
    ...
    // Init from a zip-embedded LZMA stream. Requires the original size be
    // passed from zip headers.
    ...
    // If InitZIPHeader() was called. We're expecting a zip-style header and
