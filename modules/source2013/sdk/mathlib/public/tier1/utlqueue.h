...
void CUtlQueue<T, M>::Insert(T const &element) {
    if (m_tail == QUEUE_ITERATOR_INVALID) {
        ...
        if (nextTail ==
            m_head)  // if non-empty, and growing by 1 appears to make the queue
                     // of length 1, then we were already full before the Insert
        {
            ...
            nextTail = Next_Unchecked(m_tail);  // if nextTail was 0, then it
                                                // now should be nOldAllocCount

            if (m_head != QueueIter_t(0)) {
                // if the queue wraps around the end of m_memory, move the part
                // at the end of memory to the new end of memory
                Q_memmove(&m_memory[m_head + nGrowAmount], &m_memory[m_head],
                          (nOldAllocCount - m_head) * sizeof(T));
#ifdef _DEBUG
...
bool CUtlQueue<T, M>::Check(T const element) const {
    for (QueueIter_t it = First(); it != QUEUE_ITERATOR_INVALID;
         it = Next(it)) {
        if (m_memory[it] == element) return true;
    }
    return false;
}
...
QueueIter_t CUtlQueue<T, M>::Next(QueueIter_t it) const {
    if (it == QUEUE_ITERATOR_INVALID) return QUEUE_ITERATOR_INVALID;

    if (it == m_tail) return QUEUE_ITERATOR_INVALID;

    Assert(IsValid(it));
    if (!IsValid(it)) return QUEUE_ITERATOR_INVALID;
...
bool CUtlQueue<T, M>::IsValid(QueueIter_t it) const {
    if (it == QUEUE_ITERATOR_INVALID) return false;
...
inline void CUtlQueue_Test() {
    ...
    for (int n = 1; n < 100; ++n) {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            for (QueueIter_t it = queue.First(); it != QUEUE_ITERATOR_INVALID;
                 it = queue.Next(it)) {
            ...
            for (QueueIter_t it = queue.Last(); it != QUEUE_ITERATOR_INVALID;
                 it = queue.Previous(it)) {
                Assert(queue.Element(it).m_id % queue.m_memory.Count() == id);
        ...
        for (int j = n - 1; j >= 0; --j) {
            ...
            if (queue.Count() > 0) {
                int id = queue.Head().m_id % queue.m_memory.Count();
                for (QueueIter_t it = queue.First();
                     it != QUEUE_ITERATOR_INVALID; it = queue.Next(it)) {
                    Assert(queue.Element(it).m_id % queue.m_memory.Count() ==
                           id);
                    id = (id + 1) % queue.m_memory.Count();
                }

                id = queue.Tail().m_id % queue.m_memory.Count();
                for (QueueIter_t it = queue.Last();
                     it != QUEUE_ITERATOR_INVALID; it = queue.Previous(it)) {
                    Assert(queue.Element(it).m_id % queue.m_memory.Count() ==
                           id);
                    id = (id + queue.m_memory.Count() - 1) %
                         queue.m_memory.Count();
                }
            }
