...
// their own m_pNext, (optionally), m_pPrev, and other fields contained within.
...
//
// Some functions are mean for use with lists which maintain both a head and
...
// return the node in head before in a singly linked list. returns null if head
// is empty, n is null, or if n is the first node. not fast.
template <class T>
...
class CUtlIntrusiveList {
   public:
    ...
    FORCEINLINE T *Head(void) const { return m_pHead; }
    ...
    // return the node in head before in a singly linked list. returns null if
    // head is empty, n is null, or if n is the first node. not fast. Fast for
    // dlists
   ...
   public:
    ...
    T *RemoveHead(void) {
        if (CUtlIntrusiveList<T>::m_pHead) {
            ...
            CUtlIntrusiveList<T>::m_pHead =
                CUtlIntrusiveList<T>::m_pHead->m_pNext;
            if (CUtlIntrusiveList<T>::m_pHead)
...
class CUtlIntrusiveDListWithTailPtr : public CUtlIntrusiveDList<T> {
   public:
    ...
    FORCEINLINE void AddToHead(T *node) {
        IntrusiveList::AddToDHeadWithTailPtr(CUtlIntrusiveList<T>::m_pHead,
                                             node, m_pTailPtr);
    }
    FORCEINLINE void AddToTail(T *node) {
        IntrusiveList::AddToDTailWithTailPtr(CUtlIntrusiveList<T>::m_pHead,
                                             node, m_pTailPtr);
    }

    void RemoveNode(T *pWhich) {
        IntrusiveList::RemoveFromDListWithTailPtr(CUtlIntrusiveList<T>::m_pHead,
                                                  pWhich, m_pTailPtr);
    }
    ...
    T *RemoveHead(void) {
        if (CUtlIntrusiveDList<T>::m_pHead) {
            ...
            CUtlIntrusiveDList<T>::m_pHead =
                CUtlIntrusiveDList<T>::m_pHead->m_pNext;
            if (CUtlIntrusiveDList<T>::m_pHead)
                CUtlIntrusiveDList<T>::m_pHead->m_pPrev = NULL;
            if (!CUtlIntrusiveDList<T>::m_pHead) m_pTailPtr = NULL;
...
void PrependDListWithTailToDList(CUtlIntrusiveDListWithTailPtr<T> &src,
    ...
    if (src.m_pHead) {
        ...
        if (dest.m_pHead) dest.m_pHead->m_pPrev = src.m_pTailPtr;
