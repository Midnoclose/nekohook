...
//pointer can be used.
//				  * UtlMakeDelegate() global helper function added to
...
//				  * Added FastDelegate::IsEmpty() to test if
//invocation is safe (Thanks Neville Franks).
//				  * Now tested on VS 2005 Express Beta, PGI C++
// 24-Dec-04 1.4  * Added CUtlAbstractDelegate, to allow collections of
// disparate delegates.
...
//                * Bugfix for Metrowerks: IsEmpty() was buggy because a valid
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 3 * sizeof(int)> {
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind,
        ...
        // specifying the /vmg option, but it is disabled -  you can only
        // specify /vmg on the command line. In VC1.5 and earlier, the compiler
        // would ICE if it ever encountered this situation. It is OK to use the
        // /vmg option if /vmm or /vms is specified.

        // Fortunately, the wrong function is only called in very obscure cases.
        // It only occurs when a derived class overrides a virtual function
        // declared in a virtual base class, and the member function points to
        // the *Derived* version of that function. The problem can be completely
        // averted in 100% of cases by using the *Base class* for the member
        // fpointer. Ie, if you use the base class as an interface, you'll stay
        // out of trouble. Occasionally, you might want to point directly to a
        // derived class function that isn't an override of a base class. In
        // this case, both vtable_index and 'vtordisp' are zero, but a
        // virtual_inheritance pointer will be generated. We can generate
        // correct code in this case. To prevent an incorrect call from ever
        // being made, on MSVC6 we generate a warning, and call a function to
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 3 * sizeof(int)> {
    ...
    inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind,
        ...
        // Check that the horrible_cast will work
        typedef int ERROR_CantUsehorrible_cast[sizeof(XFuncType) == sizeof(u.s)
                                                   ? 1
                                                   : -1];
        u.func = function_to_bind;
        bound_func = u.s.funcaddress;
...
// CUtlAbstractDelegate -- an opaque structure which can hold an arbitary
// delegate. It knows nothing about the calling convention or number of
// arguments used by the function pointed to. It supplies comparison operators
// so that it can be stored in STL collections. It cannot be set to anything
// other than null, nor invoked directly:
...
// horrible_cast. In this case the CUtlAbstractDelegate implementation is
// simple:
...
class CUtlAbstractDelegate {
   ...
   public:
    ...
    inline bool IsLess(const CUtlAbstractDelegate &right) const {
        ...
        // STL containers.
        return memcmp(&m_pFunction, &right.m_pFunction, sizeof(m_pFunction)) <
               0;
    }
    // BUGFIX (Mar 2005):
   ...
   public:
    ...
    void *UnsafeGetThisPtr() { return m_pthis; }
...
//						ClosurePtr<>
//
// A private wrapper class that adds function signatures to
// CUtlAbstractDelegate. It's the class that does most of the actual work. The
// signatures are specified by: GenericMemFunc: must be a type of GenericClass
...
class ClosurePtr : public CUtlAbstractDelegate {
   ...
   public:
    ...
    inline void CopyFrom(DerivedClass *pParent,
                         const CUtlAbstractDelegate &right) {
        pParent;
        SetMementoFrom(right);
    }
    // For static functions, the 'static_function_invoker' class in the parent
...
//
//			UtlMakeDelegate(&x, &X::func) returns a fastdelegate of the
...
template <class X, class Y, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE()> UtlMakeDelegate(Y *x,
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1)> UtlMakeDelegate(
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1)> UtlMakeDelegate(
    RetType (*func)(Param1 p1)) {
    return CUtlDelegate<FASTDLGT_RETTYPE(Param1)>(func);
}

// N=2
template <class X, class Y, class Param1, class Param2, class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)> UtlMakeDelegate(
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2)> UtlMakeDelegate(
    RetType (*func)(Param1 p1, Param2 p2)) {
...
template <class X, class Y, class Param1, class Param2, class Param3,
          class RetType>
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3)> UtlMakeDelegate(
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3)> UtlMakeDelegate(
    RetType (*func)(Param1 p1, Param2 p2, Param3 p3)) {
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3, Param4)> UtlMakeDelegate(
    ...
    return CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3, Param4)>(x,
                                                                          func);
}
...
CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3, Param4)> UtlMakeDelegate(
    RetType (*func)(Param1 p1, Param2 p2, Param3 p3, Param4 p4)) {
...
UtlMakeDelegate(Y *x, RetType (X::*func)(Param1 p1, Param2 p2, Param3 p3,
                                         Param4 p4, Param5 p5)) {
    return CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3, Param4,
                                         Param5)>(x, func);
}
...
UtlMakeDelegate(Y *x, RetType (X::*func)(Param1 p1, Param2 p2, Param3 p3,
                                         Param4 p4, Param5 p5, Param6 p6)) {
    return CUtlDelegate<FASTDLGT_RETTYPE(Param1, Param2, Param3, Param4, Param5,
                                         Param6)>(x, func);
}
