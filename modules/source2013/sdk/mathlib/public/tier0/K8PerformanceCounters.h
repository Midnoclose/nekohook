...
typedef union EVENT_MASK(k8_locked_op) {
    struct {
        ...
        uint16 NumCyclesInRequestGrant : 1;  // Number of cycles spent in the
                                             // lock request/grant stage
...
class k8Event_DISPATCHED_PRE_INSTRS : public k8BaseEvent {
   public:
    k8Event_DISPATCHED_PRE_INSTRS() {
        eventMask =
            (EVENT_MASK(k8_distpatch_prefetch_instructions) *)&m_eventMask;
...
typedef union EVENT_MASK(k8_lock_accesses) {
    struct {
        uint16 DcacheAccesses : 1;  // Number of dcache accesses by lock
                                    // instructions" },
        uint16 DcacheMisses : 1;    // Number of dcache misses by lock
                                    // instructions" } }
    };
    uint16 flat;
...
class k8Event_RETIRED_INSTRUCTIONS : public k8BaseEvent {
   public:
    k8Event_RETIRED_INSTRUCTIONS() {
        ...
        name =
            _T("Retired instructions (includes exceptions, interrupts, ")
            _T("resyncs)");
    }
    EVENT_MASK(NULL_MASK) * eventMask;
};
...
class k8Event_RETIRED_BRANCH_MISCOMPARE : public k8BaseEvent {
   public:
    k8Event_RETIRED_BRANCH_MISCOMPARE() {
        eventMask = (EVENT_MASK(NULL_MASK) *)&m_eventMask;
        name =
            _T("Retired taken branches mispredicted due to address miscompare");
        event_id = 0xCA;
        unitEncode = FR;
    }
    EVENT_MASK(NULL_MASK) * eventMask;
...
class k8Event_RETIRED_FASTPATH_INSTRS : public k8BaseEvent {
   public:
    k8Event_RETIRED_FASTPATH_INSTRS() {
        eventMask =
            (EVENT_MASK(k8_retired_fastpath_double_op_instr) *)&m_eventMask;
        event_id = 0xCC;
        unitEncode = FR;
...
typedef union EVENT_MASK(k8_bypass_counter_saturation) {
    struct {
        uint16 MEM_HighPriorityBypass : 1;  // Memory controller high priority
                                            // bypass" },
        uint16 MEM_LowPriorityBypass : 1;   // Memory controller low priority
                                            // bypass" },
        uint16
            DRAM_InterfaceBypass : 1;  // DRAM controller interface bypass" },
...
typedef union EVENT_MASK(k8_probe_result) {
    struct {
        ...
        uint16 ProbeHitDirtyWithoutMemoryCancel : 1;  // Probe hit dirty without
                                                      // memory cancel" },
        uint16 ProbeHitDirtyWithMemoryCancel : 1;     // Probe hit dirty with
                                                      // memory cancel" } }
