...
inline bool ThreadInterlockedAssignIf64x128(volatile int128 *pDest,
                                            const int128 &value,
                                            const int128 &comperand) {
    return ThreadInterlockedAssignIf128(pDest, value, comperand);
}
#else
#define TSLIST_HEAD_ALIGNMENT 8
#define TSLIST_NODE_ALIGNMENT 8
inline bool ThreadInterlockedAssignIf64x128(volatile int64 *pDest,
                                            const int64 value,
                                            const int64 comperand) {
...
class CTSListBase {
   public:
    ...
    static void *operator new(size_t size) {
        CTSListBase *pNode = (CTSListBase *)MemAlloc_AllocAligned(
            size, TSLIST_HEAD_ALIGNMENT, __FILE__, __LINE__);
        return pNode;
    }

    static void *operator new(size_t size, int nBlockUse, const char *pFileName,
                              int nLine) {
        CTSListBase *pNode = (CTSListBase *)MemAlloc_AllocAligned(
            size, TSLIST_HEAD_ALIGNMENT, pFileName, nLine);
        return pNode;
    }

    static void operator delete(void *p) { MemAlloc_FreeAligned(p); }

    static void operator delete(void *p, int nBlockUse, const char *pFileName,
                                int nLine) {
   ...
   public:
    ...
    TSLNodeBase_t *Push(TSLNodeBase_t *pNode) {
        ...
        return (TSLNodeBase_t *)InterlockedPushEntrySListRelease(&m_Head,
                                                                 pNode);
#else
        ...
        for (;;) {
            ...
            if (ThreadInterlockedAssignIf64x128(&m_Head.value64x128,
                                                newHead.value64x128,
                                                oldHead.value64x128)) {
                break;
            }
    ...
    TSLNodeBase_t *Pop() {
...
#else
        TSLNodeBase_t *pNode =
            (TSLNodeBase_t *)InterlockedPopEntrySList(&m_Head);
#endif
        return pNode;
#else
        TSLHead_t oldHead;
        ...
        for (;;) {
            ...
            newHead.value32.DepthAndSequence =
                oldHead.value32.DepthAndSequence - 1;
    ...
    TSLNodeBase_t *Detach() {
        ...
        do {
            ...
            // memory
            //          is probably to avoid store forward issues, but I'm not
            //          sure because I didn't construct this code. In any case,
            //          leaving it as is on big-endian
            newHead.value32.DepthAndSequence =
                oldHead.value32.DepthAndSequence & 0xffff0000;

        } while (!ThreadInterlockedAssignIf64x128(
            &m_Head.value64x128, newHead.value64x128, oldHead.value64x128));
...
class TSLIST_HEAD_ALIGN CTSPool : public CTSListBase {
   ...
   public:
    ...
    T *GetObject() {
        simpleTSPoolStruct_t *pNode =
            (simpleTSPoolStruct_t *)CTSListBase::Pop();
        if (!pNode) {
...
class TSLIST_HEAD_ALIGN CTSList : public CTSListBase {
   public:
    struct TSLIST_NODE_ALIGN Node_t : public TSLNodeBase_t {
        ...
        // override new/delete so we can guarantee 8-byte aligned allocs
        static void *operator new(size_t size) {
            Node_t *pNode = (Node_t *)MemAlloc_AllocAligned(
                size, TSLIST_NODE_ALIGNMENT, __FILE__, __LINE__);
            return pNode;
        }

        // override new/delete so we can guarantee 8-byte aligned allocs
        static void *operator new(size_t size, int nBlockUse,
                                  const char *pFileName, int nLine) {
            Node_t *pNode = (Node_t *)MemAlloc_AllocAligned(
                size, TSLIST_NODE_ALIGNMENT, pFileName, nLine);
            return pNode;
        }

        static void operator delete(void *p) { MemAlloc_FreeAligned(p); }
        static void operator delete(void *p, int nBlockUse,
                                    const char *pFileName, int nLine) {
            MemAlloc_FreeAligned(p);
        }
...
class TSLIST_HEAD_ALIGN CTSQueue {
   public:
    // override new/delete so we can guarantee 8-byte aligned allocs
    static void *operator new(size_t size) {
        CTSQueue *pNode = (CTSQueue *)MemAlloc_AllocAligned(
            size, TSLIST_HEAD_ALIGNMENT, __FILE__, __LINE__);
        return pNode;
    }
   ...
   public:
    ...
    union TSLIST_HEAD_ALIGN NodeLink_t {
        // override new/delete so we can guarantee 8-byte aligned allocs
        static void *operator new(size_t size) {
            NodeLink_t *pNode = (NodeLink_t *)MemAlloc_AllocAligned(
                size, TSLIST_HEAD_ALIGNMENT, __FILE__, __LINE__);
            return pNode;
        }
    ...
    Node_t *Push(Node_t *pNode) {
        ...
        for (;;) {
            ...
            if (InterlockedCompareExchangeNode(&(oldTail.value.pNode->pNext),
                                               pNode, End()) == End()) {
                break;
            } else {
        ...
        FinishPush(pNode, oldTail);  // This can fail if another thread pushed
                                     // between the sequence and node grabs
                                     // above. Later pushes or pops corrects
...
#define TSQUEUE_BAD_NODE_LINK ((Node_t *)INT_TO_POINTER(0xdeadbeef))
        ...
        for (;;) {
            head.value.sequence =
                *pHeadSequence;     // must grab sequence first, which allows
                                    // condition below to ensure pNext is valid
            ThreadMemoryBarrier();  // need a barrier to prevent reordering of
                                    // these assignments
            head.value.pNode = *pHeadNode;
            tailSequence = pTail->value.sequence;
            pNext = head.value.pNode->pNext;

            // Checking pNext only to force optimizer to not reorder the
            ...
            if (pNext != End()) {
                elem = pNext->elem;  // NOTE: next could be a freed node here,
                                     // by design
   ...
   private:
    ...
    bool InterlockedCompareExchangeNodeLink(NodeLink_t volatile *pLink,
        ...
        return ThreadInterlockedAssignIf64x128(
            &pLink->value64x128, value.value64x128, comperand.value64x128);
    }
