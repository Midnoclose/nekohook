...
//			play it back and reproduce it exactly. When playing it back,
//nothing 			is simulated on the server, but all server packets are recorded.
//
//			Most of the VCR mode functionality is accomplished through
//hooks 			called at various points in the engine.
//
// $NoKeywords: $
//===========================================================================//
#ifndef VCRMODE_H
...
#ifdef POSIX
DBG_INTERFACE const char *BuildCmdLine(int argc, char **argv,
                                       bool fAddSteam = true);
...
typedef struct VCR_s {
    ...
    // mode usage on a PER-THREAD basis. The assumption is that you're marking
    // out specific sections of code that you don't want to use VCR mode inside
    // of, but you're not intending to stop all the other threads from using VCR
    // mode.
    void (*SetEnabled)(int bEnabled);

    // This can be called any time to put in a debug check to make sure things
    // are synchronized.
    ...
    // Registry hooks.
    long (*Hook_RegOpenKeyEx)(void *hKey, const tchar *lpSubKey,
                              unsigned long ulOptions, unsigned long samDesired,
                              void *pHKey);
    long (*Hook_RegSetValueEx)(void *hKey, tchar const *lpValueName,
                               unsigned long Reserved, unsigned long dwType,
                               uint8 const *lpData, unsigned long cbData);
    long (*Hook_RegQueryValueEx)(void *hKey, tchar const *lpValueName,
                                 unsigned long *lpReserved,
                                 unsigned long *lpType, uint8 *lpData,
                                 unsigned long *lpcbData);
    long (*Hook_RegCreateKeyEx)(void *hKey, tchar const *lpSubKey,
                                unsigned long Reserved, tchar *lpClass,
    ...
    // correctly. If pEventName is null, then verification is not performed.
    ...
    int (*GenericPlayback)(const tchar *pEventName, void *pOutData, int maxLen,
                           bool bForceLenSame);

    // If you just want to record and playback a value and not worry about
    // whether or not you're recording or playing back, use this. It also will
    // do nothing if you're not recording or playing back.
    //
    // NOTE: also see GenericValueVerify, which allows you to have it VERIFY
    // that pData's contents are the same upon playback (rather than just
    // copying whatever is in the VCR file into pData).
    void (*GenericValue)(const tchar *pEventName, void *pData, int maxLen);

    // Get the current percent (0.0 - 1.0) that it's played back through the
    // file (only valid in playback).
    ...
    void (*GenericValueVerify)(const tchar *pEventName, const void *pData,
                               int maxLen);

    unsigned long (*Hook_WaitForMultipleObjects)(uint32 nHandles,
                                                 const void **pHandles,
