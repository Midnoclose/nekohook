...
#define CONST_INTEGER_AS_STRING(x) \
#x  // Wraps the integer in quotes, allowing us to form constant strings
        // with it
#define __HACK_LINE_AS_STRING__(x) \
    CONST_INTEGER_AS_STRING(       \
        x)  //__LINE__ can only be converted to an actual number by going
            //through this, otherwise the output is literally "__LINE__"
#define __LINE__AS_STRING    \
    __HACK_LINE_AS_STRING__( \
        __LINE__)  // Gives you the line number in constant string form
