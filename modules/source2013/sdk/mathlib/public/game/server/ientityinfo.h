...
//
// Purpose: provides an interface for plugins to query information about
// entities from the game dll
//
//===============================================================================================//
...
abstract_class IEntityInfo {
   public:
    ...
    virtual edict_t *GetGroundEntity() = 0;  // returns the entity that this one
                                             // is standing on - if set.

    // accessor to hook mod specific information about the entity.
    virtual bool GetCustomInfo(int valueType, pluginvariant &outValue,
                               pluginvariant options) = 0;

    // entity debugging stuff.
    virtual const char *GetDebugName() = 0;
    virtual void EntityText(int text_offset, const char *text, float flDuration,
...
abstract_class IEntityInfoManager {
   public:
    ...
    //-----------------------------------------------------------------------------
    virtual edict_t *FindEntityByClassname(edict_t * pStartEntity,
                                           const char *szName) = 0;
    ...
    //-----------------------------------------------------------------------------
    virtual edict_t *FindEntityByName(edict_t * pStartEntity,
                                      const char *szName) = 0;
    ...
    //-----------------------------------------------------------------------------
    virtual edict_t *FindEntityByModel(edict_t * pStartEntity,
    ...
    //			flRadius -
    //-----------------------------------------------------------------------------
    virtual edict_t *FindEntityInSphere(
        edict_t * pStartEntity, const Vector &vecCenter, float flRadius) = 0;
