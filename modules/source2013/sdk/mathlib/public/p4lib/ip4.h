...
struct P4File_t {
    ...
    CUtlSymbol
        m_sFileType;  // the type according to the server, see p4 help filetypes
...
abstract_class IP4 : public IAppSystem {
   public:
    ...
    // submit/revert
    virtual bool SubmitFile(const char *pFullPath,
    ...
    virtual bool OpenFilesForDelete(int nCount,
                                    const char **ppFullPathList) = 0;
    ...
    // Reopens a file for edit or add with the specified filetype. To see the
    // valid strings for filetypes, see p4 help fileinfo. Perforce has to know
    // about files before the filetype can be changed (so for new files, they
    // must be added first, then the filetype modified).
    ...
    // retrieves the last error from the last op (which is likely to span
    // multiple lines) this is only valid after OpenFile[s]For{Add,Edit,Delete}
    // or {Submit,Revert}File[s]
