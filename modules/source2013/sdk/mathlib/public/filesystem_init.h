...
class CFSSteamSetupInfo {
   ...
   public:
    ...
    // steam caches associated with that cache if it's there. This is so apps
    // like Hammer and hlmv can load the main steam caches (like for
    // Counter-Strike or Half-Life 2), and also load the caches that include
    // tools-specific materials (materials\editor, materials\debug, etc).
    //
    // (default: true - should be FALSE for the engine)
    bool m_bToolsMode;

    // If this is true, and m_bToolsMode is false, then it will append the path
    // to steam.dll to the PATH environment variable. This makes it so you can
    // run the engine under Steam without having to copy steam.dll up into your
...
class CFSSearchPathsInit {
   ...
   public:
    ...
    // If this is set, then any search paths with a _english will be replaced
    // with _m_pLanguage and added before the _english path (default: null)
