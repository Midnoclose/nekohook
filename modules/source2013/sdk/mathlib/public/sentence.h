...
class CSentence {
   public:
    ...
    static void ColorForLanguage(int language, unsigned char &r,
    ...
    //				This strips out all of the stuff used by the editor, leaving just
    //one blank work, no sentence text, and just 					the phonemes without the
    //phoneme text...(same as the cacherestore version below)
    void MakeRuntimeOnly();

    // This is a compressed save of just the data needed to drive phonemes in
    // the engine (no word / sentence text, etc )
