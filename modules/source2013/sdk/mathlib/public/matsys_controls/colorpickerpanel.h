...
class CColorPickerFrame : public vgui::Frame {
   ...
   public:
    ...
    // If a color is picked, the message 'ColorPickerPicked' is sent with the
    // "color" field set to the color If cancel is hit, the message
    // 'ColorPickerCancel' is sent If the color is changed in the preview, the
    // message 'ColorPickerPreview' is sent with the "color" field set to the
    // color
...
class CColorPickerButton : public vgui::Button {
   ...
   public:
    CColorPickerButton(vgui::Panel *pParent, const char *pName,
                       vgui::Panel *pActionSignalTarget = NULL);
    ~CColorPickerButton();
