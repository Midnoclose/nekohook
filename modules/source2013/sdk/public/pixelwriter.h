...
class CPixelWriter {
   public:
    ...
    FORCEINLINE void WritePixelNoAdvanceSigned(int r, int g, int b,
                                               int a = 255);
    ...
    // LHS of a SkipBytes)
    FORCEINLINE void WriteFourPixelsExplicitLocation_BGRA8888(FLTX4 rgba,
                                                              int offset);
#endif
    ...
    switch (format) {
        ...
        default: {
            ...
            if (!format_error_printed[format]) {
                Assert(0);
                Msg("CPixelWriter::SetPixelMemory:  Unsupported image format "
                    "%i\n",
                    format);
                format_error_printed[format] = true;
            }
            m_Size = 0;  // set to zero so that we don't stomp memory for
                         // formats that we don't understand.
            m_RShift = 0;
            m_GShift = 0;
            m_BShift = 0;
            m_AShift = 0;
...
FORCEINLINE_PIXEL void CPixelWriter::WritePixelNoAdvanceF(float r, float g,
    ...
    if (PIXELWRITER_USING_16BIT_FLOAT_FORMAT & m_nFlags) {
        ...
        pBuf[m_RShift >> 4] |= (fp16[0].GetBits() & m_RMask)
                               << (m_RShift & 0xF);
        pBuf[m_GShift >> 4] |= (fp16[1].GetBits() & m_GMask)
                               << (m_GShift & 0xF);
        pBuf[m_BShift >> 4] |= (fp16[2].GetBits() & m_BMask)
                               << (m_BShift & 0xF);
        pBuf[m_AShift >> 4] |= (fp16[3].GetBits() & m_AMask)
                               << (m_AShift & 0xF);
...
FORCEINLINE_PIXEL void CPixelWriter::WritePixelNoAdvance(int r, int g, int b,
    ...
    } else  // RGBA32323232 or RGBA16161616 -- PC only.
    {
        ...
        val |= (m_BShift > 0) ? (((int64)(b & m_BMask)) << m_BShift)
                              : (((int64)(b & m_BMask)) >> -m_BShift);
        ...
        switch (m_Size) {
            case 6: {
                if (IsPC() || !IsX360()) {
                    ...
                    ((unsigned short *)m_pBits)[2] =
                        (unsigned short)((val >> 32) & 0xffff);
                } else {
    ...
    switch (m_Size) {
        ...
        case 4: {
            ...
            switch (m_Format) {
                ...
                case IMAGE_FORMAT_BGRA8888:  // NOTE! : the low order bits are
                                             // first in this naming convention.
                case IMAGE_FORMAT_LINEAR_BGRA8888:
                ...
                default:
                    AssertMsg1(false,
                               "Unknown four-byte pixel format %d in lightmap "
                               "write.\n",
                               m_Format);
            }
            break;
        ...
        default:
            AssertMsg1(
                false,
                "WritePixelNoAdvance on unsupported 360 %d-byte format\n",
                m_Size);
            break;
    }
}
...
FORCEINLINE void CPixelWriter::WritePixelNoAdvance_RGBA8888(FLTX4 rgba) {
    ...
    __stvewx(N, m_pBits, 0);  // store whatever word happens to be aligned with
                              // m_pBits to that word
}
...
FORCEINLINE void CPixelWriter::WritePixelNoAdvance_BGRA8888(
    FLTX4 rgba, void *RESTRICT pBits) RESTRICT {
    // this happens to be in an order such that we can use the handy builtin
    // packing op clamp to 0..255 (coz it might have leaked over)
    ...
    static CONST XMVECTOR PackScale = {
        (1.0f / (FLOAT)(1 << 22)), (1.0f / (FLOAT)(1 << 22)),
        (1.0f / (FLOAT)(1 << 22)),
...
FORCEINLINE void CPixelWriter::WriteFourPixelsExplicitLocation_BGRA8888(
    FLTX4 rgba, int offset) {
    Assert((reinterpret_cast<unsigned int>(m_pBits) & 15) ==
           0);  // assert alignment
