...
inline ButtonCode_t GetBaseButtonCode(ButtonCode_t code) {
    ...
    if (IsJoystickPOVCode(code)) {
        int offset =
            (code - JOYSTICK_FIRST_POV_BUTTON) % JOYSTICK_POV_BUTTON_COUNT;
        return (ButtonCode_t)(JOYSTICK_FIRST_POV_BUTTON + offset);
    }

    if (IsJoystickAxisCode(code)) {
        int offset =
            (code - JOYSTICK_FIRST_AXIS_BUTTON) % JOYSTICK_AXIS_BUTTON_COUNT;
...
inline int GetJoystickForCode(ButtonCode_t code) {
    if (!IsJoystickCode(code)) return 0;
...
inline ButtonCode_t ButtonCodeToJoystickButtonCode(ButtonCode_t code,
                                                   int nDesiredJoystick) {
    if (!IsJoystickCode(code) || nDesiredJoystick == 0) return code;
