...
// make "local variable is initialized but not referenced" warnings errors for
// combo checking macros
    ...
    class CShaderParam {                                                    \
       public:                                                              \
        CShaderParam(ShaderMaterialVars_t var, ShaderParamType_t type,      \
                     const char *pDefaultParam, const char *pHelp,          \
                     int nFlags) {                                          \
...
// regarding the macro above: the "::" was added to the first argument in order
// to disambiguate it for GCC. for example, in cloak.cpp, this usage appears:
// 		SHADER_PARAM_OVERRIDE( COLOR, SHADER_PARAM_TYPE_COLOR, "{255 255
// 255}", "unused", SHADER_PARAM_NOT_EDITABLE ) which in turn tries to ask the
// compiler to instantiate an object like so: 		static CShaderParam COLOR(
// (ShaderMaterialVars_t)COLOR, SHADER_PARAM_TYPE_COLOR, "{255 255 255}",
// "unused", SHADER_PARAM_NOT_EDITABLE ) and GCC thinks that the reference to
// COLOR in the arg list is actually a reference to the object we're in the
// middle of making. and you get --> error: invalid cast from type
// ‘Cloak_DX90::CShaderParam’ to type ‘ShaderMaterialVars_t’ Resolved: add the
// "::" so compiler knows that reference is to the enum, not to the name of the
// object being made.
...
//#define CHAIN_SHADER_INIT_PARAMS()	CBaseClass::OnInitShaderParams( params,
//pMaterialName ) #define CHAIN_SHADER_FALLBACK()
//CBaseClass::GetFallbackShader( params ) #define CHAIN_SHADER_INIT()
//CBaseClass::OnInitShaderInstance( params, pShaderInit, pMaterialName ) #define
//CHAIN_SHADER_DRAW()	CBaseClass::OnDrawElements( params, pShaderShadow,
//pShaderAPI )
