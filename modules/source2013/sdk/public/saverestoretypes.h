...
struct entitytable_t {
    ...
    Vector
        landmarkModelSpace;  // a fixed position in model space for comparison
                             // NOTE: Brush models can be built in different
                             // coordiante systems
                             //		in different levels, so this fixes up local
                             //quantities to match 		those differences.
    string_t modelname;
...
struct saverestorelevelinfo_t {
    ...
    levellist_t levelList[MAX_LEVEL_CONNECTIONS];  // List of connections from
                                                   // this level
...
class CGameSaveRestoreInfo {
   public:
    ...
    Vector GetLandmark() const {
        return (levelInfo.fUseLandmark) ? levelInfo.vecLandmarkOffset
                                        : vec3_origin;
    }
    ...
    int GetEntityIndex(const CBaseEntity *pEntity) {
        ...
        if (pEntity) {
            if (m_EntityToIndex.Count()) {
                UtlHashHandle_t hElement =
                    m_EntityToIndex.Find(CHashElement(pEntity));
    ...
    Vector modelSpaceOffset;  // used only for globaly entity brushes modelled
                              // in different coordinate systems.

   private:
    int tableCount;  // Number of elements in the entity table
    entitytable_t
        *pTable;  // Array of entitytable_t elements (1 for each entity)
    ...
    class CHashFuncs {
       public:
        ...
        // COMPARE
        bool operator()(const CHashElement &lhs,
                        const CHashElement &rhs) const {
...
inline unsigned short CSaveRestoreSegment::FindCreateSymbol(
    ...
    for (int i = 0; i < tokenCount; i++) {
        ...
        if (i > 50 && !beentheredonethat) {
            ...
            AssertMsg(0,
                      ("CSaveRestoreBuffer::TokenHash() is getting too full!"));
        }
#endif
