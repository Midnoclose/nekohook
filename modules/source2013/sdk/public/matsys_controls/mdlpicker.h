...
class CMDLPicker : public CBaseAssetPicker {
   ...
   public:
    ...
    void SelectMDL(const char *pRelativePath, bool bDoLookAt = true,
                   int nSelectSecondary = 0);
   ...
   private:
    ...
    const char *CaptureModel(int nModIndex, const char *AssetName,
                             const char *OutputPath, int Width, int Height,
...
class CMDLPickerFrame : public CBaseAssetPickerFrame {
   ...
   public:
    CMDLPickerFrame(vgui::Panel *pParent, const char *pTitle,
                    int nFlags = CMDLPicker::PAGE_ALL);
