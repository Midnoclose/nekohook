...
// Set prefix to use for minidump files.  If you don't set one, it is defaulted
// for you, using the current module name
PLATFORM_INTERFACE void SetMinidumpFilenamePrefix(const char *pszPrefix);

// Set comment to put into minidump file upon next call of WriteMiniDump.  (Most
// common use is the assert text.)
...
PLATFORM_INTERFACE void CatchAndWriteMiniDump(FnWMain pfn, int argc,
                                              tchar *argv[]);  // action = Abort
PLATFORM_INTERFACE void CatchAndWriteMiniDumpForVoidPtrFn(
    FnVoidPtrFn pfn, void *pv,
    bool bExitQuietly);  // action = abort if bExitQuietly, Rethrow otherwise

PLATFORM_INTERFACE void CatchAndWriteMiniDumpEx(
    FnWMain pfn, int argc, tchar *argv[], ECatchAndWriteMinidumpAction eAction);
PLATFORM_INTERFACE int CatchAndWriteMiniDumpExReturnsInt(
    FnWMainIntRet pfn, int argc, tchar *argv[],
    ECatchAndWriteMinidumpAction eAction);
PLATFORM_INTERFACE void CatchAndWriteMiniDumpExForVoidPtrFn(
    FnVoidPtrFn pfn, void *pv, ECatchAndWriteMinidumpAction eAction);
...
typedef void (*FnMiniDump)(unsigned int uStructuredExceptionCode,
...
PLATFORM_INTERFACE bool WriteMiniDumpUsingExceptionInfo(
    ...
    int /* MINIDUMP_TYPE */ minidumpType, const char *pszFilenameSuffix = NULL,
...
//
// There currently is a single header string, and an array of 64 comment
// strings.
//	MinidumpUserStreamInfoSetHeader() will set the single header string.
//	MinidumpUserStreamInfoAppend() will round robin through and array and
//set the comment strings, overwriting old.
