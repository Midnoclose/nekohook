...
//			from the 360's Performance Monitor Counters. In particular
//we 			are interested in l2 cache misses and load-hit-stores.
//
//=============================================================================//
#ifndef CPMCDATA_H
#define CPMCDATA_H
...
class CPMCData {
   public:
    ...
    /// This function should be called exactly once during the lifespan of the
    /// program; it will set up the counters to record the information we are
    /// interested in. This will stomp on whoever else might have set the
    /// performance counters elsewhere in the game.
    ...
    // GetL2CacheMisses
    //-------------------------------------------------------------------------
    uint64 GetL2CacheMisses(void) const { return m_Delta.L2CacheMiss; }

    uint64 GetLHS(void) const { return m_Delta.LHS; }

    /*
    #ifdef DBGFLAG_VALIDATE
            void Validate( CValidator &validator, tchar *pchName );		//
    Validate our internal structures #endif // DBGFLAG_VALIDATE
    */
