...
// Generates a filename under the 'game' subdirectory given a subdirectory of
// 'content'
void ComputeModFilename(const char *pContentFileName, char *pBuf,
                        size_t nBufLen);

// Generates a filename under the 'content' subdirectory given a subdirectory of
// 'game'
void ComputeModContentFilename(const char *pGameFileName, char *pBuf,
                               size_t nBufLen);
...
FORCEINLINE const char *AdjustFileExtensionForPlatform(const char *pSourceName,
                                                       char *pTargetName,
                                                       int targetLen) {
#ifdef PLATFORM_X360
...
class CBaseFile {
   public:
    ...
    FileHandle_t Handle(void) const { return m_FileHandle; }
    ...
    unsigned int Tell() { return g_pFullFileSystem->Tell(m_FileHandle); }
...
class CAppendTextFile : public CBaseFile {
   public:
    void Open(char const *pFname) { CBaseFile::Open(pFname, "a+"); }

    CAppendTextFile(char const *pFname) : CBaseFile() { Open(pFname); }
