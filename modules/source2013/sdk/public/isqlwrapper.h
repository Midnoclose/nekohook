...
class IResultSet {
   public:
    ...
    virtual const ISQLRow *
    PSQLRowNextResult() = 0;  // note, not const on the class because it makes a
                              // new row object
};
...
// NOTE - you can only have one outstanding query at a time. When you are done
// with a query call FreeResult()
//-----------------------------------------------------------------------------
class ISQLWrapper {
   public:
    // run a SQL statement against the DB, typically an insert statement as no
    // data is returned
    ...
    // run a query against the db and then iterate the result set (you can only
    // have 1 outstanding query at a time, and call FreeResults() when you are
    // done)
    virtual IResultSet *PResultSetQuery(const char *pchQueryString) = 0;
    // you MUST call then after you finish with the IResultSet from the above
    // query
    virtual void FreeResult() = 0;
};

//-----------------------------------------------------------------------------
// This is a factory to create objects that let you interact with a MySQL
// database. Make sure you Free() any interfaces you create.
//-----------------------------------------------------------------------------
class ISQLWrapperFactory {
   public:
    // setup details about this db connection
    virtual ISQLWrapper *Create(const char *pchDB, const char *pchHost,
