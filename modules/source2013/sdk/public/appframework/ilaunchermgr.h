...
// if you rev this version also update materialsystem/cmaterialsystem.cpp
// CMaterialSystem::Connect as it defines the string directly
...
class ILauncherMgr : public IAppSystem {
   public:
    ...
    // Create the window.
    virtual bool CreateGameWindow(const char *pTitle, bool bWindowed, int width,
                                  int height) = 0;

    virtual void IncWindowRefCount() = 0;
    virtual void DecWindowRefCount() = 0;

    // Get the next N events. The function returns the number of events that
    // were filled into your array.
    virtual int GetEvents(CCocoaEvent *pEvents, int nMaxEventsToReturn,
                          bool debugEvents = false) = 0;
#ifdef LINUX
    virtual int PeekAndRemoveKeyboardEvents(bool *pbEsc, bool *pbReturn,
                                            bool *pbSpace,
                                            bool debugEvents = false) = 0;
#endif
    ...
    virtual void SetWindowFullScreen(bool bFullScreen, int nWidth,
                                     int nHeight) = 0;
    ...
    virtual void GetNativeDisplayInfo(
        int nDisplay, uint &nWidth, uint &nHeight,
        uint &nRefreshHz) = 0;  // Retrieve the size of the monitor (desktop)
    virtual void RenderedSize(uint &width, uint &height,
                              bool set) = 0;  // either set or retrieve rendered
                                              // size value (from dxabstract)
    virtual void DisplayedSize(
        uint &width, uint &height) = 0;  // query backbuffer size (window size
                                         // whether FS or windowed)

#if defined(DX_TO_GL_ABSTRACTION)
    virtual PseudoGLContextPtr GetMainContext() = 0;
    // Get the NSGLContext for a window's main view - note this is the carbon
    // windowref as an argument
    ...
    virtual void GetDesiredPixelFormatAttribsAndRendererInfo(
        uint **ptrOut, uint *countOut, GLMRendererInfoFields *rendInfoOut) = 0;
    ...
    virtual void SetGammaRamp(const uint16 *pRed, const uint16 *pGreen,
                              const uint16 *pBlue) = 0;
...
enum CocoaEventType_t {
    ...
    CocoaEvent_AppQuit,
    CocoaEvent_Deleted,  // Event was one of the above, but has been handled and
                         // should be ignored now.
};

// enum values need to match bit-shifting logic in
// CInputSystem::UpdateMouseButtonState and the codes from NSEvent
// pressedMouseButtons, turns out the two are in agreement right now
...
class CCocoaEvent {
   public:
    ...
    int m_MousePos[2];
    int m_MouseButtonFlags;  // Current state of the mouse buttons. See
                             // COCOABUTTON_xxxx.
    uint m_nMouseClickCount;
    int m_MouseButton;  // which of the CocoaMouseButton_t buttons this is for
                        // from above
};
