...
abstract_class IPhysicsCollision {
   public:
    ...
    // produce a set of convex triangles from a convex polygon, normal is
    // assumed to be on the side with forward point ordering, which should be
    // clockwise, output will need to be able to hold exactly (iPointCount-2)
    // convexes
    virtual void ConvexesFromConvexPolygon(
        const Vector &vPolyNormal, const Vector *pPoints, int iPointCount,
...
abstract_class IPhysicsShadowController {
   public:
    ...
    virtual void GetMaxSpeed(float *pMaxSpeedOut,
                             float *pMaxAngularSpeedOut) = 0;
};
...
abstract_class IPhysicsEnvironment {
   public:
    ...
    // returns the current simulation clock's value at the next frame.  This is
    // an absolute time.
...
abstract_class IPhysicsObject {
   public:
    ...
    // NOTE: Contact points aren't updated when collision rules change, call
    // this to force an update UNDONE: Force this in RecheckCollisionFilter() ?
    ...
    // update
    virtual void GetImplicitVelocity(
        Vector * velocity, AngularImpulse * angularVelocity) const = 0;
    ...
    virtual void LocalToWorld(Vector * worldPosition,
                              const Vector &localPosition) const = 0;
    virtual void WorldToLocal(Vector * localPosition,
                              const Vector &worldPosition) const = 0;
    ...
    virtual void CalculateVelocityOffset(
        const Vector &forceVector, const Vector &worldPosition,
        Vector *centerVelocity, AngularImpulse *centerAngularVelocity)
        const = 0;
    ...
    // BUGBUG: Use CreateFrictionSnapshot instead of this - this is a simple
    // hack
