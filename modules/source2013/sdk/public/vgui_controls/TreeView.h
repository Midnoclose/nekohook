...
class TreeView : public Panel {
   ...
   public:
    ...
    virtual void AddSelectedItem(int itemIndex, bool clearCurrentSelection,
    ...
    virtual void FindNodesInRange(int startItem, int endItem,
                                  CUtlVector<int> &itemIndices);
    ...
    virtual void SetItemSelectionUnfocusedBgColor(int itemIndex,
                                                  const Color &clr);
    ...
    virtual void OnLabelChanged(int itemIndex, char const *oldString,
    ...
    */
    ...
    // By default, the tree view expands nodes on left-click. This
    ...
    virtual bool IsItemDroppable(int itemIndex,
                                 CUtlVector<KeyValues *> &msglist);
    virtual void OnItemDropped(int itemIndex, CUtlVector<KeyValues *> &msglist);
    virtual bool GetItemDropContextMenu(int itemIndex, Menu *menu,
                                        CUtlVector<KeyValues *> &msglist);
    virtual HCursor GetItemDropCursor(int itemIndex,
                                      CUtlVector<KeyValues *> &msglist);
    ...
    void SetMultipleItemDragEnabled(
        bool state);  // if this is set, then clicking on one row and dragging
                      // will select a run or items, etc.
   ...
   protected:
    ...
    // called when a node, marked as "Expand", needs to generate it's child
    // nodes when expanded
    virtual void GenerateChildrenOfNode(int itemIndex) {}

    // override to open a custom context menu on a node being selected and
    // right-clicked
