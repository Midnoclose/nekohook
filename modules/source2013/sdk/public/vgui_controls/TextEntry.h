...
class TextEntry : public Panel {
   ...
   public:
    ...
    virtual void GetText(OUT_Z_BYTECAP(bufLenInBytes) char *buf,
                         int bufLenInBytes);
    virtual void GetText(OUT_Z_BYTECAP(bufLenInBytes) wchar_t *buf,
                         int bufLenInBytes);
    ...
    // sets the width of the window so all text is visible. (will create one
    // line) used by tooltips
    void SetToFullWidth();
    ...
    */
    ...
    /* MESSAGE SENDING (to action signal targets)
            ...
            "TextNewLine" - sent when the end key is pressed in the text entry
       AND _sendNewLines is true
   ...
   protected:
    ...
    virtual void OnKeyCodePressed(
        ButtonCode_t code);  // respond to keyboard events
    ...
    virtual void OnMousePressed(
        MouseCode code);  // respond to mouse down events
   ...
   public:
    virtual void GetTextRange(
        wchar_t *buf, int from,
        int numchars);  // copy a portion of the text to the buffer and add
                        // zero-termination
    virtual void GetTextRange(
        char *buf, int from,
        int numchars);  // copy a portion of the text to the buffer and add
   ...
   private:
    ...
    CUtlVector<int> m_LineBreaks;  // an array that holds the index in the
                                   // buffer to wrap lines at
    ...
    bool _mouseSelection;  // whether we are highlighting text or not (selecting
                           // text)
    bool _mouseDragSelection;     // tells weather mouse is outside window and
                                  // button is down so we select text
    ...
    int _select[2];  // select[1] is the offset in the text to where the cursor
                     // is currently select[0] is the offset to where the cursor
                     // was dragged to. or -1 if no drag.
    int _pixelsIndent;
    int _charCount;
    ...
    bool _selectAllOnFirstFocus : 1;  // highlights all text in window when
                                      // focus is gained.
