...
class Label : public Panel {
   ...
   public:
    ...
    virtual void GetText(OUT_Z_BYTECAP(bufLenInBytes) wchar_t *textOut,
                         int bufLenInBytes);
    ...
    // Moves where the default text image is within the image array (it starts
    // in position 0) Setting it to -1 removes it from the image list Returns
    // the index the default text image was previously in
   ...
   private:
    ...
    TextImage
        *_textImage;  // this is the textImage, if the full text will not
    ...
    int _textImageIndex;  // index in the image array that the default
                          // _textimage resides
