...
class Frame : public EditablePanel {
   ...
   public:
    ...
    // Set the text in the title bar.  Set surfaceTitle=true if you want this to
    // be the taskbar text as well.
    virtual void SetTitle(const char *title, bool surfaceTitle);
    virtual void SetTitle(const wchar_t *title, bool surfaceTitle);

    // Bring the frame to the front and requests focus, ensures it's not
    // minimized
    virtual void Activate();

    // activates the dialog; if dialog is not currently visible it starts it
    // minimized and flashing in the taskbar
    ...
    void SetImages(const char *pEnabledImage,
                   const char *pDisabledImage = NULL);
    ...
    */
    ...
    // Temporarily enables or disables the fade effect rather than zeroing the
    // fade times as done in DisableFadeEffect
   ...
   protected:
    ...
    // Respond when a close message is recieved.  Can be called directly to
    // close a frame.
    virtual void OnClose();
    // Respond to a window finishing its closure. i.e. when a fading window has
    // fully finished its fadeout.
    virtual void OnFinishedClose();
    // Minimize the window on the taskbar.
    MESSAGE_FUNC(OnMinimize, "Minimize");
    // Called when minimize-to-systray button is pressed (does nothing by
    // default)
    ...
    // gets the default position and size on the screen to appear the first time
   ...
   private:
    ...
    CPanelAnimationVarAliasType(int, m_iTitleTextInsetXOverride,
                                "titletextinsetX", "0", "proportional_int");
