...
// So that an app can have a "custom" tool window class created during window
// drag/drop operations on the property sheet
class IToolWindowFactory {
   public:
    virtual ToolWindow *InstanceToolWindow(Panel *parent, bool contextLabel,
                                           Panel *firstPage, char const *title,
                                           bool contextMenu) = 0;
};
...
class ToolWindow : public Frame {
   ...
   public:
    ToolWindow(Panel *parent, bool contextLabel,
               IToolWindowFactory *factory = 0, Panel *page = NULL,
