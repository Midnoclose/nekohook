...
class MenuButton : public Button {
   ...
   public:
    ...
    MESSAGE_FUNC_PARAMS(OnKillFocus, "KillFocus",
                        kv);  // called after the panel loses the keyboard focus
    virtual void DoClick();
    virtual void SetOpenOffsetY(int yOffset);

    virtual bool CanBeDefaultButton(void);

    // sets the direction in which the menu opens from the button, defaults to
    // down
    ...
    // This style is like the IE "back" button where the left side acts like a
    // regular button, the the right side has a little
