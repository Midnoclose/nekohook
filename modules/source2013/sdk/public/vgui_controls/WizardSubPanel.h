...
class WizardSubPanel : public EditablePanel {
   ...
   public:
    ...
    // called anytime the panel is first displayed, whether the user is moving
    // forward or back called immediately after OnDisplayAsNext/OnDisplayAsPrev
    ...
    // returns true if this panel should be displayed, or if we should just skip
    // over it
    virtual bool ShouldDisplayPanel() { return true; }

    // return true if this subpanel doesn't need the next/prev/finish/cancel
    // buttons or will do it itself
    ...
    // returning NULL tells it to use the immediate previous panel in the
    // history
