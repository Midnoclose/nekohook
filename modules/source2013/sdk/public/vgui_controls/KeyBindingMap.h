...
//-----------------------------------------------------------------------------
// Purpose: An actual keybinding, where bindingname references a bindingmap
// mentioned below
//-----------------------------------------------------------------------------
struct BoundKey_t {
    BoundKey_t();
    ...
    bool isbuiltin;           // whether this was by the #DECLARE macros or via
                              // code/parsing a config file
...
#define DECLARE_KEYBINDINGMAP(className)                                    \
    static void KB_AddToMap(                                                \
        char const *bindingname, ButtonCode_t defaultcode,                  \
        int default_modifiers, vgui::MessageFunc_t function,                \
        char const *helpstring, char const *docstring, bool passive) {      \
        vgui::PanelKeyBindingMap *map =                                     \
            vgui::FindOrAddPanelKeyBindingMap(GetPanelClassName());         \
                                                                            \
    ...
    static void KB_ChainToMap(void) {                                       \
        ...
        if (map && GetPanelBaseClassName() && GetPanelBaseClassName()[0]) { \
            map->baseMap =                                                  \
                vgui::FindOrAddPanelKeyBindingMap(GetPanelBaseClassName()); \
        }                                                                   \
    }                                                                       \
                                                                            \
    static void KB_AddBoundKey(char const *bindingname, int keycode,        \
                               int modifiers) {                             \
        vgui::PanelKeyBindingMap *map =                                     \
            vgui::FindOrAddPanelKeyBindingMap(GetPanelClassName());         \
        vgui::BoundKey_t kb;                                                \
    ...
    class className##_RegisterKBMap {                                       \
       public:                                                              \
        className##_RegisterKBMap() { className::KB_ChainToMap(); }         \
    };                                                                      \
    className##_RegisterKBMap m_RegisterClassKB;                            \
                                                                            \
    virtual vgui::PanelKeyBindingMap *GetKBMap() {                          \
        static vgui::PanelKeyBindingMap *s_pMap =                           \
            vgui::FindOrAddPanelKeyBindingMap(GetPanelClassName());         \
        return s_pMap;                                                      \
    }

#define _KBMapFuncCommonFunc(name, keycode, modifiers, function, help, doc,  \
                             passive)                                        \
    ...
    class PanelKBMapFunc_##name {                                            \
       public:                                                               \
        static void InitVar() {                                              \
            ...
            if (!bAdded) {                                                   \
                bAdded = true;                                               \
                KB_AddToMap(#name, keycode, modifiers,                       \
                            (vgui::MessageFunc_t)&ThisClass::function, help, \
                            doc, passive);                                   \
            }                                                                \
        }                                                                    \
        PanelKBMapFunc_##name() { PanelKBMapFunc_##name::InitVar(); }        \
    };                                                                       \
...
#define KEYBINDING_FUNC(name, keycode, modifiers, function, help, doc)  \
    _KBMapFuncCommonFunc(name, keycode, modifiers, function, help, doc, \
                         false);                                        \
    virtual void function()
#define KEYBINDING_FUNC_NODECLARE(name, keycode, modifiers, function, help, \
                                  doc)                                      \
...
#define KEYBINDING_FUNC_PASSIVE(name, keycode, modifiers, function, help, doc) \
    ...
    virtual void function()
#define KEYBINDING_FUNC_PASSIVE_NODECLARE(name, keycode, modifiers, function, \
                                          help, doc)                          \
    _KBMapFuncCommonFunc(name, keycode, modifiers, function, help, doc, true);

// For definding additional (non-default) keybindings
#define KEYBINDING_ADDBINDING(name, keycode, modifiers) \
    _KBBindKeyCommon(name, keycode, modifiers, name);
