...
class IPanelAnimationPropertyConverter {
   public:
    ...
    virtual void SetData(Panel *panel, KeyValues *kv,
                         PanelAnimationMapEntry *entry) = 0;
    virtual void InitFromDefault(Panel *panel,
                                 PanelAnimationMapEntry *entry) = 0;
};
...
#define PANEL_ROUND_CORNER_ALL                                   \
    PANEL_ROUND_CORNER_TOP_LEFT | PANEL_ROUND_CORNER_TOP_RIGHT | \
        PANEL_ROUND_CORNER_BOTTOM_LEFT | PANEL_ROUND_CORNER_BOTTOM_RIGHT
//=============================================================================
// HPE_END
//=============================================================================//-----------------------------------------------------------------------------
// Purpose: Base interface to all vgui windows
//			All vgui controls that receive message and/or have a
//physical presence 			on screen are be derived from Panel. 			This is designed as an
//easy-access to the vgui-functionality; for more 			low-level access to vgui
//functions use the IPanel/IClientPanel interfaces directly
//-----------------------------------------------------------------------------
class Panel : public IClientPanel, virtual IForceVirtualInheritancePanel {
   ...
   public:
    ...
    static void AddPropertyConverter(
        char const *typeName, IPanelAnimationPropertyConverter *converter);
    ...
    static char const *KeyCodeToString(ButtonCode_t code);
    static wchar_t const *KeyCodeToDisplayString(ButtonCode_t code);
    static wchar_t const *KeyCodeModifiersToDisplayString(
        ButtonCode_t code, int modifiers);  // L"Ctrl+Alt+Shift+Backspace"

    static ButtonCode_t StringToKeyCode(char const *str);
#if defined(VGUI_USEKEYBINDINGMAPS)
    ...
    //  for specific keys
    virtual bool IsKeyOverridden(ButtonCode_t code, int modifiers);

    virtual void AddKeyBinding(char const *bindingName, int keycode,
                               int modifiers);

    KeyBindingMap_t *LookupBinding(char const *bindingName);
    KeyBindingMap_t *LookupBindingByKeyCode(ButtonCode_t code, int modifiers);
...
class CSortedPanelYLess {
   public:
    bool Less(const SortedPanel_t &src1, const SortedPanel_t &src2,
              void *pCtx) {
        int nX1, nY1, nX2, nY2;
