...
// sorting function, should return true if itemID1 should be displayed before
// itemID2
typedef bool (*SectionSortFunc_t)(SectionedListPanel *list, int itemID1,
                                  int itemID2);
...
class SectionedListPanel : public Panel {
   ...
   public:
    ...
    virtual void AddSection(int sectionID, const char *name,
                            SectionSortFunc_t sortFunc = NULL);
    virtual void AddSection(int sectionID, const wchar_t *name,
                            SectionSortFunc_t sortFunc = NULL);
    virtual void AddSection(int sectionID, SectionedListPanelHeader *pHeader,
                            SectionSortFunc_t sortFunc = NULL);
    ...
    enum ColumnFlags_e {
        HEADER_IMAGE = 0x01,  // set if the header for the column is an image
                              // instead of text
        COLUMN_IMAGE = 0x02,  // set if the column contains an image instead of
                              // text (images are looked up by index from the
                              // ImageList) (see SetImageList below)
        COLUMN_BRIGHT =
            0x04,  // set if the column text should be the bright color
        COLUMN_CENTER = 0x08,  // set to center the text/image in the column
        COLUMN_RIGHT = 0x10,   // set to right-align the text in the column
    };
    virtual bool AddColumnToSection(int sectionID, const char *columnName,
                                    const char *columnText, int columnFlags,
                                    int width,
                                    HFont fallbackFont = INVALID_FONT);
    virtual bool AddColumnToSection(int sectionID, const char *columnName,
                                    const wchar_t *columnText, int columnFlags,
                                    int width,
                                    HFont fallbackFont = INVALID_FONT);

    // modifies the text in an existing column
    virtual bool ModifyColumn(int sectionID, const char *columnName,
    ...
    // removes an item from the list; returns false if the item does not exist
    ...
    */
    ...
    // returns the name of a column by section and column index; returns NULL if
    // there are no more columns valid range of columnIndex is [0,
    // GetColumnCountBySection)
    virtual const char *GetColumnNameBySection(int sectionID, int columnIndex);
    virtual const wchar_t *GetColumnTextBySection(int sectionID,
                                                  int columnIndex);
    ...
    virtual HFont GetColumnFallbackFontBySection(int sectionID,
                                                 int columnIndex);
    ...
    // returns the item ID from the row, again ignoring section dividers - valid
    // from [0, GetItemCount )
    ...
    virtual bool GetCellBounds(int itemID, int column, int &x, int &y,
                               int &wide, int &tall);

    // Gets the coordinates of a section header
    virtual bool GetSectionHeaderBounds(int sectionID, int &x, int &y,
                                        int &wide, int &tall);
    ...
    virtual bool GetMaxCellBounds(int itemID, int column, int &x, int &y,
                                  int &wide, int &tall);

    // gets the local coordinates of an item
    virtual bool GetItemBounds(int itemID, int &x, int &y, int &wide,
                               int &tall);
   ...
   private:
    ...
    // Returns the index of a new item button, reusing an existing item button
    // if possible
...
class SectionedListPanelHeader : public Label {
   ...
   public:
    SectionedListPanelHeader(SectionedListPanel *parent, const char *name,
                             int sectionID);
    SectionedListPanelHeader(SectionedListPanel *parent, const wchar_t *name,
                             int sectionID);
