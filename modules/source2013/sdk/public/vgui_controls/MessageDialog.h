...
#define MD_COMMANDAFTERCLOSE \
    0x0200  // send command at dialog termination (i.e. after fade)
#define MD_RESTRICTPAINT \
    0x0400  // only paint this dialog (hide any other ui elements)
#define MD_COMMANDONFORCECLOSE \
    0x0800  // send command when the dialog is closed assuming A input
...
class CMessageDialog : public vgui::Frame {
   ...
   public:
    CMessageDialog(vgui::Panel *parent, const uint nType, const char *pTitle,
                   const char *pMsg, const char *pCmdA, const char *pCmdB,
                   vgui::Panel *pParent, bool bShowActivity);
    ~CMessageDialog();
    ...
    virtual void OnKeyCodePressed(vgui::ButtonCode_t code);
   ...
   private:
    void CreateButtonLabel(ButtonLabel_s *pButton, const char *pIcon,
                           const char *pText);
