...
#define VecToString(v)                        \
    (static_cast<const char*>(                \
        CFmtStr("(%f, %f, %f)", (v).x, (v).y, \
                (v).z)))  // ** Note: this generates a temporary, don't hold
                          // reference!
...
class Vector {
   public:
    ...
    // NJS: note, VC wasn't inlining it correctly in several deeply nested
    // inlines due to being an 'out of line' inline. may be able to tidy this up
    // after switching to VC7
...
// Don't mark this as inline in its function declaration. That's only necessary
// on its definition, and 'inline' here leads to gcc warnings.
...
// check if a point is in the field of a view of an object. supports up to 180
// degree fov.
...
FORCEINLINE vec_t InvRSquared(float const* v) {
    ...
    _mm_store_ss(&result, _mm_rcp_ss(_mm_max_ss(_mm_set_ss(1.0f),
                                                _mm_load_ss(&sqrlen))));
    return result;
#else
