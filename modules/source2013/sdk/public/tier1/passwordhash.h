...
// k_EHashBigPassword: For testing purposes, a salted SHA-1 hash extended to 32
// bytes, with 6 bytes of 0x1 on either side. k_EHashPBKDF2_1000: A PKCS#5 v2.0
// Password-Based Key Derivation Function hash (PBKDF2-HMAC-SHA256) with 1,000
// iterations.
//						The digest width is 32 bytes.
// k_EHashPBKDF2_5000: A PKCS#5 v2.0 Password-Based Key Derivation Function hash
// (PBKDF2-HMAC-SHA256) with 5,000 iterations. k_EHashPBKDF2_10000: A PKCS#5
// v2.0 Password-Based Key Derivation Function hash (PBKDF2-HMAC-SHA256) with
// 10,000 iterations. k_EHashSHA1WrappedWithPBKDF2_10000: A SHA-1 hash which is
// then further hashed with PBKDF2 at 10,000 rounds.  Used for
//										strengthening old
//hashes in the database that haven't been logged in in a long time.
//
// Make sure to update k_EHashMax when adding new hash types.  Also add the
// length into the k_HashLengths array below.
...
#if defined(C_ASSERT)
//
// If you're hitting this assert at compile time, it means that you've added a
// new hash type and properly updated k_EHashMax, but you forgot to add the
// length of the new hash type into k_HashLengths.  So do that.
//
