...
// $NoKeywords: $
//
// A Fixed-allocation class for maintaining a 1d or 2d or 3d array of data in a
// structure-of-arrays (SOA) sse-friendly manner.
// =============================================================================//
...
// strided pointers. gives you a class that acts like a pointer, but the ++ and
// += operators do the right thing
template <class T>
class CStridedPtr {
   protected:
    T *m_pData;
...
// allowed field data types. if you change these values, you need to change the
// tables in the .cpp file
...
class CSOAContainer {
   protected:
    ...
    size_t m_nStrideInBytes[MAX_SOA_FIELDS];  // stride from one field datum to
                                              // another
    size_t m_nRowStrideInBytes[MAX_SOA_FIELDS];  // stride from one row datum to
                                                 // another per field
    size_t m_nSliceStrideInBytes[MAX_SOA_FIELDS];  // stride from one slice
                                                   // datum to another per field
   ...
   public:
    ...
    // #define ATTR_GREEN 1
    // #define ATTR_BLUE 2
    // CSOAContainer myimage( 256, 256, ATTR_RED, ATTRDATATYPE_FLOAT,
    // ATTR_GREEN, ATTRDATATYPE_FLOAT,
    ...
    // set the data type for an attribute. If you set the data type, but tell it
    // not to allocate, the data type will be set but writes will assert, and
    // reads will give you back zeros.
    FORCEINLINE void SetAttributeType(int nAttrIdx,
    ...
    FORCEINLINE int NumCols(void) const { return m_nColumns; }
    FORCEINLINE int NumSlices(void) const { return m_nSlices; }

    FORCEINLINE void AssertDataType(int nAttrIdx,
                                    EAttributeDataType nDataType) const {
        Assert(nAttrIdx >= 0);
        Assert(nAttrIdx < MAX_SOA_FIELDS);
    ...
    // how much to step to go from the end of one row to the start of the next
    // one. Basically, how many bytes to add at the end of a row when iterating
    // over the whole 2d array with ++
    FORCEINLINE size_t RowToRowStep(int nAttrIdx) const { return 0; }

    FORCEINLINE void *RowPtr(int nAttributeIdx, int nRowNumber,
                             int nSliceNumber = 0) const {
    ...
    FORCEINLINE void const *ConstRowPtr(int nAttributeIdx, int nRowNumber,
                                        int nSliceNumber = 0) const {
        Assert(nRowNumber < m_nRows);
        Assert(nAttributeIdx < MAX_SOA_FIELDS);
        Assert(m_nDataType[nAttributeIdx] != ATTRDATATYPE_NONE);
    ...
    template <class T>
    FORCEINLINE T *ElementPointer(int nAttributeIdx, int nX = 0, int nY = 0,
                                  int nZ = 0) const {
        Assert(nAttributeIdx < MAX_SOA_FIELDS);
    ...
    // copy the attribute data from another soacontainer. must be compatible
    // geometry
    void CopyAttrFrom(CSOAContainer const &other, int nAttributeIdx);

    // copy the attribute data from another attribute. must be compatible data
    // format
    void CopyAttrToAttr(int nSrcAttributeIndex, int nDestAttributeIndex);

    // move all the data from one csoacontainer to another, leaving the source
    // empty. this is just a pointer copy.
    FORCEINLINE void MoveDataFrom(CSOAContainer other) {
        (*this) = other;
        other.Init();
    }

    void AllocateData(int nNCols, int nNRows,
                      int nSlices = 1);  // actually allocate the memory and set
                                         // the pointers up
    ...
    void FillAttrWithInterpolatedValues(int nAttr, float flValue00,
                                        float flValue10, float flValue01,
                                        float flValue11) const;
    void FillAttrWithInterpolatedValues(int nAttr, Vector flValue00,
                                        Vector flValue10,
                                        Vector const &flValue01,
                                        Vector const &flValue11) const;
};

class CFltX4AttributeIterator : public CStridedConstPtr<fltx4> {
    FORCEINLINE CFltX4AttributeIterator(CSOAContainer const *pContainer,
                                        int nAttribute, int nRowNumber = 0)
        : CStridedConstPtr<fltx4>(
              pContainer->ConstRowPtr(nAttribute, nRowNumber),
              pContainer->ItemByteStride(nAttribute)) {}
};

class CFltX4AttributeWriteIterator : public CStridedPtr<fltx4> {
    FORCEINLINE CFltX4AttributeWriteIterator(CSOAContainer const *pContainer,
                                             int nAttribute, int nRowNumber = 0)
