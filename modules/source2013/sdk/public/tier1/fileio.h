...
class CPathString {
   public:
    // Constructors: Automatically fixes slashes and removes double slashes when
    // the object is constructed, and then knows how to append magic \\?\ on
    // Windows for unicode paths
    ...
    // Gets wchar_t based path, with \\?\ pre-pended (allowing long paths on
    // Win32, should only be used with unicode aware filesystem calls)
