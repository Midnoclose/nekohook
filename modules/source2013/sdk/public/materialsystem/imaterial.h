...
inline VertexCompressionType_t CompressionType(VertexFormat_t vertexFormat) {
    // This is trivial now, but we may add multiple flavours of compressed
    // vertex later on
...
{
    // Make it harder for someone to change VertexElement_t without noticing
    // that dependent code (GetVertexElementSize, VertexElementToDeclType,
    ...
    switch (element) {
        ...
        default:
            Assert(
                0);  // Invalid input or VertexElement_t has definitely changed
            break;
    }
}
