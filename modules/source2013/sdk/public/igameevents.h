...
#define INTERFACEVERSION_GAMEEVENTSMANAGER \
    "GAMEEVENTSMANAGER001"  // old game event manager, don't use it!
#define INTERFACEVERSION_GAMEEVENTSMANAGER2 \
    "GAMEEVENTSMANAGER002"  // new game event manager,
...
abstract_class IGameEvent {
   public:
    ...
    virtual bool IsEmpty(const char *keyName =
                             NULL) = 0;  // check if data field exists
...
abstract_class IGameEventManager2 : public IBaseInterface {
   public:
    ...
    virtual IGameEvent *UnserializeEvent(
        bf_read * buf) = 0;  // create new KeyValues, must be deleted
};
...
abstract_class IGameEventManager : public IBaseInterface {
   public:
    ...
    virtual KeyValues *GetEvent(
        const char *name) = 0;  // returns keys for event

    // adds a listener for a particular event
    virtual bool AddListener(IGameEventListener * listener, const char *event,
                             bool bIsServerSide) = 0;
    // registers for all known events
    virtual bool AddListener(IGameEventListener * listener,
                             bool bIsServerSide) = 0;
    ...
    // write/read event to/from bitbuffer
    virtual bool SerializeKeyValues(KeyValues * event, bf_write * buf,
                                    CGameEvent *eventtype = NULL) = 0;
    virtual KeyValues *UnserializeKeyValue(
        bf_read * msg) = 0;  // create new KeyValues, must be deleted
};
