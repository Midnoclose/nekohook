...
#define INVALID_STEAM_LOGON_TICKET_CANCELED \
...
#define MAX_PLAYERS_PER_CLIENT \
    XUSER_MAX_COUNT  // Xbox 360 supports 4 players per console
#else
...
// Max name used in save files. Needs to be left at 32 for SourceSDK
// compatibility.
...
// PRECISION This top block is for singleplayer games only....no HL2:DM (which
// defines HL2_DLL)
#if !defined(HL2MP) && (defined(PORTAL) || defined(HL2_EPISODIC) || \
                        defined(HL2_DLL) || defined(HL2_LOSTCOAST))
...
#define FL_BASEVELOCITY \
    (1 << 23)  // Base velocity has been applied this frame (used to convert
               // base velocity into momentum)
#define FL_WORLDBRUSH \
    (1                \
     << 24)  // Not moveable/removeable brush entity (really part of the world,
             // but represented as an entity for transparency or something)
#define FL_OBJECT \
    (1 << 25)  // Terrible name. This is an object that NPCs should see.
...
#define FL_ANIMDUCKING \
    (1 << 2)  // Player flag -- Player is in the process of crouching or
              // uncrouching but could be in transition
// examples:                                   Fully ducked:  FL_DUCKING &
// FL_ANIMDUCKING
//           Previously fully ducked, unducking in progress:  FL_DUCKING &
//           !FL_ANIMDUCKING
//                                           Fully unducked: !FL_DUCKING &
//                                           !FL_ANIMDUCKING
//           Previously fully unducked, ducking in progress: !FL_DUCKING &
//           FL_ANIMDUCKING
...
#define FL_FAKECLIENT \
    (1 << 9)  // Fake client, simulated server side; don't send network messages
              // to them
// NON-PLAYER SPECIFIC (i.e., not used by GameMovement or the client .dll ) --
// Can still be applied to players, though
...
#define FL_DONTTOUCH \
    (1 << 23)  // Doesn't generate touch functions, generates Untouch() for
               // anything it was touching when this flag was set
#define FL_BASEVELOCITY \
    (1 << 24)  // Base velocity has been applied this frame (used to convert
               // base velocity into momentum)
#define FL_WORLDBRUSH \
    (1                \
     << 25)  // Not moveable/removeable brush entity (really part of the world,
             // but represented as an entity for transparency or something)
#define FL_OBJECT \
    (1 << 26)  // Terrible name. This is an object that NPCs should see.
               // Missiles, for example.
#define FL_KILLME \
...
#define FL_UNBLOCKABLE_BY_PLAYER \
    (1 << 31)  // pusher that can't be blocked by the player
#endif
...
enum MoveCollide_t {
    ...
    MOVECOLLIDE_FLY_BOUNCE,  // bounces, reflects, based on elasticity of
                             // surface and object - applies friction (adjust
                             // velocity)
    MOVECOLLIDE_FLY_CUSTOM,  // Touch() will modify the velocity however it
                             // likes
...
enum SolidType_t {
    ...
    SOLID_VPHYSICS = 6,  // solid vphysics object, get vcollide from the model
                         // and collide with that
    SOLID_LAST,
};

enum SolidFlags_t {
    FSOLID_CUSTOMRAYTEST = 0x0001,  // Ignore solid type + always call into the
                                    // entity for ray tests
    ...
    FSOLID_VOLUME_CONTENTS =
        0x0020,  // Contains volumetric contents (like water)
    FSOLID_FORCE_WORLD_ALIGNED =
        0x0040,  // Forces the collision rep to be world-aligned even if it's
                 // SOLID_BSP or SOLID_VPHYSICS
...
enum {
    ...
    EF_BONEMERGE_FASTCULL =
        0x080,  // For use with EF_BONEMERGE. If this is set, then it places
                // this ent's origin at its parent and uses the parent's bbox +
                // the max extents of the aiment. Otherwise, it sets up the
                // parent's bones every frame to figure out where to place the
                // aiment, which is inefficient because it'll setup the parent's
...
enum RenderFx_t {
    ...
    kRenderFxClampMinScale,  // Keep this sprite from getting very small
                             // (SPRITES only!)
...
enum Collision_Group_t {
    ...
    COLLISION_GROUP_INTERACTIVE_DEBRIS,  // Collides with everything except
                                         // other interactive debris or debris
    COLLISION_GROUP_INTERACTIVE,  // Collides with everything except interactive
                                  // debris or debris
    COLLISION_GROUP_PLAYER,
    COLLISION_GROUP_BREAKABLE_GLASS,
    COLLISION_GROUP_VEHICLE,
    COLLISION_GROUP_PLAYER_MOVEMENT,  // For HL2, same as
                                      // Collision_Group_Player, for TF2, this
                                      // filters out other players and
                                      // CBaseObjects
