...
inline const char *servernetadr_t::ToString(uint32 unIP, uint16 usPort) const {
    ...
    V_snprintf(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[0]),
               (int)(ipByte[1]), (int)(ipByte[2]), (int)(ipByte[3]), usPort);
#else
    V_snprintf(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[3]),
...
class gameserveritem_t {
   ...
   public:
    ...
    char
        m_szGameDescription[k_cbMaxGameServerGameDescription];  ///< game
                                                                ///< description
