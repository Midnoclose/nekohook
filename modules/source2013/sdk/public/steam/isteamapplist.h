...
// approved apps,
//	contact your Steam Account Manager if you believe you need access to
//this API. 	This interface lets you detect installed apps for the local Steam
//client, useful for debugging tools 	to offer lists of apps to debug via Steam.
//-----------------------------------------------------------------------------
class ISteamAppList {
   public:
    ...
    virtual int GetAppInstallDir(
        AppId_t nAppID, char *pchDirectory,
        int cchNameMax) = 0;  // returns -1 if no dir was found

    virtual int GetAppBuildId(
        AppId_t nAppID) = 0;  // return the buildid of this app, may change at
                              // any time based on backend updates to the game
};
