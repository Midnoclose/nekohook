...
enum EResult {
    ...
    //	k_EResultNoConnectionRetry = 4,				// OBSOLETE -
    //removed
    ...
    k_EResultPending = 22,  // Request is pending (may be in process, or waiting
                            // on third party)
    ...
    k_EResultDuplicateRequest =
        29,  // The request is a duplicate and the action has already occurred
             // in the past, ignored this time
    ...
    k_EResultTryAnotherCM = 48,  // The current CM can't service the user making
    ...
    k_EResultRemoteFileConflict =
        60,  // The sync cannot resume due to a conflict between the local and
             // remote files
    ...
    k_EResultRateLimitExceeded =
        84,  // Temporary rate limit exceeded, try again later, different from
...
enum EAccountType {
    ...
    k_EAccountTypeConsoleUser = 9,  // Fake SteamID for local PSN account on PS3
                                    // or Live account on 360, etc.
...
enum EAppReleaseState {
    k_EAppReleaseState_Unknown =
        0,  // unknown, required appinfo or license info is missing
    k_EAppReleaseState_Unavailable =
        1,  // even if user 'just' owns it, can see game at all
    k_EAppReleaseState_Prerelease =
        2,  // can be purchased and is visible in games list, nothing else.
            // Common appInfo section released
...
enum EAppOwnershipFlags {
    ...
    k_EAppOwnershipFlags_InvalidPlatform =
        0x0010,  // app not supported on current platform
    k_EAppOwnershipFlags_SharedLicense =
        0x0020,  // license was granted by authorized local device
    k_EAppOwnershipFlags_FreeWeekend =
        0x0040,  // owned by a free weekend licenses
    k_EAppOwnershipFlags_RetailLicense =
        0x0080,  // has a retail license for game, (CD-Key etc)
    k_EAppOwnershipFlags_LicenseLocked =
        0x0100,  // shared license is locked (in use) by other user
    k_EAppOwnershipFlags_LicensePending =
        0x0200,  // owns app, but transaction is still pending. Can't install or
                 // play
    k_EAppOwnershipFlags_LicenseExpired =
        0x0400,  // doesn't own app anymore since license expired
    ...
    k_EAppOwnershipFlags_AutoGrant =
        0x4000,  // Ownership is based on any kind of autogrant license
};
...
enum EAppType {
    ...
    k_EAppType_Media_DEPRECATED =
        0x010,  // legacy - was used for game trailers, which are now just
                // videos on the web
    ...
    k_EAppType_Config = 0x100,    // hidden app used to config Steam features
                                  // (backpack, sales, etc)
    k_EAppType_Hardware = 0x200,  // a hardware device (Steam Machine, Steam
                                  // Controller, Steam Link, etc.)
    // 0x400 is up for grabs here
    k_EAppType_Video =
        0x800,  // A video component of either a Film or TVSeries (may be the
                // feature, an episode, preview, making-of, etc)
    ...
    k_EAppType_DepotOnly = 0x80000000,  // placeholder since depots and apps
...
enum EChatRoomEnterResponse {
    ...
    k_EChatRoomEnterResponseLimited =
        7,  // Joining this chat is not allowed because you are a limited user
            // (no value on account)
...
// we allow 3 simultaneous user account instances right now, 1= desktop, 2 =
// console, 4 = web, 0 = all
...
class CSteamID {
   public:
    ...
    CSteamID(uint32 unAccountID, unsigned int unAccountInstance,
        ...
        Assert(
            !((k_EAccountTypeIndividual == eAccountType) &&
              (unAccountInstance >
               k_unSteamUserWebInstance)));  // enforce that for individual
    ...
    // Note:	Will not accept a uint32 or int32 as input, as that is a probable
    // mistake.
    //			See the stubbed out overloads in the private: section for more
    //info.
    //-----------------------------------------------------------------------------
    CSteamID(uint64 ulSteamID) { SetFromUint64(ulSteamID); }
#ifdef INT64_DIFFERENT_FROM_INT64_T
    CSteamID(uint64_t ulSteamID) { SetFromUint64((uint64)ulSteamID); }
#endif
    ...
    //			eUniverse -				universe this ID belongs
    //to
    //-----------------------------------------------------------------------------
    void SetFromSteam2(TSteamGlobalUserID *pTSteamGlobalUserID,
        ...
        m_steamid.m_comp.m_EAccountType =
            k_EAccountTypeIndividual;  // Steam 2 accounts always map to account
                                       // type of individual
        m_steamid.m_comp.m_unAccountInstance =
    ...
    //the 			same static account key, so they can be grouped together by the static
    //			account key.
    ...
    static const char *Render(
        uint64 ulSteamID);  // static method to render a uint64 representation
...
inline bool CSteamID::IsValid() const {
    ...
    if (m_steamid.m_comp.m_EAccountType == k_EAccountTypeGameServer) {
        if (m_steamid.m_comp.m_unAccountID == 0) return false;
...
class CGameID {
   public:
...
#ifdef INT64_DIFFERENT_FROM_INT64_T
    CGameID(uint64_t ulGameID) { m_ulGameID = (uint64)ulGameID; }
#endif
    ...
    static const char *Render(
        uint64 ulGameID);  // static method to render a uint64 representation of
                           // a Game ID to a string
    ...
    CGameID(uint32 nAppID, const char *pchModPath) {
        ...
        // reduces crc32 to 31bits, but lets us use the modID as a guaranteed
        // unique replacement for appID's
    ...
    uint64 *GetUint64Ptr() { return &m_ulGameID; }

    void Set(uint64 ulGameID) { m_ulGameID = ulGameID; }
    ...
    uint32 ModID() const { return m_gameID.m_nModID; }

    uint32 AppID() const { return m_gameID.m_nAppID; }
...
//-----------------------------------------------------------------------------
// Purpose: Used by ICrashHandler interfaces to reference particular installed
// crash handlers
//-----------------------------------------------------------------------------
