...
class UtlVectorTemplate {
   public:
    static void ResizeUtlVector(void *pStruct, int offsetToUtlVector, int len) {
        CUtlVector<T, A> *pVec =
            (CUtlVector<T, A> *)((char *)pStruct + offsetToUtlVector);
        if (pVec->Count() < len)
        ...
        // This is important to do because EnsureCapacity doesn't actually call
        // the constructors on the elements, but we need them to be initialized,
        // otherwise it'll have out-of-range values which will piss off the
        // datatable encoder.
        UtlVector_InitializeAllocatedElements(pVec->Base() + pVec->Count(),
                                              nNumAllocated - pVec->Count());
    }
...
// Allocates a string for a data table name. Data table names must be unique, so
// this will assert if you try to allocate a duplicate.
