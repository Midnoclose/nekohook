...
#define GAMELUMP_MAKE_CODE(a, b, c, d) \
    ((a) << 24 | (b) << 16 | (c) << 8 | (d) << 0)
enum {
...
struct DetailSpriteDictLump_t {
    DECLARE_BYTESWAP_DATADESC();
    // NOTE: All detail prop sprites must lie in the material
...
struct DetailObjectLump_t {
    ...
    Vector m_Origin;
    QAngle m_Angles;
    unsigned short m_DetailModel;  // either index into DetailObjectDictLump_t
                                   // or DetailPropSpriteLump_t
...
enum {
    ...
    STATIC_PROP_NO_PER_VERTEX_LIGHTING =
        0x40,  // in vrad, compute lighting at
    ...
    STATIC_PROP_NO_PER_TEXEL_LIGHTING =
        0x100,  // whether we should do per-texel lightmaps in vrad.
...
struct StaticPropLumpV4_t {
    ...
    //	int				m_Lighting;			// index into the
    //GAMELUMP_STATIC_PROP_LIGHTING lump
};
...
struct StaticPropLump_t {
    ...
    StaticPropLump_t& operator=(const StaticPropLumpV4_t& _rhs) {
        ...
        // Value judgement: This makes the code easier to work with, so unless
        // it's a perf issue...
