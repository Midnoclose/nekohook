...
class IHTML {
   public:
    ...
    // returns the width and height (in pixels) of the HTML full page (not just
    ...
    virtual void Obsolete_OnMouse(MouseCode code, MOUSE_STATE s, int x,
                                  int y) = 0;
    virtual void Obsolete_OnChar(wchar_t unichar) = 0;
    virtual void Obsolete_OnKeyDown(ButtonCode_t code) = 0;
    ...
    virtual void OnMouse(MouseCode code, MOUSE_STATE s, int x, int y,
                         bool bPopupMenuMenu) = 0;
    virtual void OnChar(wchar_t unichar, bool bPopupMenu) = 0;
    virtual void OnKeyDown(ButtonCode_t code, bool bPopupMenu) = 0;
    ...
    // called when the browser needs to be resized
    virtual void OnKeyUp(ButtonCode_t code, bool bPopupMenu) = 0;

    // open a URL with the provided POST data (which can be much larger than the
    // max URL of 512 chars) NOTE - You CANNOT have get params (i.e a "?" ) in
    // pchURL if pchPostData is set (due to an IE bug)
    ...
    virtual void HorizontalScrollBarSize(int &x, int &y, int &wide,
                                         int &tall) = 0;
    virtual void VerticalScrollBarSize(int &x, int &y, int &wide,
                                       int &tall) = 0;
...
enum EWebPageLoadError {
    ...
    eCacheMiss,  // Usually caused by navigating to a page with POST data via
                 // back or forward buttons
    eBadURL,     // bad url passed in (invalid hostname, malformed)
    eConnectionProblem,  // network connectivity problem, server offline or user
                         // not on internet
    eProxyConnectionProblem,  // User is configured to use proxy, but we can't
                              // use it

    eLoadErrorUnknown,  // not a load type we classify right now, check out
                        // cef_handler_errorcode_t for the full list we could
                        // translate
};
...
class IHTMLEvents {
   public:
    // unused functions we keep around so the vtable layout is binary compatible
    virtual bool Obsolete_OnStartURL(const char *url, const char *target,
                                     bool first) = 0;
    ...
    // when the top level browser is changing the page they are looking at (not
    // sub iframes or the like loading)
    virtual void OnURLChanged(const char *url, const char *pchPostData,
                              bool bIsRedirect) = 0;
    // the control has finished loading a request, could be a sub request in the
    // page
    ...
    virtual bool OnStartRequestInternal(const char *url, const char *target,
    ...
    virtual bool OnPopupHTMLWindow(const char *pchURL, int x, int y, int wide,
                                   int tall) = 0;
    ...
    // the browser wants to ask the user to select a local file and tell it
    // about it
    virtual void OnFileLoadDialog(const char *pchTitle,
                                  const char *pchInitialFile) = 0;
