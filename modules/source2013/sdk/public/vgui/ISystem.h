...
class ISystem : public IBaseInterface {
   public:
    ...
    virtual KeyValues *GetUserConfigFileData(const char *dialogName,
                                             int dialogID) = 0;
    // sets the name of the config file to save/restore from.  Settings are
    // loaded immediately.
    ...
    virtual ButtonCode_t KeyCode_VirtualKeyToVGUI(int ButtonCode_t) = 0;
    ...
    virtual bool CreateShortcut(const char *linkFileName,
                                const char *targetPath, const char *arguments,
    ...
    //!! move this function next to other registry function on changing
    ...
    // Pixel at (x1,y1) is copied, pixels at column x2 and row y2 are *not*
    // copied
