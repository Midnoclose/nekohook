...
//			This functionality is all wrapped in the Panel::GetScheme*()
//functions
//-----------------------------------------------------------------------------
...
class ISchemeManager : public IBaseInterface {
   public:
    ...
    virtual HScheme LoadSchemeFromFile(const char *fileName,
                                       const char *tag) = 0;
    ...
    virtual HTexture GetImageID(const char *imageName,
                                bool hardwareFiltered) = 0;
    ...
    // layouts use these for font, image and panel size scaling (they all use
    // the pixel height of the display for scaling)
    ...
    // first scheme loaded becomes the default scheme, and all subsequent loaded
    // scheme are derivitives of that
    virtual HScheme LoadSchemeFromFileEx(VPANEL sizingPanel,
                                         const char *fileName,
                                         const char *tag) = 0;
    // gets the proportional coordinates for doing screen-size independant panel
    // layouts use these for font, image and panel size scaling (they all use
