...
class IInput : public IBaseInterface {
   public:
    ...
    virtual void GetKeyCodeText(ButtonCode_t code,
                                OUT_Z_BYTECAP(buflen) char *buf,
                                int buflen) = 0;

    // focus
    virtual VPANEL GetFocus() = 0;
    virtual VPANEL GetCalculatedFocus() = 0;  // to handle cases where the focus
                                              // changes inside a frame.
    virtual VPANEL
    GetMouseOver() = 0;  // returns the panel the mouse is currently over,
                         // ignoring mouse capture
    ...
    // etc.)
    virtual void GetIMELanguageName(OUT_Z_BYTECAP(unicodeBufferSizeInBytes)
                                        wchar_t *buf,
                                    int unicodeBufferSizeInBytes) = 0;
    // Returns the short code for the language (EN, CH, KO, JP, RU, TH, etc. ).
    virtual void GetIMELanguageShortCode(OUT_Z_BYTECAP(unicodeBufferSizeInBytes)
                                             wchar_t *buf,
                                         int unicodeBufferSizeInBytes) = 0;
    ...
    virtual void GetCandidate(int num,
                              OUT_Z_BYTECAP(destSizeBytes) wchar_t *dest,
                              int destSizeBytes) = 0;
    ...
    // Passes in a ButtonCode_t which allows hitting other mouse buttons w/o
    // cancelling capture mode
    ...
    //  function and if we ever get to a Panel::OnKeyCodeTypes we know that
    //  nobody handled the message and in that case we can post a message to any
    ...
    virtual void OnKeyCodeUnhandled(int ButtonCode_t) = 0;

    // Assumes subTree is a child panel of the root panel for the vgui contect
    //  if restrictMessagesToSubTree is true, then mouse and kb messages are
    //  only routed to the subTree and it's children and mouse/kb focus
    ...
    //   "UnhandleMouseClick" message is sent to unhandledMouseClickListener
    //   panel if it's set
    ...
    // These toggle whether the modal subtree is exclusively receiving messages
    // or conversely whether it's being excluded from receiving messages Sends a
