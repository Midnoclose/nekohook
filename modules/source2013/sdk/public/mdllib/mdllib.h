...
abstract_class IMdlLib : public IAppSystem {
   ...
   public:
    ...
    //change 		vvdBuffer			- vvd buffer, updated, size reduced
    //		vtxBuffer			- vtx buffer, updated, size
    //reduced
    //		ppStripInfo			- if nonzero on return will be filled with the
    //stripping info
    //
    virtual bool StripModelBuffers(
        CUtlBuffer & mdlBuffer, CUtlBuffer & vvdBuffer, CUtlBuffer & vtxBuffer,
...
abstract_class IMdlStripInfo {
   ...
   public:
    ...
    //	plChecksumOriginal		if non-NULL will hold the checksum of the original
    //model submitted for stripping 	plChecksumStripped		if non-NULL will
    //hold the resulting checksum of the stripped model
