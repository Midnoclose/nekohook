...
struct Bitmap_t {
    ...
    /// Set the bitmap to the specified buffer.  Any existing data is
    /// discarded/freed as appropriate.
    void SetBuffer(int nWidth, int nHeight, ImageFormat imageFormat,
                   unsigned char *pBits, bool bAssumeOwnership,
                   int nStride = 0);

    /// Sets / releases ownershp of the buffer.  This does not otherwise alter
    ...
    /// of the buffer.
    void MakeLogicalCopyOf(Bitmap_t &src,
                           bool bTransferBufferOwnership = false);
    ...
    /// the crop in place.
    void Crop(int x0, int y0, int nWidth, int nHeight,
              const Bitmap_t *pImgSource = NULL);

    /// Blit a rectangle of pixel data into this image.
    void SetPixelData(const Bitmap_t &src, int nSrcX1, int nSrcY1,
                      int nCopySizeX, int nCopySizeY, int nDestX1, int nDestY1);
