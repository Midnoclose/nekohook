...
struct DataCacheLimits_t {
    DataCacheLimits_t(unsigned _nMaxBytes = (unsigned)-1,
                      unsigned _nMaxItems = (unsigned)-1,
                      unsigned _nMinBytes = 0, unsigned _nMinItems = 0)
...
//			areas of the cache with thier own memory constraints and
//common 			management.
//-----------------------------------------------------------------------------
abstract_class IDataCacheSection {
   public:
    ...
    //--------------------------------------------------------
    // Purpose: Finds an item in the cache, returns NULL if item is not in
    ...
    //--------------------------------------------------------
    // Purpose: Returns if the data is currently in memory, but does *not*
    // change its location in the LRU
    //--------------------------------------------------------
    ...
    //--------------------------------------------------------
    // Purpose: Get an item without locking it, returns NULL if item is not in
    ...
    //--------------------------------------------------------
    // Purpose: "Frame locking" (not game frame). A crude way to manage locks
    // over relatively
    ...
    //--------------------------------------------------------
    virtual void UpdateSize(DataCacheHandle_t handle,
...
class CManagedDataCacheClient : public CDefaultDataCacheClient {
   public:
    ...
    DataCacheHandle_t CacheCreate(const CREATE_PARAMS &createParams,
        ...
        m_pCache->AddEx((DataCacheClientID_t)pStore, pStore, pStore->Size(),
                        flags, &handle);
        return handle;
    }
    ...
    bool HandleCacheNotification(const DataCacheNotification_t &notification) {
        switch (notification.type) {
            ...
            case DC_REMOVED: {
                ...
                return true;
            default:
                return CDefaultDataCacheClient::HandleCacheNotification(
                    notification);
        }
    }
