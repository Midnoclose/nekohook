...
class CPassengerSeat {
   ...
   private:
    ...
    CUtlVector<CPassengerSeatTransition>
        m_EntryTransitions;  // Entry information
...
class CBaseServerVehicle : public IServerVehicle {
   ...
   public:
    bool NPC_AddPassenger(CBaseCombatCharacter *pPassenger,
    ...
    virtual int NPC_GetPassengerSeatAttachment(
   ...
   public:
    ...
    struct exitanim_t {
        ...
        bool bEscapeExit;
        char szAnimName[128];
        Vector vecExitPointLocal;  // Point the animation leaves the player at
                                   // when finished
    ...
    bool m_bUseLegacyExitChecks;  // HACK: Choreo vehicles use non-sensical
                                  // setups to move the player, we need to poll
    ...
    CHandle<CEntityBlocker>
        m_hExitBlocker;  // Entity to prevent other entities blocking the
                         // player's exit point during the exit animation
   ...
   private:
    ...
    bool GetLocalAttachmentAtTime(int nQuerySequence, int nAttachmentIndex,
                                  ...
                                  QAngle *vecAnglesOut);
    bool GetLocalAttachmentAtTime(const char *lpszAnimName,
                                  int nAttachmentIndex, float flCyclePoint,
