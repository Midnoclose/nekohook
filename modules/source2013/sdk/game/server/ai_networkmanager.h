...
//			isn't there, saving & loading of the network, and holding
//the 			CAI_Network instance.
//
//-----------------------------------------------------------------------------
...
abstract_class CAI_NetworkBuildHelper : public CLogicalEntity {
   ...
   public:
    virtual void PostInitNodePosition(CAI_Network * pNetwork,
                                      CAI_Node * pNode) = 0;
};
...
class CAI_NetworkEditTools {
   public:
    ...
    static void DrawNextHull(
        const char
            *ainet_name);  // Draws next hull set for the named ai network
    static void SetDebugBits(const char *ainet_name, int debug_bit);

    static CAI_Node *FindAINodeNearestFacing(const Vector &origin,
                                             const Vector &facing,
                                             float threshold, int nNodeType);
    static CAI_Link *FindAILinkNearestFacing(const Vector &origin,
                                             const Vector &facing,
                                             float threshold);
