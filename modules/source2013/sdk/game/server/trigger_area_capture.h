...
//-----------------------------------------------------------------------------
// Purpose: An area entity that players must remain in in order to active
// another entity
//			Triggers are fired on start of capture, on end of capture
//and on broken capture 			Can either be capped by both teams at once, or just by
//one 			Time to capture and number of people required to capture are both passed
//by the mapper
//-----------------------------------------------------------------------------
...
class CTriggerAreaCapture : public CTriggerAreaCaptureShim,
                            public ITriggerAreaCaptureAutoList {
   ...
   public:
    ...
    bool IsActive(void);
    bool CheckIfDeathCausesBlock(CBaseMultiplayerPlayer *pVictim,
    ...
    void ForceOwner(
        int team);  // by the control_point_round to force an owner of this
                    // point (so we can play a specific round)
   ...
   private:
    ...
    void SetOwner(int team);  // sets the owner of this point - useful for
                              // resetting all to -1
   ...
   private:
    ...
    float
        m_flCapTime;  // the total time it takes to capture the area, in seconds
    ...
    struct perteamdata_t {
        ...
        int iNumTouching;
        int iBlockedTouching;  // Number of capping players on the cap while
                               // it's being blocked
        bool bCanCap;
        int iSpawnAdjust;
