...
class CBreakableProp : public CBaseProp,
   ...
   public:
    ...
    virtual bool HasPreferredCarryAnglesForPlayer(CBasePlayer *pPlayer) {
        if (HasInteraction(PROPINTER_PHYSGUN_LAUNCH_SPIN_Z)) return true;
    ...
    virtual void Ignite(float flFlameLifetime, bool bNPCOnly,
                        float flSize = 0.0f,
                        bool bCalledByLevelDesigner = false);

    // Specific interactions
    void HandleFirstCollisionInteractions(int index,
                                          gamevcollisionevent_t *pEvent);
    void HandleInteractionStick(int index, gamevcollisionevent_t *pEvent);
    void StickAtPosition(const Vector &stickPosition,
                         const Vector &savePosition, const QAngle &saveAngles);
   ...
   protected:
    ...
    virtual void OnBreak(const Vector &vecVelocity,
                         const AngularImpulse &angVel, CBaseEntity *pBreaker) {}
   ...
   private:
    ...
    COutputEvent m_OnPhysCannonAnimatePreStarted;  // Started playing the
                                                   // pre-pull animation
    COutputEvent
        m_OnPhysCannonAnimatePullStarted;  // Player started the pull anim
    COutputEvent m_OnPhysCannonAnimatePostStarted;  // Started playing the
                                                    // post-pull animation
    COutputEvent
        m_OnPhysCannonPullAnimFinished;  // We've had our pull anim finished, or
                                         // the post-pull has finished if there
                                         // is one
    float
        m_flDefaultFadeScale;  // Things may temporarily change the fade scale,
                               // but this is its steady-state condition
...
class CPhysicsProp : public CBreakableProp, public IPhysicsPropAutoList {
   ...
   public:
    ...
    // Specific interactions
    void HandleAnyCollisionInteractions(int index,
                                        gamevcollisionevent_t *pEvent);
...
// An interface so that objects parented to props can receive collision
// interaction events.
...
abstract_class IParentPropInteraction {
   public:
    virtual void OnParentCollisionInteraction(
        parentCollisionInteraction_t eType, int index,
        gamevcollisionevent_t *pEvent) = 0;
    virtual void OnParentPhysGunDrop(CBasePlayer * pPhysGunUser,
...
CPhysicsProp *CreatePhysicsProp(const char *pModelName,
...
float GetBreakableDamage(const CTakeDamageInfo &inputInfo,
                         IBreakableWithPropData *pProp = NULL);
