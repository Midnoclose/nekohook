...
enum AssaultCue_t {
    CUE_NO_ASSAULT =
        0,  // used to indicate that no assault is being conducted presently
...
// Allow diversion from the assault up to this amount of time after last having
// an enemy
...
class CAI_AssaultBehavior : public CAI_SimpleBehavior {
   ...
   public:
    ...
    CRallyPoint *FindBestRallyPointInRadius(const Vector &vecCenter,
                                            float flRadius);
    ;
    ...
    enum {
        SCHED_MOVE_TO_RALLY_POINT =
            BaseClass::NEXT_SCHEDULE,  // Try to get out of the player's way
   ...
   private:
    ...
    AssaultCue_t
        m_AssaultCue;  // the cue we're waiting for to begin the assault
    AssaultCue_t m_ReceivedAssaultCue;  // the last assault cue we received from
                                        // someone/thing external.
