...
#define ANTLION_FOLLOW_DISTANCE_SQR \
    (ANTLION_FOLLOW_DISTANCE * ANTLION_FOLLOW_DISTANCE)
...
class CNPC_Antlion : public CAI_BaseAntlionBase {
   public:
    ...
    void ZapThink(void);
    void BurrowUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
    ...
    virtual int GetSoundInterests(void) {
        return (BaseClass::GetSoundInterests()) |
               (SOUND_DANGER | SOUND_PHYSICS_DANGER | SOUND_THUMPER |
                SOUND_BUGBAIT);
    }
   ...
   private:
    ...
    bool FindBurrow(const Vector &origin, float distance, int type,
                    bool excludeNear = true);
    ...
    void MeleeAttack(float distance, float damage, QAngle &viewPunch,
                     Vector &shove);
    ...
    virtual bool GetSpitVector(const Vector &vecStartPos,
    ...
    Vector
        m_vecSaveSpitVelocity;  // Saved when we start to attack and used if we
                                // failed to get a clear shot once we release
    ...
    COutputEvent
        m_OnReachFightGoal;  // Reached our scripted destination to fight to
    ...
    float m_flNextAcknowledgeTime;  // Next time an antlion can make an
    ...
    float m_flEludeDistance;  // Distance until the antlion will consider
                              // himself "eluded" if so flagged
    bool m_bLeapAttack;
    bool m_bDisableJump;
    float m_flTimeDrown;
    float m_flTimeDrownSplash;
    bool m_bDontExplode;  // Suppresses worker poison burst when drowning,
                          // failing to unburrow, etc.
    bool m_bLoopingStarted;
    bool m_bSuppressUnburrowEffects;  // Don't kick up dust when spawning
#if HL2_EPISODIC
    bool m_bHasDoneAirAttack;  ///< only allowed to apply this damage once per
                               ///< glide
#endif
