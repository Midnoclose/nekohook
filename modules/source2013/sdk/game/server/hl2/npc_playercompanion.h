...
//
// Purpose: Base class for humanoid NPCs intended to fight along side player in
// close environments
//
//=============================================================================//
...
class CCompanionActivityRemap : public CActivityRemap {
   public:
    ...
    // This bitfield maps which bits of data are being utilized by this data
    // structure, since not all criteria in the parsed file are essential.  You
    // must add corresponding bits to the definitions below and maintain their
    // state in the parsing of the file, as well as check the bitfield before
    // accessing the data.  This could be encapsulated into this class, but
    // we'll probably move away from this model and closer to something more
    // akin to the response rules -- jdw
    ...
    bool m_bInVehicle;  // For future expansion, this needs to speak more to the
                        // exact seat, role, and vehicle
};

// Usage bits for remap "extra" parsing - if these bits are set, the associated
// data has changed
...
class CNPC_PlayerCompanion : public CAI_PlayerAlly {
   ...
   public:
    ...
    void LockReadiness(
        float duration = -1.0f);  // Defaults to indefinitely locked
   ...
   public:
    ...
    bool FindCoverPosInRadius(CBaseEntity *pEntity, const Vector &goalPos,
    ...
    bool OverrideMove(float flInterval);  // Override to take total control of
                                          // movement (return true if done so)
...
#if HL2_EPISODIC
    void InputClearAllOuputs(
        inputdata_t &inputdata);  ///< annihilate every output on this npc
#endif
   ...
   private:
    ...
    enum eCoverType { CT_NORMAL, CT_TURRET, CT_MORTAR };
   ...
   protected:
    ...
    // Readiness is a value that's fed by various events in the NPC's AI. It is
    // used to make decisions about what type of posture the NPC should be in
    // (relaxed, agitated). It is not used to make decisions about what to do in
    // the AI.
    float m_flReadiness;
    ...
    int m_iNumConsecutiveBarrelsExploded;  // Companions keep track of the # of
                                           // consecutive barrels exploded by
                                           // the player and speaks a response
                                           // as it increases
    int m_iNumConsecutivePlayerKills;      // Alyx keeps track of the # of
                                       // consecutive kills by the player and
                                       // speaks a response as it increases
