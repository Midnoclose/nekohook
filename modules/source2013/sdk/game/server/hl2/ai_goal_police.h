...
class CAI_PoliceGoal : public CBaseEntity {
   public:
    ...
    bool ShouldKnockOutTarget(
        const Vector &targetPos,
        bool bTargetVisible);  // If the target should be knocked out
    void KnockOutTarget(CBaseEntity *pTarget);  // Send an output that we've
                                                // knocked out this target
...
#define SF_POLICE_GOAL_KNOCKOUT_BEHIND \
    (1 << 1)  // Knockout a target that's behind the plane that cuts
              // perpendicularly through us
#define SF_POLICE_GOAL_DO_NOT_LEAVE_POST \
    (1 << 2)  // Cop will not come off his policing goal, even when angered
