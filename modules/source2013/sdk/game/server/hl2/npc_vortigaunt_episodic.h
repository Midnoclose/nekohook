...
class CNPC_Vortigaunt : public CNPC_PlayerCompanion {
   ...
   public:
    ...
    virtual int RangeAttack1Conditions(float flDot,
                                       float flDist);  // Primary zap
    virtual int RangeAttack2Conditions(
        float flDot, float flDist);  // Concussive zap (larger)
    ...
    bool IsLeading(void) {
        return (GetRunningBehavior() == &m_LeadBehavior &&
                m_LeadBehavior.HasGoal());
    }
   ...
   private:
    ...
    void DispelAntlions(const Vector &vecOrigin, float flRadius,
                        bool bDispel = true);
   ...
   private:
    ...
    enum {
        ...
        COND_VORTIGAUNT_HEAL_TARGET_BEHIND_US,  // Not within our "forward"
                                                // range
    ...
    bool m_bPlayerRequestedHeal;  // This adds some priority to our heal (allows
                                  // it to happen in combat, etc)
...
class CVortigauntChargeToken : public CBaseEntity {
   ...
   public:
    static CVortigauntChargeToken *CreateChargeToken(const Vector &vecOrigin,
                                                     CBaseEntity *pOwner,
                                                     CBaseEntity *pTarget);
...
class CVortigauntEffectDispel : public CBaseEntity {
   ...
   public:
    static CVortigauntEffectDispel *CreateEffectDispel(const Vector &vecOrigin,
                                                       CBaseEntity *pOwner,
                                                       CBaseEntity *pTarget);
