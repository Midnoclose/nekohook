...
class CAntlionTemplateMaker : public CTemplateNPCMaker {
   public:
    ...
    void SetFightTarget(string_t strTarget, CBaseEntity *pActivator = NULL,
                        CBaseEntity *pCaller = NULL);
    void SetFightTarget(CBaseEntity *pEntity);
    void SetFightTarget(const Vector &position);

    void SetFollowTarget(string_t strTarget, CBaseEntity *pActivator = NULL,
                         CBaseEntity *pCaller = NULL);
    ...
    CBaseEntity *AllHintsFromClusterBlocked(CAI_Hint *pNode,
                                            bool &bChosenHintBlocked);
   ...
   protected:
    ...
    bool FindHintSpawnPosition(const Vector &origin, float radius,
                               string_t hintGroupName, CAI_Hint **pHint,
                               bool bRandom = false);
    bool FindNearTargetSpawnPosition(Vector &origin, float radius,
                                     CBaseEntity *pTarget);

    // These are used by FindNearTargetSpawnPosition
    bool FindPositionOnFoot(Vector &origin, float radius, CBaseEntity *pTarget);
    bool FindPositionOnVehicle(Vector &origin, float radius,
                               CBaseEntity *pTarget);
   ...
   protected:
    // FIXME: The m_strSpawnGroup is redundant to the m_iszDestinationGroup in
    // the base class NPC template maker
    string_t m_strSpawnGroup;  // if present, spawn children on the nearest node
                               // of this group (to the player)
