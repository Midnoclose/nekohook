...
enum busyinterrupt_t {
    ...
    BA_INT_DANGER,  // Only danger signals interrupts this busy anim. The player
                    // will be ignored.
    BA_INT_PLAYER,  // The Player's presence interrupts this busy anim
    BA_INT_AMBUSH,  // We're waiting to ambush enemies. Don't break on danger
...
struct busyanim_t {
    ...
    float flMaxTime;  // Max time spent in this busy animation. 0 means continue
                      // until interrupted.
...
class CAI_ActBusyBehavior : public CAI_SimpleBehavior {
   ...
   public:
    ...
    void Disable(void);
    void ForceActBusy(CAI_ActBusyGoal *pGoal, CAI_Hint *pHintNode = NULL,
                      float flMaxTime = NO_MAX_TIME, bool bVisibleOnly = false,
                      bool bTeleportToBusy = false,
    ...
    bool IsActive(void);
    // Returns true if the current NPC is actually acting busy (i.e. inside an
    // act busy anim)
   ...
   private:
    ...
    int m_iNumConsecutivePathFailures;  // Count how many times we failed to
                                        // find a path to a node, so we can
...
class CAI_ActBusyGoal : public CAI_GoalEntity {
   ...
   public:
    CAI_ActBusyGoal() {
        // Support legacy maps, where this value used to be set from a constant
        // (with a value of 1). Now designers can specify whatever they want in
        // Hammer. Take care of old maps by setting this in the constructor.
        // (sjb)
   ...
   protected:
    CAI_ActBusyBehavior *GetBusyBehaviorForNPC(const char *pszActorName,
                                               CBaseEntity *pActivator,
                                               CBaseEntity *pCaller,
                                               const char *sInputName);
    CAI_ActBusyBehavior *GetBusyBehaviorForNPC(CBaseEntity *pEntity,
                                               const char *sInputName);
   ...
   public:
    // Let the actbusy behavior query these so we don't have to duplicate the
    // data.
