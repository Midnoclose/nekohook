...
enum eyeState_t {
    ...
    TURRET_EYE_DISABLED,  // Turned off, must be reactivated before it'll deploy
...
//		this class may have undesired side effects due to these being
//set.
...
//-----------------------------------------------------------------------------
class CNPC_FloorTurret : public CNPCBaseInteractive<CAI_BaseNPC>,
   ...
   public:
    ...
    // Use functions
    void ToggleUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
    ...
    Vector EyePosition(void) {
        ...
        // Note: We back up into the model to avoid an edge case where the eyes
        // clip out of the world and
   ...
   protected:
    ...
    CHandle<CBaseCombatCharacter>
        m_hLastNPCToKickMe;  // Stores the last NPC who tried to knock me over
    float m_flKnockOverFailedTime;  // Time at which we should tell the NPC that
                                    // he failed to knock me over
...
class CTurretTipController : public CPointEntity, public IMotionEvent {
   ...
   public:
    ...
    static CTurretTipController *CreateTipController(CNPC_FloorTurret *pOwner) {
        ...
        CTurretTipController *pController = (CTurretTipController *)Create(
            "floorturret_tipcontroller", pOwner->GetAbsOrigin(),
            pOwner->GetAbsAngles());
