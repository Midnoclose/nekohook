...
class CSDKPlayer : public CBasePlayer, public ISDKPlayerAnimStateHelpers {
   public:
    ...
    virtual void LeaveVehicle(const Vector &vecExitPoint,
    ...
    CNetworkVar(
        int,
        m_iThrowGrenadeCounter);  // used to trigger grenade throw animations.
    CNetworkQAngle(m_angEyeAngles);   // Copied from EyeAngles() so we can send
                                      // it to the client.
   ...
   public:
    ...
    void FireBullet(Vector vecSrc, const QAngle &shootAngles, float vecSpread,
                    int iDamage, int iBulletType, CBaseEntity *pevAttacker,
                    bool bDoEffects, float x, float y);
