...
//-----------------------------------------------------------------------------
// NOTE: these must correspond with the AI_FollowFormation_t array in
...
int AIGetNumFollowers(CBaseEntity *pEntity,
                      string_t iszClassname = NULL_STRING);
...
struct AI_FollowParams_t {
    AI_FollowParams_t(AI_Formations_t formation = AIF_SIMPLE,
                      bool bNormalMemoryDiscard = false)
...
class CAI_FollowBehavior : public CAI_SimpleBehavior {
   ...
   public:
    ...
    virtual bool FarFromFollowTarget() {
        return (m_hFollowTarget &&
                (GetAbsOrigin() - m_hFollowTarget->GetAbsOrigin()).LengthSqr() >
                    (75 * 12) * (75 * 12));
    }
   ...
   protected:
    enum {
        SCHED_FOLLOWER_MOVE_AWAY_FAIL =
            BaseClass::NEXT_SCHEDULE,  // Turn back toward player
   ...
   protected:
    ...
    bool m_bFollowNavFailed;  // Set when pathfinding fails to limit impact of
                              // m_FollowDelay on ShouldFollow
