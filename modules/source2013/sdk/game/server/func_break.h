...
#define SF_BREAK_TOUCH \
    0x0002  // can be 'crashed through' by running player (plate glass)
#define SF_BREAK_PRESSURE 0x0004  // can be broken by a player standing on it
#define SF_BREAK_PHYSICS_BREAK_IMMEDIATELY \
    0x0200  // the first physics collision this breakable has will immediately
            // break it
#define SF_BREAK_DONT_TAKE_PHYSICS_DAMAGE \
    0x0400  // this breakable doesn't take damage from physics collisions
#define SF_BREAK_NO_BULLET_PENETRATION \
    0x0800  // don't allow bullets to penetrate

// Spawnflags for func_pushable (it's also func_breakable, so don't collide with
// those flags)
...
class CBreakable : public CBaseEntity,
   ...
   public:
    ...
    static void MaterialSoundRandom(int entindex, Materials soundMaterial,
                                    float volume);
   ...
   protected:
    ...
    EHANDLE m_hBreaker;  // The entity that broke us. Held as a data member
