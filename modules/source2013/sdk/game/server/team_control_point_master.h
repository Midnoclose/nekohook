...
//-----------------------------------------------------------------------------
// Purpose: One ControlPointMaster is spawned per level. Shortly after spawning
// it detects all the Control points in the map and puts them into the
// m_ControlPoints. From there it detects the state where all points are
// captured and resets them if necessary It gives points every time interval to
...
class CTeamControlPointMaster : public CBaseEntity {
    ...
    // Derived, game-specific control point masters must override these
    // functions
   public:
   ...
   public:
    ...
    CTeamControlPoint *GetControlPoint(int point) {
        ...
        for (unsigned int i = 0; i < m_ControlPoints.Count(); i++) {
            ...
            if (pPoint && pPoint->GetPointIndex() == point) return pPoint;
        }
    ...
    int NumPlayableControlPointRounds(
        void);  // checks to see if there are any more rounds to play (but
                // doesn't actually "get" one to play)
   ...
   private:
    ...
    int TeamOwnsAllPoints(CTeamControlPoint *pOverridePoint = NULL,
                          int iOverrideNewTeam = TEAM_UNASSIGNED);

    bool FindControlPoints(
        void);  // look in the map to find active control points
    bool FindControlPointRounds(
        void);  // look in the map to find active control point rounds
    bool GetControlPointRoundToPlay(
        void);                       // gets the next round we should play
    ...
    bool m_bFoundPoints;  // true when the control points have been found and
