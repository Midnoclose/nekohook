...
#define STOPPED_VELOCITY_THRESHOLD_SQR \
    (STOPPED_VELOCITY_THRESHOLD * STOPPED_VELOCITY_THRESHOLD)

#define STARTED_VELOCITY_THRESHOLD 64.0f
#define STARTED_VELOCITY_THRESHOLD_SQR \
    (STARTED_VELOCITY_THRESHOLD * STARTED_VELOCITY_THRESHOLD)
...
class CAI_PassengerBehavior : public CAI_SimpleBehavior {
   ...
   public:
    ...
    bool ForceVehicleInteraction(const char *lpszInteractionName,
                                 CBaseCombatCharacter *pOther);
   ...
   protected:
    ...
    bool SpeakIfAllowed(AIConcept_t concept, const char *modifiers = NULL,
    ...
    }  // NPC attempted to leave vehicle, but was unable to
    ...
    void DrawDebugTransitionInfo(const Vector &vecIdealPos,
                                 const QAngle &vecIdealAngles,
                                 const Vector &vecAnimPos,
                                 const QAngle &vecAnimAngles);
    bool GetEntryPoint(int nSequence, Vector *vecEntryPoint,
                       QAngle *vecEntryAngles = NULL);
    bool GetExitPoint(int nSequence, Vector *vecExitPoint,
                      QAngle *vecExitAngles = NULL);
    ...
    bool FindGroundAtPosition(const Vector &in, float flUpDelta,
                              float flDownDelta, Vector *out);
    bool DoTransitionMovement(void);
    bool GetSequenceBlendAmount(float flCycle, float *posBlend,
                                float *angBlend);
    bool LocalIntervalMovement(float flInterval, bool &bMoveSeqFinished,
                               Vector &newPosition, QAngle &newAngles);
    void GetTransitionAnimationIdeal(float flCycle, const Vector &vecTargetPos,
                                     const QAngle &vecTargetAngles,
    ...
    bool IsValidTransitionPoint(const Vector &vecStartPos,
    ...
    CHandle<CEntityBlocker>
        m_hBlocker;              // Blocking entity for space reservation
    ...
    passesngerVehicleIntent_e
        m_PassengerIntent;  // Gives us information about whether we're meant to
                            // get in/out, etc.
...
class CTraceFilterVehicleTransition : public CTraceFilterSkipTwoEntities {
   public:
    ...
    CTraceFilterVehicleTransition(const IHandleEntity *passentity,
