...
class CNodeList : public CUtlPriorityQueue<AI_NearNode_t> {
   public:
    static bool IsLowerPriority(const AI_NearNode_t &node1,
                                const AI_NearNode_t &node2) {
        // nodes with greater distance are lower priority
        return node1.dist > node2.dist;
    }
    static bool RevIsLowerPriority(const AI_NearNode_t &node1,
                                   const AI_NearNode_t &node2) {
    ...
    CNodeList(int growSize = 0, int initSize = 0)
        : CUtlPriorityQueue<AI_NearNode_t>(growSize, initSize,
                                           IsLowerPriority) {}
...
class CAI_Network : public IPartitionEnumerator {
   public:
    ...
    CAI_Node *AddNode(const Vector &origin,
                      float yaw);  // Returns a new node in the network
    CAI_Link *CreateLink(int srcID, int destID,
                         CAI_DynamicLink *pDynamicLink = NULL);

    bool IsConnected(int srcID, int destID);  // Use during run time
    void TestIsConnected(int startID,
                         int endID);  // Use only for initialization!
    ...
    static int FindBSSmallest(CVarBitVec *bitString, float *float_array,
                              int array_size);

    int NearestNodeToPoint(CAI_BaseNPC *pNPC, const Vector &vecOrigin,
                           bool bCheckVisiblity, INearestNodeFilter *pFilter);
    int NearestNodeToPoint(CAI_BaseNPC *pNPC, const Vector &vecOrigin,
                           bool bCheckVisiblity = true) {
   ...
   private:
    ...
    int GetCachedNearestNode(const Vector &checkPos, CAI_BaseNPC *pNPC,
                             int *pCachePos);
    ...
    int ListNodesInBox(CNodeList &list, int maxListCount, const Vector &mins,
                       const Vector &maxs, INodeListFilter *pFilter);
    ...
    struct NearNodeCache_T {
        ...
        int hull;          // Hull	type tested (or HULL_NONE is only visibility
                           // tested)
    };
    ...
    enum { PARTITION_NODE = (1 << 0) };

    NearNodeCache_T
        m_NearestCache[NEARNODE_CACHE_SIZE];  // Cache of nearest nodes
