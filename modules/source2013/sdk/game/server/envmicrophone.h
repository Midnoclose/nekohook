...
enum MicrophoneResult_t {
    ...
    MicrophoneResult_Swallow,  // The microphone swallowed the sound. Don't play
                               // it.
    MicrophoneResult_Remove,   // The microphone should be removed from the list
                               // of microphones.
};
...
class CEnvMicrophone : public CPointEntity {
   ...
   public:
    ...
    bool CanHearSound(int entindex, soundlevel_t soundlevel, float &flVolume,
    ...
    // Hook for the sound system to tell us when a sound's been played. Returns
    // true if it's to swallow the passed in sound.
    static bool OnSoundPlayed(int entindex, const char *soundname,
                              soundlevel_t soundlevel, float flVolume,
                              int iFlags, int iPitch, const Vector *pOrigin,
                              float soundtime,
                              CUtlVector<Vector> &soundorigins);

   private:
    // Per-microphone notification that a sound has played.
    MicrophoneResult_t SoundPlayed(int entindex, const char *soundname,
                                   soundlevel_t soundlevel, float flVolume,
    ...
    float m_flSmoothFactor;  // 0 = no smoothing of samples, 0.9 = maximum
                             // smoothing
    float
        m_flMaxRange;  // Maximum sound hearing range, irrelevant of attenuation
    string_t m_iszSpeakerName;  // Name of a speaker to output any heard sounds
                                // through
