...
typedef void (*TextStatPrintFn)(IFileSystem *pFileSys, FileHandle_t hFile,
                                void *pUserData);
typedef void (*TextStatFileFn)();

// The CTextStatsMgr is just a collection of CTextStat's that go into the same
// file.
class CTextStatsMgr {
   public:
    CTextStatsMgr(void);
    ...
    // Get the preset filename to write stats to, if none is specified when
    // writing
...
class CTextStat {
   ...
   public:
    CTextStat();
    CTextStat(TextStatPrintFn fn, void *pUserData,
              CTextStatsMgr *pMgr = &g_TextStatsMgr);
    ~CTextStat();

    // This can be called if you don't want to pass parameters into the
    // constructor.
...
// This class registers like a ConVar and acts like an int. When the game is
// shutdown, its value will be saved in the stats file along with its name.s
class CTextStatInt {
   public:
    CTextStatInt(const char *pName, int initialValue = 0,
                 CTextStatsMgr *pMgr = &g_TextStatsMgr);
