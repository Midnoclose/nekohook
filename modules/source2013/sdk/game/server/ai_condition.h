...
// NOTE: Changing this constant will break save files!!! (changes type of
// CAI_ScheduleBits)
...
enum SCOND_t {
    ...
    COND_IN_PVS,
    COND_IDLE_INTERRUPT,  // The schedule in question is a low priority idle,
                          // and therefore a candidate for translation into
                          // something else
    ...
    COND_SEE_ENEMY,
    COND_LOST_ENEMY,
    COND_ENEMY_WENT_NULL,  // What most people think COND_LOST_ENEMY is: This
                           // condition is set in the edge case where you had an
                           // enemy last think, but don't have one this think.
    ...
    COND_SCHEDULE_DONE,
    COND_SMELL,
    COND_TOO_CLOSE_TO_ATTACK,  // FIXME: most of this next group are meaningless
                               // since they're shared between all attack
                               // checks!
    ...
    COND_NO_HEAR_DANGER,  // Since we can't use ~CONDITION. Mutually exclusive
                          // with COND_HEAR_DANGER

    COND_FLOATING_OFF_GROUND,

    COND_MOBBED_BY_ENEMIES,  // Surrounded by a large number of enemies melee
                             // attacking me. (Zombies or Antlions, usually).
    ...
    COND_PLAYER_PUSHING,
    COND_NPC_FREEZE,    // We received an npc_freeze command while we were
                        // unfrozen
    COND_NPC_UNFREEZE,  // We received an npc_freeze command while we were
                        // frozen

    // This is a talker condition, but done here because we need to handle it in
    // base AI due to it's interaction with behaviors.
    COND_TALKER_RESPOND_TO_QUESTION,

    COND_NO_CUSTOM_INTERRUPTS,  // Don't call BuildScheduleTestBits for this
                                // schedule. Used for schedules that must
