...
// Codes are either one of the enumerated types below, or a string (similar to
...
enum sharedtasks_e {
    ...
    // Move to m_hTargetEnt's location using the activity specified by
    // m_hCine->m_iszCustomMove.
    ...
    // Path to the enemy's location or path to a LOS with the enemy's last known
    // position, depending on range
    ...
    // Path to node with line of sight to enemy, at least flTaskData units away
    // from m_vSavePosition
    TASK_GET_FLANK_RADIUS_PATH_TO_ENEMY_LOS,

    // Path to node with line of sight to enemy, at least flTaskData degrees
    // away from m_vSavePosition from the enemy's POV
    ...
    // Allow a little slop, and allow for some Z offset (like the target is a
    // gun on a table).
    ...
    // Path to last position (Last position must be stored with
    // TASK_STORE_LAST_POSITION)
    ...
    // Path to location that has line of sight to saved position (Save position
    // must by set in code or by a task)
    ...
    // Find an interesting direction to face. Don't face into walls, corners if
    // you can help it.
    ...
    // Turn to face directly away from stored save position (save position must
    // be stored first!)
    ...
    // Emit a sound because you are pissed off because you just saw someone you
    // don't like
    TASK_SOUND_WAKE,
    ...
    TASK_WEAPON_FIND,
    TASK_WEAPON_PICKUP,
    TASK_WEAPON_RUN_PATH,  // run to weapon but break if someone else picks it
                           // up
    TASK_WEAPON_CREATE,
    ...
    // regather conditions at the start of a schedule (all conditions are
    // cleared between schedules)
    TASK_GATHER_CONDITIONS,

    // Require an enemy be seen after the task is run to be considered a
    // candidate enemy
