...
class CAI_PassengerBehaviorCompanion : public CAI_PassengerBehavior {
   ...
   public:
    ...
    virtual void Enable(CPropJeepEpisodic *pVehicle,
   ...
   private:
    ...
    float GetArcToEntryPoint(const Vector &vecCenterPoint,
                             const Vector &vecEntryPoint, bool &bClockwise);
    int SelectScheduleInsideVehicle(void);
    int SelectScheduleOutsideVehicle(void);
    bool FindPathToVehicleEntryPoint(void);
    bool CanEnterVehicleImmediately(int *pResultSequence, Vector *pResultPos,
    ...
    float m_flNextOverturnWarning;  // The next time the NPC may complained
                                    // about being upside-down
    float m_flOverturnedDuration;   // Amount of time we've been stuck in the
                                    // vehicle (unable to exit)
    float m_flUnseenDuration;       // Amount of time we've been hidden from the
                                    // player's view

    float m_flEnterBeginTime;  // Time the NPC started to try and enter the
                               // vehicle
    int m_nExitAttempts;  // Number of times we've attempted to exit the vehicle
                          // but failed
    int m_nVisibleEnemies;  // Keeps a record of how many enemies I know about
    float m_flLastLateralLean;  // Our last lean value

    CAI_MoveMonitor m_VehicleMonitor;  // Used to keep track of the vehicle's
                                       // movement relative to a mark
    CUtlVector<FailPosition_t>
        m_FailedEntryPositions;  // Used to keep track of the vehicle's movement
                                 // relative to a mark
