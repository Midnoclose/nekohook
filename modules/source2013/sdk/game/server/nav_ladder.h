...
//--------------------------------------------------------------------------------------------------------------
/**
 * The NavLadder represents ladders in the Navigation Mesh, and their
 * connections to adjacent NavAreas
...
class CNavLadder {
   public:
    ...
    static void CompressIDs(
        void);  ///< re-orders ladder ID's so they are continuous
    ...
    Vector GetPosAtHeight(float height)
        const;  ///< Compute x,y coordinate of the ladder at a given height

    CNavArea *m_topForwardArea;  ///< the area at the top of the ladder
    CNavArea *m_topLeftArea;
    CNavArea *m_topRightArea;
    CNavArea *m_topBehindArea;  ///< area at top of ladder "behind" it - only
                                ///< useful for descending
    CNavArea *m_bottomArea;     ///< the area at the bottom of the ladder

    bool IsConnected(const CNavArea *area, LadderDirectionType dir)
        const;  ///< returns true if given area is connected in given direction

    void ConnectGeneratedLadder(
        float maxHeightAboveTopArea);  ///< Connect a generated ladder to nav
                                       ///< areas at the end of nav generation

    void ConnectTo(CNavArea *area);  ///< connect this ladder to given area
    void Disconnect(
        CNavArea *area);  ///< disconnect this ladder from given area

    void OnSplit(CNavArea *original, CNavArea *alpha,
                 CNavArea *beta);  ///< when original is split into alpha and
                                   ///< beta, update our connections
    ...
    void UpdateDangling(
        void);  ///< Checks if the ladder is dangling (bots cannot go up)

    bool IsInUse(const CBasePlayer *ignore =
                     NULL) const;  ///< return true if someone is on this ladder
                                   ///< (other than 'ignore')
   ...
   private:
    ...
    NavDirType m_dir;  ///< which way the ladder faces (ie: surface normal of
                       ///< climbable side)
    Vector m_normal;   ///< surface normal of the ladder surface (or Vector-ized
                       ///< m_dir, if the traceline fails)

    enum LadderConnectionType  ///< Ladder connection directions, to facilitate
...
inline bool CNavLadder::IsUsableByTeam(int teamNumber) const {
    if (m_ladderEntity.Get() == NULL) return true;
...
//--------------------------------------------------------------------------------------------------------------
inline NavDirType CNavLadder::GetDir(void) const { return m_dir; }

//--------------------------------------------------------------------------------------------------------------
inline const Vector &CNavLadder::GetNormal(void) const { return m_normal; }
