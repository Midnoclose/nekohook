...
enum {
    ...
    SF_TRIGGER_ONLY_PLAYER_ALLY_NPCS =
        0x10,  // *if* NPCs can fire this trigger, this flag means only player
               // allies do so
    SF_TRIGGER_ONLY_CLIENTS_IN_VEHICLES =
        0x20,  // *if* Players can fire this trigger, this flag means only
               // players inside vehicles can
    SF_TRIGGER_ALLOW_ALL =
        0x40,  // Everything can fire this trigger EXCEPT DEBRIS!
    SF_TRIGGER_ONLY_CLIENTS_OUT_OF_VEHICLES =
        0x200,  // *if* Players can fire this trigger, this flag means only
                // players outside vehicles can
    SF_TRIG_PUSH_ONCE = 0x80,  // trigger_push removes itself after firing once
    SF_TRIG_PUSH_AFFECT_PLAYER_ON_LADDER =
        0x100,  // if pushed object is player on a ladder, then this disengages
                // them from the ladder (HL2only)
    SF_TRIG_TOUCH_DEBRIS = 0x400,  // Will touch physics debris objects
    SF_TRIGGER_ONLY_NPCS_IN_VEHICLES =
        0X800,  // *if* NPCs can fire this trigger, only NPCs in vehicles do so
                // (respects player ally flag too)
    SF_TRIGGER_DISALLOW_BOTS =
        0x1000,  // Bots are not allowed to fire this trigger
};
...
//-----------------------------------------------------------------------------
// Purpose: Variable sized repeatable trigger.  Must be targeted at one or more
// entities.
//			If "delay" is set, the trigger waits some time after
//activating before firing. 			"wait" : Seconds between triggerings. (.2
//default/minimum)
//-----------------------------------------------------------------------------
...
// NOTE: This uses vphysics to compute touch events.  It doesn't do a per-frame
// Touch call, so the Entity I/O is different from a regular trigger
//------------------------------------------------------------------------------
...
// This class is to get around the fact that DEFINE_FUNCTION doesn't like
...
class CTriggerHurt : public CTriggerHurtShim, public ITriggerHurtAutoList {
   public:
    CTriggerHurt() {
        // This field came along after levels were built so the field defaults
        // to 20 here in the constructor.
        m_flDamageCap = 20.0f;
    }
    ...
    float m_flDmgResetTime;    // For forgiveness, the time to reset the counter
                               // that accumulates damage.
    int m_bitsDamageInflict;   // DMG_ damage type that the door or tigger does
    int m_damageModel;
    bool m_bNoDmgForce;  // Should damage from this trigger impart force on what
                         // it's hurting
