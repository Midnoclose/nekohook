...
class CAI_BaseActor : public CAI_ExpresserHost<CAI_BaseHumanoid> {
   ...
   public:
    // FIXME: this method is lame, isn't there some sort of template thing that
    // would get rid of the Outer pointer?
    ...
    // Note, you must add all names to this static function in order for Init to
    // work
    ...
    void Init(FlexWeight_t &index, const char *szName) {
        ...
        if (!IsServerSideFlexController(szName)) {
            Error(
                "You forgot to add flex controller %s to list in "
