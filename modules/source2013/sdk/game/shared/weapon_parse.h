...
class FileWeaponInfo_t {
   ...
   public:
    ...
    char szWorldModel[MAX_WEAPON_STRING];  // Model of this weapon seen carried
                                           // by the player
    char szAnimationPrefix[MAX_WEAPON_PREFIX];  // Prefix of the animations that
                                                // should be used by the player
    ...
    int iWeight;  // this value used to determine this weapon's importance in
                  // autoselection.
    int iRumbleEffect;     // Which rumble effect to use when fired? (xbox)
    bool bAutoSwitchTo;    // whether this weapon should be considered for
                           // autoswitching to
    bool bAutoSwitchFrom;  // whether this weapon can be autoswitched away from
                           // when picking up another weapon or ammo
    ...
    bool m_bAllowFlipping;  // False to disallow flipping the model, regardless
                            // of whether it is built left or right handed.
    ...
    // TF2 specific
    bool bShowUsageHint;  // if true, then when you receive the weapon, show a
                          // hint about it
...
KeyValues *ReadEncryptedKVFile(IFileSystem *filesystem,
                               ...
                               const unsigned char *pICEKey,
                               bool bForceReadEncryptedFile = false);
