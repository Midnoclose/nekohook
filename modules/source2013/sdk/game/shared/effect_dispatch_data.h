...
// NOTE: These flags are specifically *not* networked; so it's placed above the
// max effect flag bits
...
// This is the class that holds whatever data we're sending down to the client
// to make the effect.
class CEffectData {
   public:
    Vector m_vOrigin;
   ...
   private:
    ...
    int m_iEffectName;  // Entry in the EffectDispatch network string table. The
                        // is automatically handled by DispatchEffect().
};
