...
#define VEC_VIEW_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vView * player->GetModelScale())
#define VEC_HULL_MIN_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vHullMin * player->GetModelScale())
#define VEC_HULL_MAX_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vHullMax * player->GetModelScale())

#define VEC_DUCK_HULL_MIN_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vDuckHullMin * player->GetModelScale())
#define VEC_DUCK_HULL_MAX_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vDuckHullMax * player->GetModelScale())
#define VEC_DUCK_VIEW_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vDuckView * player->GetModelScale())

#define VEC_OBS_HULL_MIN_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vObsHullMin * player->GetModelScale())
#define VEC_OBS_HULL_MAX_SCALED(player) \
    (g_pGameRules->GetViewVectors()->m_vObsHullMax * player->GetModelScale())

#define VEC_DEAD_VIEWHEIGHT_SCALED(player)               \
    (g_pGameRules->GetViewVectors()->m_vDeadViewHeight * \
...
#define HIDEHUD_MISCSTATUS \
    (1 << 6)  // Hide miscellaneous status elements (trains, pickup history,
              // death notices, etc)
#define HIDEHUD_CHAT \
...
// You might be wondering why these aren't multiple of 2. Well the reason is
// that if servers decide to have HLTV or Replay enabled we need the extra slot.
// This is ok since MAX_PLAYERS is used for code specific things like arrays and
// loops, but it doesn't really means that this is the max number of players
// allowed Since this is decided by the gamerules (and it can be whatever number
// as long as its less than MAX_PLAYERS).
...
#define PLAYER_FALL_PUNCH_THRESHOLD \
    303.0f  // won't punch player's screen/make scrape noise unless player
            // falling at least this fast - at least a 76" fall (sqrt( 2 * g *
            // 76))
#else
...
#define DMG_GENERIC \
    0  // generic damage -- do not use if you want players to flinch and bleed!
#define DMG_CRUSH \
    (1 << 0)  // crushed by falling or moving object.
              // NOTE: It's assumed crush damage is occurring as a result of
              // physics collision, so no extra physics force is generated by
              // crush damage. DON'T use DMG_CRUSH when damaging entities unless
              // it's the result of a physics collision. You probably want
              // DMG_CLUB instead.
...
#define DMG_REMOVENORAGDOLL \
    (1 << 22)  // with this bit OR'd in, no ragdoll will be created, and the
               // target will be quietly removed. use this to kill an entity
               // that you've already got a server-side ragdoll for
...
enum {
    ...
    OBS_MODE_POI,  // PASSTIME point of interest - game objective, big fight,
                   // anything interesting; added in the middle of the enum due
                   // to tons of hard-coded "<ROAMING" enum compares
...
enum {
    ...
    OBS_ALLOW_NONE,  // don't allow any spectating after death (fixed & fade to
                     // black)
...
// of options.  Passing a command string is dangerous and allowed a server
// network message to run arbitrary commands on the client.
//=============================================================================
enum {
...
enum {
    EFL_KILLME = (1 << 0),   // This entity is marked for death -- This allows
                             // the game to actually delete ents at a safe time
    ...
    EFL_KEEP_ON_RECREATE_ENTITIES =
        (1 << 4),  // This is a special entity that should not be deleted when
                   // we restart entities only
    ...
    EFL_IN_SKYBOX =
        (1
         << 17),  // This is set if the entity detects that it's in the skybox.
                  // This forces it to pass the "in PVS" for transmission.
    EFL_USE_PARTITION_WHEN_NOT_SOLID =
        (1 << 18),  // Entities with this flag set show up in the partition even
                    // when not solid
    EFL_TOUCHING_FLUID =
        (1 << 19),  // Used to determine if an entity is floating

    // FIXME: Not really sure where I should add this...
    EFL_IS_BEING_LIFTED_BY_BARNACLE = (1 << 20),
    EFL_NO_ROTORWASH_PUSH =
        (1 << 21),  // I shouldn't be pushed by the rotorwash
    ...
    EFL_NO_WATER_VELOCITY_CHANGE =
        (1 << 29),  // Don't adjust this entity's velocity when transitioning
                    // into water
...
struct FireBulletsInfo_t {
    ...
    int m_iTracerFreq;
    float m_flDamage;
    int m_iPlayerDamage;  // Damage to be used instead of m_flDamage if we hit a
                          // player
...
struct StepSimulationData {
    ...
    // This is the pre-pre-Think position, orientation (Quaternion) and tick
    // count
    ...
    // This is the goal or post-Think position and orientation (and Quaternion
    // for blending) and next think time tick
    StepSimulationStep m_Next;
    QAngle m_angNextRotation;

    // This variable is used so that we only compute networked origin/angles
    // once per tick
...
struct EmitSound_t {
    ...
    float
        m_flSoundTime;  ///< NOT DURATION, but rather, some absolute time in the
...
enum { CP_WARN_NORMAL = 0, CP_WARN_FINALCAP, CP_WARN_NO_ANNOUNCEMENTS };
...
#define TEAM_TRAIN_HILLS_ARRAY_SIZE \
    TEAM_TRAIN_MAX_TEAMS *TEAM_TRAIN_MAX_HILLS *TEAM_TRAIN_FLOATS_PER_HILL
