...
class CAmmoDef {
   public:
    ...
    void AddAmmoType(char const* name, int damageType, int tracerType,
                     ...
                     char const* carry_cvar, float physicsForceImpulse,
                     int nFlags, int minSplashSize = 4, int maxSplashSize = 8);
...
// Get the global ammodef object. This is usually implemented in each mod's game
// rules file somewhere, so the mod can setup custom ammo types.
CAmmoDef* GetAmmoDef();
