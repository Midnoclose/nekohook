...
class CEnvMeteorShared {
   public:
    ...
    void Init(int nID, float flStartTime, float flPassiveTime,
              const Vector &vecStartPosition, const Vector &vecDirection,
              float flSpeed, float flDamageRadius, const Vector &vecTriggerMins,
    ...
    //-------------------------------------------------------------------------
    // Returns whether or not the object will ever transition from skybox to
    // world.
    //-------------------------------------------------------------------------
   ...
   public:
    ...
    // object leaves the skybox and enters the world. m_flExitTime is the
    // opposite.
   ...
   private:
    ...
    void CalcEnterAndExitTimes(const Vector &vecTriggerMins,
                               const Vector &vecTriggerMaxs);
};
...
abstract_class IMeteorFactory {
   public:
    virtual void CreateMeteor(int nID, int iType, const Vector &vecPosition,
                              const Vector &vecDirection, float flSpeed,
                              float flStartTime, float flDamageRadius,
                              const Vector &vecTriggerMins,
...
class CEnvMeteorSpawnerShared {
   ...
   public:
    ...
    CNetworkVector(
        m_vecTriggerMins);  // World Bounds (Trigger) in 3D Skybox - Min
    CNetworkVector(
        m_vecTriggerMaxs);  //                                       Max
