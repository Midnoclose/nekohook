...
struct vehicleview_t {
    ...
    CNetworkVar(
        float,
        flPitchCurveZero);  // Pitch values below this are clamped to zero.
    CNetworkVar(
        float,
        flPitchCurveLinear);  // Pitch values above this are mapped directly.
    //		Spline in between.
    CNetworkVar(
        float,
        flRollCurveZero);  // Pitch values below this are clamped to zero.
    CNetworkVar(
        float,
        flRollCurveLinear);  // Roll values above this are mapped directly.
    //		Spline in between.
