...
class CVoiceLabel {
   public:
    ...
    int m_clientindex;  // Client index of the speaker. -1 if this label isn't
                        // being used.
};

// This is provided by each mod to access data that may not be the same across
// mods.
...
class CVoiceStatus /*: public vgui::CDefaultInputSignal*/
{
   ...
   public:
    ...
    // ackPosition is the bottom position of where CVoiceStatus will draw the
    // voice acknowledgement labels.
   ...
   public:
    ...
    // comes back from the server). When the server acknowledges that the local
    // client is talking, then entindex will be gEngfuncs.GetLocalPlayer().
    // entindex is -2 to represent the local client's voice being acked by the
    // server.
    void UpdateSpeakerStatus(int entindex, bool bTalking);

    // Call from the HUD_CreateEntities function so it can add sprites above
    // player heads.
    void DrawHeadLabels();
    ...
    // The server sends this message initially to tell the client to send their
    // state.
   ...
   public:
    ...
    // returns false if the player can't hear the other client due to game rules
    // (eg. the other team)
   ...
   private:
    ...
    int m_bServerModEnable;         // What we've sent to the server about our
                                    // "voice_modenable" cvar.

    vgui::VPANEL m_pParentPanel;
    CPlayerBitVec
        m_VoicePlayers;  // Who is currently talking. Indexed by client index.

    // This is the gamerules-defined list of players that you can hear. It is
