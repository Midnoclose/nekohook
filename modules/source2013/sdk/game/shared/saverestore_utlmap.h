...
class CUtlMapDataOps : public CDefSaveRestoreOps {
   public:
    ...
    virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
        datamap_t *pKeyDatamap =
            CTypedescDeducer<KEY_TYPE>::Deduce((UTLMAP *)NULL);
        datamap_t *pFieldDatamap =
            CTypedescDeducer<FIELD_TYPE>::Deduce((UTLMAP *)NULL);
        ...
        typename UTLMAP::CTree *pUtlRBTree =
            ((UTLMAP *)fieldInfo.pField)->AccessTree();
...
#define SaveUtlMap(pSave, pUtlMap, fieldtype)                                 \
    CUtlMapDataopsInstantiator<fieldtype>::GetDataOps(pUtlMap)->Save(pUtlMap, \
                                                                     pSave);

#define RestoreUtlMap(pRestore, pUtlMap, fieldtype)                      \
    CUtlMapDataopsInstantiator<fieldtype>::GetDataOps(pUtlMap)->Restore( \
        pUtlMap, pRestore);
...
#define DEFINE_UTLMAP(name, keyType, fieldtype)                         \
    {                                                                   \
        FIELD_CUSTOM, #name, {offsetof(classNameTypedef, name), 0}, 1,  \
            FTYPEDESC_SAVE, NULL,                                       \
            CUtlMapDataopsInstantiator<keyType, fieldtype>::GetDataOps( \
