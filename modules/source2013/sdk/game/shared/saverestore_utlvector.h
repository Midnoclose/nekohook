...
class CUtlVectorDataOps : public CDefSaveRestoreOps {
   public:
    CUtlVectorDataOps() { UTLCLASS_SAVERESTORE_VALIDATE_TYPE(FIELD_TYPE); }

    virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
        datamap_t *pArrayTypeDatamap =
        ...
        if (pArrayTypeDatamap == NULL) {
            if (nElems) {
                ...
                dataDesc.fieldSizeInBytes =
                    nElems * CDatamapFieldSizeDeducer<FIELD_TYPE>::FieldSize();
                pSave->WriteFields("elems", &((*pUtlVector)[0]), &dataMap,
                                   &dataDesc, 1);
            }
        } else {
            // @Note (toml 11-21-02): Save load does not support arrays of user
            // defined types (embedded)
            dataDesc.fieldSizeInBytes =
    ...
    virtual void Restore(const SaveRestoreFieldInfo_t &fieldInfo,
        ...
        if (pArrayTypeDatamap == NULL) {
            if (nElems) {
                ...
                dataDesc.fieldSizeInBytes =
                    nElems * CDatamapFieldSizeDeducer<FIELD_TYPE>::FieldSize();
                pRestore->ReadFields("elems", &((*pUtlVector)[0]), &dataMap,
                                     &dataDesc, 1);
            }
        } else {
            // @Note (toml 11-21-02): Save load does not support arrays of user
...
#define SaveUtlVector(pSave, pUtlVector, fieldtype)                  \
    CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(pUtlVector) \
        ->Save(pUtlVector, pSave);

#define RestoreUtlVector(pRestore, pUtlVector, fieldtype)            \
    CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(pUtlVector) \
        ->Restore(pUtlVector, pRestore);
...
#define DEFINE_UTLVECTOR(name, fieldtype)                              \
    {                                                                  \
        FIELD_CUSTOM, #name, {offsetof(classNameTypedef, name), 0}, 1, \
            FTYPEDESC_SAVE, NULL,                                      \
            CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(      \
...
#define DEFINE_GLOBAL_UTLVECTOR(name, fieldtype)                       \
    {                                                                  \
        FIELD_CUSTOM, #name, {offsetof(classNameTypedef, name), 0}, 1, \
            FTYPEDESC_SAVE | FTYPEDESC_GLOBAL, NULL,                   \
            CUtlVectorDataopsInstantiator<fieldtype>::GetDataOps(      \
                &(((classNameTypedef *)0)->name)),                     \
            NULL                                                       \
    }
