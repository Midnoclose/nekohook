...
// These structs are contained in each player's local data and shared by the
// client & server
...
struct audioparams_t {
    ...
    CNetworkArray(Vector, localSound, NUM_AUDIO_LOCAL_SOUNDS)
        CNetworkVar(int, soundscapeIndex);  // index of the current soundscape
                                            // from soundscape.txt
    CNetworkVar(int, localBits);       // if bits 0,1,2,3 are set then position
                                       // 0,1,2,3 are valid/used
