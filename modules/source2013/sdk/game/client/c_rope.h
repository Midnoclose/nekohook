...
#define MAX_ROPE_SEGMENTS \
    (ROPE_MAX_SEGMENTS + (ROPE_MAX_SEGMENTS - 1) * MAX_ROPE_SUBDIVS)
...
class C_RopeKeyframe : public C_BaseEntity {
    ...
    class CPhysicsDelegate : public CSimplePhysics::IHelper {
       public:
        virtual void GetNodeForces(CSimplePhysics::CNode *pNodes, int iNode,
                                   Vector *pAccel);
        virtual void ApplyConstraints(CSimplePhysics::CNode *pNodes,
                                      int nNodes);
   ...
   public:
    ...
    static C_RopeKeyframe *Create(
        C_BaseEntity *pStartEnt, C_BaseEntity *pEndEnt,
        int iStartAttachment = 0, int iEndAttachment = 0, float ropeWidth = 2,
        const char *pMaterialName =
            "cable/cable",  // Note: whoever creates the rope must
                            // use PrecacheModel for whatever material
                            // it specifies here.
        int numSegments = 5, int ropeFlags = ROPE_SIMULATE);

    // Create a client-only rope and initialize it with the parameters from the
    // KeyValues.
    static C_RopeKeyframe *CreateFromKeyValues(C_BaseAnimating *pEnt,
                                               KeyValues *pValues);

    // Find ropes (with both endpoints connected) that intersect this AABB. This
    // is just an approximation.
    static int GetRopesIntersectingAABB(C_RopeKeyframe **pRopes, int nMaxRopes,
    ...
    // Hook the physics. Pass in your own implementation of
    // CSimplePhysics::IHelper. The default implementation is returned so you
    // can call through to it if you want.
    CSimplePhysics::IHelper *HookPhysics(CSimplePhysics::IHelper *pHook);

    // Attach to things (you can also just lock the endpoints down yourself if
    // you hook the physics).

    // Client-only right now. This could be moved to the server if there was a
    // good reason.
    void SetColorMod(const Vector &vColorMod);

    // Use this when rope length and slack change to recompute the spring
    // length.
   ...
   public:
    ...
    // Specify ROPE_ATTACHMENT_START_POINT or ROPE_ATTACHMENT_END_POINT for the
    // attachment.
   ...
   private:
    ...
    Vector ConstrainNode(const Vector &vNormal, const Vector &vNodePosition,
                         const Vector &vMidpiont, float fNormalLength);
    ...
    bool CalculateEndPointAttachment(C_BaseEntity *pEnt, int iAttachment,
                                     Vector &vPos, QAngle *pAngles);

   private:
    // Track which links touched something last frame. Used to prevent wind from
    // gusting on them.
    ...
    int m_iRopeMaterialModelIndex;  // Index of sprite model with the rope's
                                    // material.

    CRopePhysics<ROPE_MAX_SEGMENTS> m_RopePhysics;
    Vector m_LightValues[ROPE_MAX_SEGMENTS];  // light info when the rope is
                                              // created.
    ...
    EHANDLE m_hEndPoint;
    short m_iStartAttachment;  // StartAttachment/EndAttachment are attachment
                               // points.
    ...
    IMaterial *m_pBackMaterial;  // Optional translucent background material for
                                 // the rope to help reduce aliasing.
    ...
    bool m_bConstrainBetweenEndpoints;  // Simulated segment points won't
                                        // stretch beyond the endpoints

    bool m_bEndPointAttachmentPositionsDirty : 1;
    bool m_bEndPointAttachmentAnglesDirty : 1;
    bool m_bNewDataThisFrame : 1;  // Set to true in OnDataChanged so that we
                                   // simulate that frame
