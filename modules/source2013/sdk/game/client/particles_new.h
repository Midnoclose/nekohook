...
class CNewParticleEffect : public IParticleEffect,
   ...
   public:
    ...
    // Call this before adding a bunch of particles to give it a rough estimate
    // of where your particles are for sorting amongst other translucent
    // entities.
    ...
    // check if the new bounds of the particle system needs its client-leaf info
    // needs to be updated
    ...
    Particle *AddParticle(unsigned int particleSize, PMaterialHandle material,
                          const Vector &origin);
    ...
    static CSmartPtr<CNewParticleEffect> Create(CBaseEntity *pOwner,
   ...
   protected:
    ...
    // holds the min/max bounds used to manage this thing in the client leaf
    // system
    Vector m_LastMin;
    Vector m_LastMax;
   ...
   private:
    ...
    int m_RefCount;  // When this goes to zero and the effect has no more active
                     // particles, (and it's dynamically allocated), it will
                     // delete itself.

    CNewParticleEffect(
        const CNewParticleEffect &);  // not defined, not accessible
};
...
// Call this before adding a bunch of particles to give it a rough estimate of
// where your particles are for sorting amongst other translucent entities.
...
inline Particle *CNewParticleEffect::AddParticle(unsigned int particleSize,
                                                 PMaterialHandle material,
                                                 const Vector &origin) {
    //!!
    Assert(0);
    return NULL;
...
inline CSmartPtr<CNewParticleEffect> CNewParticleEffect::Create(
    CBaseEntity *pOwner, const char *pParticleSystemName,
    const char *pDebugName) {
    CNewParticleEffect *pRet =
        new CNewParticleEffect(pOwner, pParticleSystemName);
...
inline CSmartPtr<CNewParticleEffect> CNewParticleEffect::Create(
    CBaseEntity *pOwner, CParticleSystemDefinition *pDef,
    const char *pDebugName) {
...
//--------------------------------------------------------------------------------
// If you use an HPARTICLEFFECT instead of a cnewparticleeffect *, you get a
// pointer which will go to null when the effect is deleted
//--------------------------------------------------------------------------------
