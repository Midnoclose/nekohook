...
class C_BaseExplosionEffect {
   ...
   public:
    ...
    virtual void Create(const Vector &position, float force, float scale,
                        int flags);
   ...
   protected:
    ...
    float ScaleForceByDeviation(Vector &deviant, Vector &source, float spread,
                                float *force = NULL);

    float Probe(const Vector &origin, Vector *direction, float strength);
    void GetForceDirection(const Vector &origin, float magnitude,
                           Vector *resultDirection, float *resultForce);
