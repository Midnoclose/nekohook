...
abstract_class IReplayItemManager : public IBaseInterface {
   public:
    ...
    virtual void GetItems(CUtlLinkedList<IQueryableReplayItem *, int> &
                          items) = 0;
    virtual IQueryableReplayItem *GetItem(ReplayItemHandle_t hItem) = 0;
    virtual bool AreItemsMovies() = 0;
    virtual void DeleteItem(vgui::Panel * pPage, ReplayItemHandle_t hItem,
                            bool bNotifyUI) = 0;
};
...
IQueryableReplayItem *FindReplayItem(ReplayItemHandle_t hItem,
                                     IReplayItemManager **ppItemManager);
