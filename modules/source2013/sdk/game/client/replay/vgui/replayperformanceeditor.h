...
// NOTE: Should not change order here - if you do, you need to modify
// g_pCamNames.
enum CameraMode_t {
    CAM_INVALID = -1,
    CAM_FREE,
...
class CReplayPerformanceEditorPanel : public vgui::EditablePanel,
   ...
   private:
    ...
    void EnsureRecording(bool bShouldSnip = true);  // Start recording now if
   ...
   private:
    ...
    static void OnConfirmSaveAs(bool bShouldSave, wchar_t *pTitle,
                                void *pContext);
    ...
    float
        m_flLastTime;  // Can't use gpGlobals->frametime when playback is paused
    float m_flOldFps;
    ...
    float m_flSpaceDownStart;  // The time at which user started holding down
                               // space bar
    bool m_bSpaceDown;
    ...
    bool m_bMouseDown;
    float m_flDefaultFramerate;  // host_framerate before perf editor started
                                 // mucking about with it
    CameraMode_t
        m_nMouseClickedOverCameraSettingsPanel;  // Allows user to drag slider
                                                 // outside of camera settings
                                                 // panel w/o the panel
                                                 // disappearing
    CRecLightPanel *m_pRecLightPanel;
    bool m_bShownAtLeastOnce;  // Has the replay editor shown at least once?  In
                               // other words, has the user hit the space bar at
                               // all yet?
    char m_szSuspendedEvent[128];

    bool m_bAchievementAwarded;  // Was an achievement awarded during this
                                 // editing session?
    float m_flLastTimeSpaceBarPressed;
    float m_flActiveTimeInEditor;  // Will be zero'd out if user is idle (ie if
                                   // they don't press space bar often enough)

    CPanelAnimationVarAliasType(int, m_nRightMarginWidth, "right_margin_width",
                                "0", "proportional_xpos");
...
CReplayPerformanceEditorPanel *ReplayUI_InitPerformanceEditor(
