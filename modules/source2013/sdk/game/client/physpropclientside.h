...
class C_PhysPropClientside : public C_BreakableProp,
   ...
   public:
    ...
    float m_flTouchDelta;  // Amount of time that must pass before another touch
                           // function can be called
    float m_fDeathTime;    // Point at which this object self destructs.
                           // The default of -1 indicates the object shouldn't
                           // destruct.
...
//-----------------------------------------------------------------------------
// Purpose: A clientside zone that respawns physics props in it when the player
// leaves the PVS
//-----------------------------------------------------------------------------
class C_FuncPhysicsRespawnZone : public C_BaseEntity {
   ...
   public:
    ...
    bool CanMovePropAt(Vector vecOrigin, const Vector &vecMins,
