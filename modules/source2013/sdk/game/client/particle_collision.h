...
class CBaseSimpleCollision {
   public:
    ...
    virtual void TraceLine(const Vector &start, const Vector &end,
                           trace_t *pTrace, bool coarse = true);

    void ClearActivePlanes(void);

   protected:
    virtual void TestForPlane(const Vector &start, const Vector &dir,
...
class CParticleCollision : public CBaseSimpleCollision {
   public:
    ...
    virtual void Setup(const Vector &origin, const Vector *dir,
                       float angularSpread, float minSpeed, float maxSpeed,
                       float gravity, float dampen);
    virtual bool MoveParticle(Vector &origin, Vector &velocity,
                              float *rollDelta, float timeDelta,
                              trace_t *pTrace);
