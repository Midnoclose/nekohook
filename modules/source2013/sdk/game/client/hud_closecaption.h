...
struct AsyncCaption_t {
    ...
    AsyncCaption_t &operator=(const AsyncCaption_t &rhs) {
        ...
        for (int i = rhs.m_RequestedBlocks.FirstInorder();
             i != rhs.m_RequestedBlocks.InvalidIndex();
...
class CHudCloseCaption : public CHudElement, public vgui::Panel {
   ...
   public:
    ...
    virtual void LevelShutdown(void) { Reset(); }
    ...
    // Clear all CC data
    void Reset(void);
    void Process(const wchar_t *stream, float duration, char const *tokenstream,
                 bool fromplayer, bool direct = false);

    bool ProcessCaption(char const *tokenname, float duration,
                        bool fromplayer = false, bool direct = false);
    void ProcessCaptionDirect(char const *tokenname, float duration,
    ...
    void OnFinishAsyncLoad(int nFileIndex, int nBlockNum,
   ...
   private:
    ...
    bool AddAsyncWork(char const *tokenstream, bool bIsStream, float duration,
                      bool fromplayer, bool direct = false);

    void _ProcessSentenceCaptionStream(int wordCount, char const *tokenstream,
                                       const wchar_t *caption_full);
    void _ProcessCaption(const wchar_t *caption, char const *tokenname,
                         float duration, bool fromplayer, bool direct = false);
   ...
   private:
    static bool CaptionTokenLessFunc(const CaptionRepeat &lhs,
                                     const CaptionRepeat &rhs);

    void DrawStream(wrect_t &rect, wrect_t &rcWindow, CCloseCaptionItem *item,
                    int iFadeLine, float flFadeLineAlpha);
    ...
    bool GetFloatCommandValue(const wchar_t *stream, const wchar_t *findcmd,
                              float &value) const;
