...
class C_BasePlayer : public C_BaseCombatCharacter, public CGameEventListener {
   public:
    ...
    // This can be overridden to return something other than m_pRagdoll if the
    // mod uses separate entities for ragdolls.
    ...
    void BuildFirstPersonMeathookTransformations(
    ...
    // vDelta is the line from where the client had predicted the player to at
    // the usercmd in question, to where the server says the client should be at
    // said usercmd.
    ...
    bool HintMessage(int hint, bool bForce = false, bool bOnlyIfClear = false) {
        return Hints() ? Hints()->HintMessage(hint, bForce, bOnlyIfClear)
                       : false;
    }
   ...
   public:
    ...
    EHANDLE m_hZoomOwner;  // This is a pointer to the entity currently
                           // controlling the player's zoom Only this entity can
                           // change the zoom state once it has ownership
   ...
   protected:
    ...
    virtual void CalcInEyeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                  float &fov);
    ...
    virtual void CalcFreezeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                   float &fov);
   ...
   protected:
    ...
    bool ForceSetupBonesAtTimeFakeInterpolation(matrix3x4_t *pBonesOut,
                                                float curtimeOffset);
    ...
    int m_nForceVisionFilterFlags;  // Force our vision filter to a specific
                                    // setting
   ...
   public:
    ...
    bool ShouldGoSouth(Vector vNPCForward,
                       Vector vNPCRight);  // Such a bad name.
