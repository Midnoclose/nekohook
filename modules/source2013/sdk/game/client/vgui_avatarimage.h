...
//-----------------------------------------------------------------------------
// Purpose: avatar sizes, formerly used in ISteamFriends, but now only used in
// game code
//-----------------------------------------------------------------------------
enum EAvatarSize {
...
class CAvatarImage : public vgui::IImage {
   public:
    ...
    bool SetAvatarSteamID(CSteamID steamIDUser,
                          EAvatarSize avatarSize = k_EAvatarSize32x32);
    //=============================================================================
    // HPE_END
    //=============================================================================
    ...
    // Image will draw within the current panel context at the specified
    // position
   ...
   private:
    ...
    // [pmf] image to use as a fallback when get from steam fails (or not
    // called)
    ...
    CCallback<CAvatarImage, PersonaStateChange_t, false>
        m_sPersonaStateChangedCallback;
...
class CAvatarImagePanel : public vgui::Panel {
   public:
    ...
    //=============================================================================
    // HPE_BEGIN:
    // [menglish] Added default variable of scalable to allow the avatar to be
    // drawn at sizes other than 32, 32 [tj] added a parameter for drawing the
    // friend icon. Defaulted to true to maintain backward compatibility.
    // [menglish] Added parameter to specify a default avatar
    // [menglish] Added a function to set the avatar size of the AvatarImage
    //=============================================================================

    // reset the image to its default value, clearing any info retrieved from
    // Steam
    void ClearAvatar();
    ...
    // sets whether or not the image should scale to fit the size of the
    // ImagePanel (defaults to false)
    void SetShouldScaleImage(bool bScaleImage);

    // sets whether to automatically draw the friend icon behind the avatar for
    // Steam friends
    void SetShouldDrawFriendIcon(bool bDrawFriend);

    // specify the size of the avatar portion of the image (the actual image may
    // be larger than this when it incorporates the friend icon)
