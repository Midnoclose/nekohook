...
class CHudElement : public CGameEventListener {
   public:
    ...
    // called whenever the hud receives "reset" message, which is (usually)
    // every time the client respawns after getting killed
    ...
    // Return true if this hud element should be visible in the current hud
    // state
    ...
    // hidden by panels with a lower priority and will only lock out panels with
    // a lower priority
