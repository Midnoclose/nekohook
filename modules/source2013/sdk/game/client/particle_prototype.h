...
// This defines things that allow particle effects to hook into the particle
// app.
...
class IPrototypeArgAccess {
   public:
    virtual const char *FindArg(const char *pName,
                                const char *pDefault = 0) = 0;
};

// You must implement this interface for the prototype app to be able to run
// your effect.
class IPrototypeAppEffect {
   public:
    ...
    virtual void Start(CParticleMgr *pParticleMgr,
                       IPrototypeArgAccess *pArgs) = 0;

    // Return false if you don't allow properties to be edited in the prototype
    // app.
...
extern PrototypeEffectLink
    *g_pPrototypeEffects;  // The list of prototype effects..
...
#define EXPOSE_PROTOTYPE_EFFECT(effectName, className)       \
    ...
    static PrototypeEffectLink ___effectlink_##effectName##( \
        ___Create##effectName##, #effectName);
#else
#define EXPOSE_PROTOTYPE_EFFECT(effectName, className)
#endif
