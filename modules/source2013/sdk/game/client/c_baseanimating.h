...
{
        virtual void RenderMaterial( C_BaseEntity *pEntity, int count, const
vec4_t *verts, const vec4_t *normals, const vec2_t *texcoords, vec4_t
*lightvalues );
};
*/
...
struct ClientModelRenderInfo_t : public ModelRenderInfo_t {
    // Added space for lighting origin override. Just allocated space, need to
    // set base pointer
...
class C_BaseAnimating : public C_BaseEntity, private IModelLoadCallback {
   public:
    ...
    void MaintainSequenceTransitions(IBoneSetup &boneSetup, float flCycle,
                                     Vector pos[], Quaternion q[]);
    virtual void AccumulateLayers(IBoneSetup &boneSetup, Vector pos[],
                                  Quaternion q[], float currentTime);

    virtual void ChildLayerBlend(Vector pos[], Quaternion q[],
    ...
    // Bone attachments
    virtual void AttachEntityToBone(C_BaseAnimating *attachTarget,
                                    int boneIndexAttached = -1,
                                    Vector bonePosition = Vector(0, 0, 0),
    ...
    virtual void SaveRagdollInfo(int numbones,
                                 const matrix3x4_t &cameraTransform,
    ...
    virtual bool GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0,
    ...
    inline bool IsModelScaleFractional()
        const;  /// very fast way to ask if the model scale is < 1.0f  (faster
                /// than if (GetModelScale() < 1.0f) )
    ...
    // All view model attachments origins are stretched so you can place
    // entities at them and they will match up with where the attachment winds
    // up being drawn on the view model, since the view models are drawn with a
    // different FOV.
    //
    // If you're drawing something inside of a view model's DrawModel()
    // function, then you want the original attachment origin instead of the
    // adjusted one. To get that, call this on the adjusted attachment origin.
    ...
    // Used for debugging. Will produce asserts if someone tries to setup bones
    // or attachments before it's allowed. Use the "AutoAllowBoneAccess" class
    // to auto push/pop bone access. Use a distinct "tag" when pushing/popping -
    // asserts when push/pop tags do not match.
    struct AutoAllowBoneAccess {
        AutoAllowBoneAccess(bool bAllowForNormalModels,
    ...
    virtual void
    DoMuzzleFlash();  // Force a muzzle flash event. Note: this only QUEUES an
                      // event, so ProcessMuzzleFlashEvent will get called
                      // later.
   ...
   protected:
    // View models scale their attachment positions to account for FOV. To get
    // the unmodified attachment position (like if you're rendering something
    // else during the view model's DrawModel call), use
   ...
   private:
    // This method should return true if the bones have changed + SetupBones
    // needs to be called
   ...
   protected:
    ...
    // Bone attachments. Used for attaching one BaseAnimating to another's
    // bones. Client side only.
   ...
   private:
    ...
    int m_nOldSequence;
    CBoneMergeCache *m_pBoneMergeCache;  // This caches the strcmp lookups that
                                         // it has to do when merg
...
inline CStudioHdr *C_BaseAnimating::GetModelPtr() const {
    if (IsDynamicModelLoading()) return NULL;
        ...
        // it up first chance.
//	static IDataCacheSection *pModelCache = datacache->FindSection(
//"ModelData" ); 	AssertOnce( pModelCache->IsFrameLocking() );
#endif
    if (!m_pStudioHdr) {
        const_cast<C_BaseAnimating *>(this)->LockStudioHdr();
    }
    Assert(m_pStudioHdr ? m_pStudioHdr->GetRenderHdr() ==
                              mdlcache->GetStudioHdr(m_hStudioHdr)
...
inline bool C_BaseAnimating::IsModelScaled() const {
    return (m_flModelScale > 1.0f + FLT_EPSILON ||
            m_flModelScale < 1.0f - FLT_EPSILON);
}
