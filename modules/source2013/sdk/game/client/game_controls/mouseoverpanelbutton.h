...
class MouseOverButton : public vgui::Button {
   ...
   public:
    MouseOverButton(vgui::Panel *parent, const char *panelName,
                    T *templatePanel)
        ...
        m_pPanel->SetAutoResize(templatePanel->GetPinCorner(),
                                templatePanel->GetAutoResize(), px, py, rx, ry);
    ...
    const char *GetClassPage(const char *className) {
        ...
        if (g_pFullFileSystem->FileExists(classPanel,
                                          IsX360() ? "MOD" : "GAME")) {
        } else if (g_pFullFileSystem->FileExists("classes/default.res",
                                                 IsX360() ? "MOD" : "GAME")) {
    ...
    virtual void OnCursorEntered() {
        ...
        // are we preserving the armed state (and need to turn off the old
        // button)?
        if (m_bPreserveArmedButtons) {
