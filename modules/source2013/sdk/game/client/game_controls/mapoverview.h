...
class CMapOverview : public CHudElement,
   ...
   protected:                  // private structures & types
    ...
    typedef struct MapPlayer_s {
        ...
        Vector2D trail[MAX_TRAIL_LENGTH];  // save 1 footstep each second for 1
                                           // minute
    } MapPlayer_t;
...
#define MAP_OBJECT_ALIGN_TO_MAP (1 << 0)
   ...
   protected:
    ...
    /*virtual bool	DrawIcon(	int textureID,
                                                            int
       offscreenTextureID, Vector pos, float scale, float angle, int alpha =
       255, const char *text = NULL, Color *textColor = NULL, float status = -1,
                                                            Color *statusColor =
       NULL, int objectType = OBJECT_TYPE_NORMAL );*/
