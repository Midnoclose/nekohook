...
class CBaseModelPanel : public CMDLPanel {
   ...
   public:
    ...
    // Set to true if external code has set a specific camera position that
    // shouldn't be clobbered by layout
    void SetForcedCameraPosition(bool bForcedCameraPosition) {
        m_bForcedCameraPosition = bForcedCameraPosition;
    }

    int FindSequenceFromActivity(CStudioHdr *pStudioHdr,
   ...
   protected:
    ...
    CPanelAnimationVar(bool, m_bDisableManipulation, "disable_manipulation",
                       "0");
    ...
    struct particle_data_t {
        ...
        void UpdateControlPoints(CStudioHdr *pStudioHdr,
