...
class CImageMouseOverButton : public MouseOverButton<T> {
   ...
   public:
    CImageMouseOverButton(vgui::Panel *parent, const char *panelName,
                          T *templatePanel);
...
template <class T>
CImageMouseOverButton<T>::CImageMouseOverButton(vgui::Panel *parent,
...
void CImageMouseOverButton<T>::ApplySettings(KeyValues *inResourceData) {
    ...
    const char *inactiveImageName =
        inResourceData->GetString("inactiveimage", "");
...
void CImageMouseOverButton<T>::ApplySchemeSettings(vgui::IScheme *pScheme) {
    ...
    if (m_pszActiveImageName && strlen(m_pszActiveImageName) > 0) {
        this->SetActiveImage(
            vgui::scheme()->GetImage(m_pszActiveImageName, m_bScaleImage));
    }

    if (m_pszInactiveImageName && strlen(m_pszInactiveImageName) > 0) {
        this->SetInactiveImage(
            vgui::scheme()->GetImage(m_pszInactiveImageName, m_bScaleImage));
    }
...
void CImageMouseOverButton<T>::OnSizeChanged(int newWide, int newTall) {
    if (m_bScaleImage) {
        ...
        if (m_pActiveImage) m_pActiveImage->SetSize(newWide, newTall);

        if (m_pInactiveImage) m_pInactiveImage->SetSize(newWide, newTall);
    }
