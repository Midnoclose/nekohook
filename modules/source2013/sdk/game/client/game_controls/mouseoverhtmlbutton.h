...
class MouseOverHTMLButton : public vgui::Button {
   public:
    MouseOverHTMLButton(vgui::Panel *parent, const char *panelName,
                        vgui::HTML *html, const char *page)
    ...
    virtual void SetText(const char *text) {
        if (m_iIndex != -1) {
            ...
            if (text[0] == '#' && (ansiLocal = g_pVGuiLocalize->Find(text))) {
                ...
                wcsncpy(localizeText, ansiLocal,
                        sizeof(localizeText) / sizeof(wchar_t));
            } else {
            ...
            if (m_bAddShortCut) {
#ifdef WIN32
                _snwprintf(newText, sizeof(newText) / sizeof(wchar_t),
                           L"&%i %s", m_iIndex, localizeText);
#else
                _snwprintf(newText, sizeof(newText) / sizeof(wchar_t),
                           L"&%i %S", m_iIndex, localizeText);
#endif
