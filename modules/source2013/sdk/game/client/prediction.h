...
class CPrediction : public IPrediction {
   ...
   public:
    ...
    float GetIdealPitch(void) const { return m_flIdealPitch; }
    ...
    virtual void RunCommand(C_BasePlayer *player, CUserCmd *ucmd,
   ...
   protected:
    ...
    virtual void FinishMove(C_BasePlayer *player, CUserCmd *ucmd,
                            CMoveData *move);
    virtual void SetIdealPitch(C_BasePlayer *player, const Vector &origin,
                               const QAngle &angles, const Vector &viewheight);
   ...
   private:
    ...
    bool PerformPrediction(bool received_new_world_update,
                           C_BasePlayer *localPlayer, int incoming_acknowledged,
    ...
    void RunSimulation(int current_command, float curtime, CUserCmd *cmd,
