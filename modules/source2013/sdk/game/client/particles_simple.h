...
// ------------------------------------------------------------------------------------------------
// // CParticleEffect is the base class that you can derive from to make a
// particle effect. These can be used two ways:
//
// 1. Allocate a CParticleEffect-based object using the class's static Create()
// function. This gives
...
class CParticleEffect : public IParticleEffect {
   ...
   protected:
    ...
    enum {
        FLAG_ALLOCATED =
            (1 << 1),  // Most of the CParticleEffects are dynamically allocated
                       // but some are member variables of a class. If they're
                       // member variables.
...
enum SimpleParticleFlag_t {
    ...
    SIMPLE_PARTICLE_FLAG_NO_VEL_DECAY =
        0x2  // Used by the blood spray emitter. By default, it decays the
             // particle velocity.
};
...
// for the effect so they don't have to implement the simulation code. It
// simulates velocity, and fades their alpha from invisible to solid and back to
// invisible over their lifetime.
//
// Particles you add using this effect must use the class
// CParticleSimple::Particle.
class CSimpleEmitter : public CParticleEffect {
   ...
   public:
    ...
    SimpleParticle *AddSimpleParticle(PMaterialHandle hMaterial,
