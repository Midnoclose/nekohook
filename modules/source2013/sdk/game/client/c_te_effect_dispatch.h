...
// If you do DECLARE_CLIENT_EFFECT( "MyEffectName", MyCallback ), then
// MyCallback will be called when the server does DispatchEffect( "MyEffect",
// data )
//
#define DECLARE_CLIENT_EFFECT(effectName, callbackFunction)              \
    static CClientEffectRegistration ClientEffectReg_##callbackFunction( \
        effectName, callbackFunction);

void DispatchEffectToCallback(const char *pEffectName,
                              const CEffectData &m_EffectData);
