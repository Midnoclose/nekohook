...
// These are set as it draws reflections, refractions, etc, so certain effects
// can avoid drawing themselves in reflections.
enum DrawFlags_t {
...
abstract_class IViewRender {
   public:
    ...
    // RENDERING
    // Called right before simulation. It must setup the view model origins and
    // angles here so the correct attachment points can be used during
    // simulation.
    ...
    // Called to render just a particular setup ( for timerefresh and envmap
    // creation )
    ...
    virtual void SetCheapWaterStartDistance(
        float flCheapWaterStartDistance) = 0;
    virtual void SetCheapWaterEndDistance(float flCheapWaterEndDistance) = 0;

    virtual void GetWaterLODParams(float &flCheapWaterStartDistance,
                                   float &flCheapWaterEndDistance) = 0;
    ...
    virtual void WriteReplayScreenshot(WriteReplayScreenshotParams_t &
                                       params) = 0;
