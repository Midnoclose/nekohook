...
class CBoneMergeCache {
   public:
    ...
    void CopyParentToChild(const Vector parentPos[], const Quaternion parentQ[],
                           Vector childPos[], Quaternion childQ[],
                           int boneMask);
    void CopyChildToParent(const Vector childPos[], const Quaternion childQ[],
                           Vector parentPos[], Quaternion parentQ[],
                           int boneMask);

    // Returns true if the specified bone is one that gets merged in
    // MergeMatchingBones.
   ...
   private:
    ...
    // All the cache data is based off these. When they change, the cache data
    // is regenerated. These are either all valid pointers or all NULL.
    C_BaseAnimating *m_pFollow;
    CStudioHdr *m_pFollowHdr;
    const studiohdr_t *m_pFollowRenderHdr;
    CStudioHdr *m_pOwnerHdr;

    // This is the mask we need to use to set up bones on the followed entity to
    // do the bone merge
    ...
    CUtlVector<unsigned char>
        m_BoneMergeBits;  // One bit for each bone. The bit is set if the bone
                          // gets merged.
};
