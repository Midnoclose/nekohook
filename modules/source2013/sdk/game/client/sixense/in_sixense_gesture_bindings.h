...
class SixenseGestureBindings {
   public:
    ...
    // Add a new binding. release_command can be empty. If press_command starts
    // with '+', a release_command is generated with '-'.
    void AddBinding(CUtlString hand, CUtlString action, CUtlString arg,
                    CUtlString press_command, CUtlString release_command);

    // Print bindings to console
    void ListBindings();

    // Write the bindings to a cfg file that can be loaded from the console or
    // at startup (defaults to sixense_bindings.cfg)
    ...
    // Check to see if any bindings need to be triggered. disable_activations
    // allows the caller to prevent new bindings from being triggered, while
    // still allowing enabled gestures to disable.
    void UpdateBindings(sixenseUtils::IButtonStates *pLeftButtonStates,
                        sixenseUtils::IButtonStates *pRightButtonStates,
                        bool bIsMenuVisible);
