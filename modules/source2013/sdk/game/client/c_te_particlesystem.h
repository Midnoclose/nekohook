...
class CTEParticleRenderer : public CParticleEffect {
   public:
    ...
    static CSmartPtr<CTEParticleRenderer> Create(const char *pDebugName,
    ...
    // Get/set lifetime. Note: lifetime here is a counter. You set it to a value
    // and it counts down and disappears after that long.
   ...
   private:
    ...
    CTEParticleRenderer(
        const CTEParticleRenderer &);  // not defined, not accessible
...
// Inlines.
// ------------------------------------------------------------------------ //
inline void CTEParticleRenderer::SetParticleType(StandardParticle_t *pParticle,
                                                 ptype_t type) {
    pParticle->m_EffectData = (unsigned char)type;
}

inline ptype_t CTEParticleRenderer::GetParticleType(
    StandardParticle_t *pParticle) {
    return (ptype_t)pParticle->m_EffectData;
}

// Get/set lifetime. Note that
inline void CTEParticleRenderer::SetParticleLifetime(
    StandardParticle_t *pParticle, float lifetime) {
    pParticle->m_Lifetime = lifetime;
}

inline float CTEParticleRenderer::GetParticleLifetime(
    StandardParticle_t *pParticle) {
