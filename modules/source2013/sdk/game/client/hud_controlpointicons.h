...
// Options for how the cap progress teardrop positions itself around the cap
// point icon
enum {
    CP_DIR_N,
    CP_DIR_NW,
   ...
   public:
    ...
    virtual void PaintBackground(void) {
        ...
        if (GetImage()) {
            ...
            int iYPos = RemapValClamped(
                flElapsedTime, 0, STARTCAPANIM_SWOOP_LENGTH, 0, GetTall());
...
class CControlPointIconCapArrow : public vgui::Panel {
   ...
   public:
    CControlPointIconCapArrow(CControlPointIcon *pIcon, Panel *parent,
                              const char *name);
    ...
    void SetImage(const char *pszImage) {
        m_pArrowMaterial =
            materials->FindMaterial(pszImage, TEXTURE_GROUP_VGUI);
    }
...
class CControlPointIconCapturePulse : public vgui::ImagePanel {
   ...
   public:
    ...
    virtual void PaintBackground(void) {
        if (m_flFinishCapAnimStart &&
            gpGlobals->curtime > m_flFinishCapAnimStart) {
            float flElapsedTime =
                MAX(0, (gpGlobals->curtime - m_flFinishCapAnimStart));
            if (GetImage()) {
                surface()->DrawSetColor(255, 255, 255, 255);
                int iSize = RemapValClamped(flElapsedTime, 0,
                                            FINISHCAPANIM_SWOOP_LENGTH,
                                            GetWide(), m_iShrinkSize);
                GetImage()->SetPos((GetWide() - iSize) * 0.5,
                                   (GetTall() - iSize) * 0.5);
...
class CControlPointIcon : public vgui::EditablePanel, public CHudElement {
   ...
   public:
    ...
    void SetTimerTime(
        float flTime);  // used to display CCPTimerLogic countdowns
   ...
   private:
    ...
    DHANDLE<CExLabel>
        m_pCPTimerLabel;  // used to display CCPTimerLogic countdowns
    DHANDLE<vgui::ImagePanel>
        m_pCPTimerBG;  // used to display CCPTimerLogic countdowns
...
class CHudControlPointIcons : public CHudElement, public vgui::Panel {
   public:
    ...
    void DrawBackgroundBox(int xpos, int ypos, int nBoxWidth, int nBoxHeight,
                           bool bCutCorner);
    bool PaintTeamBaseIcon(int index, float flXPos, float flYPos,
                           float flIconSize);

    bool IsFakingCapture(int index = -1, bool *bMult = NULL,
                         float *flFakeTime = NULL) {
   ...
   private:
    ...
    int m_iCurrentCP;  // the index of the control point the local is currently
                       // in
    int m_iLastCP;  // the index of the control point the local player was last
                    // in
    ...
    CPanelAnimationVarAliasType(float, m_flIconExpand, "icon_expand", "0",
                                "proportional_float");
    CPanelAnimationVarAliasType(int, m_iIconSize, "iconsize", "24",
                                "proportional_int");

    CPanelAnimationVarAliasType(int, m_iIconGapWidth, "separator_width", "7",
                                "proportional_int");
    CPanelAnimationVarAliasType(int, m_iIconGapHeight, "separator_height", "9",
                                "proportional_int");
    CPanelAnimationVarAliasType(int, m_nHeightOffset, "height_offset", "0",
                                "proportional_int");
