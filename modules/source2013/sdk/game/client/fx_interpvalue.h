...
class CInterpolatedValue {
   public:
    ...
    CInterpolatedValue(float startTime, float endTime, float startValue,
                       float endValue, InterpType_t type);
    ...
    void Init(float startValue, float endValue, float dt,
              InterpType_t type = INTERP_LINEAR);

    // Start from the current value and move towards the end value
    void InitFromCurrent(float endValue, float dt,
                         InterpType_t type = INTERP_LINEAR);
