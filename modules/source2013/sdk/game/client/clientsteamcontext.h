...
class CClientSteamContext : public CSteamAPIContext {
   public:
...
#if !defined(NO_STEAM)
    STEAM_CALLBACK(CClientSteamContext, OnSteamServersDisconnected,
                   SteamServersDisconnected_t,
                   m_CallbackSteamServersDisconnected);
    STEAM_CALLBACK(CClientSteamContext, OnSteamServerConnectFailure,
                   SteamServerConnectFailure_t,
                   m_CallbackSteamServerConnectFailure);
    STEAM_CALLBACK(CClientSteamContext, OnSteamServersConnected,
                   SteamServersConnected_t, m_CallbackSteamServersConnected);
#endif
    ...
    // Allow others to register for a callback when the Steam logged on status
    // changes
    void InstallCallback(
        CUtlDelegate<void(const SteamLoggedOnChange_t &)> delegate);
    void RemoveCallback(
        CUtlDelegate<void(const SteamLoggedOnChange_t &)> delegate);
   ...
   private:
    ...
    CUtlVector<CUtlDelegate<void(const SteamLoggedOnChange_t &)> >
        m_LoggedOnCallbacks;
};
