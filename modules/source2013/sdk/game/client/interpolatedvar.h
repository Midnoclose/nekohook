...
class CInterpolationContext {
   public:
    CInterpolationContext() {
        ...
        // By default, disable extrapolation unless they call
        // EnableExtrapolation.
...
struct CInterpolatedVarEntryBase {
    ...
    // allocation pointers can be transferred (only one varentry has a copy),
    // but not trivially copied
   ...
   public:
    ...
    void EnsureCapacity(int capSize) {
        if (capSize > m_maxElement) {
            int newMax = m_maxElement +
                         ((capSize + m_growSize - 1) / m_growSize) * m_growSize;
            T *pNew = new T[newMax];
            for (int i = 0; i < m_maxElement; i++) {
                // ------------
                // If you wanted to make this a more generic container you'd
                // probably want this code instead - since FastTransferFrom() is
                // an optimization dependent on types stored here defining this
                // operation.
                // pNew[i] = m_pElements[WrapRange(i+m_firstElement)];
                pNew[i].FastTransferFrom(
                    m_pElements[WrapRange(i + m_firstElement)]);
                // ------------
            }
            m_firstElement = 0;
            m_maxElement = newMax;
...
class CInterpolatedVarArrayBase : public IInterpolatedVar {
   ...
   public:
    ...
    void GetDerivative_SmoothVelocity(
        Type *pOut,
        float currentTime);  // See notes on ::Derivative_HermiteLinearVelocity
                             // for info.
    ...
    // Returns the time difference betweem the most recent sample and its
    // previous sample.
   ...
   protected:
    ...
    // Force the time between prev and start to be dt (and extend prev out
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::NoteChanged(
    ...
    if (m_bDebug) {
        ...
        Msg("%s LatchChanged at %f changetime %f:  %s\n", GetDebugName(),
            gpGlobals->curtime, changetime, pDiffString);
    }
...
#else
    // JAY: It doesn't seem like the above code is correct.  This is keeping
    // more than two seconds of history for variables that aren't being
    // interpolated for some reason.  For example, the player model isn't drawn
    // in first person, so the history is only truncated here and will
    // accumulate ~40 entries instead of 2 or 3 changing over to the method in
    // Interpolate() means that we always have a 3-sample neighborhood around
    // any data we're going to need.  Unless gpGlobals->curtime is different
    // when samples are added vs. when they are interpolated I can't see this
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::AddToHead(
    ...
    if (bFlushNewer) {
        // Get rid of anything that has a timestamp after this sample. The
        // server might have corrected our clock and moved us back, so our
        // current changeTime is less than a changeTime we added samples during
        // previously.
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::RemoveEntriesPreviousTo(
    ...
    for (int i = 0; i < m_VarHistory.Count(); i++) {
        if (m_VarHistory[i].changetime < flTime) {
            ...
            // timestamp) and the sample right before it (for hermite blending),
            // and we can get rid of everything else.
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(
    ...
    for (int i = 0; i < varHistory.Count(); i++) {
        ...
        if (dt > 0.0001f) {
            ...
            // If pInfo->newer is the most recent entry we have, and all 2 or 3
            // other entries are identical, then we're always going to return
            // the same value if currentTime increases.
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::DebugInterpolate(
    ...
    if (info.m_bHermite) {
        ...
        if (CInterpolationContext::IsExtrapolationAllowed() &&
            ...
            // At this point, we know we're out of data and we have the ability
            // to get a velocity to extrapolate with.
            //
            // However, we only want to extraploate if the server is choking. We
            // don't want to extrapolate if the object legimately stopped moving
            // and the server stopped sending updates for it.
            //
            // The way we know that the server is choking is if we haven't heard
            // ANYTHING from it for a while. The server's update interval should
            // be at least as often as our interpolation amount (otherwise, we
            // wouldn't have the ability to interpolate).
            //
            // So right here, if we see that we haven't gotten any server
            // updates since the last interpolation history update to this
            // entity (and since we're in here, we know that we're out of
            // interpolation data), then we can assume that the server is
            // choking and decide to extrapolate.
            //
            // The End
            ...
            _Interpolate(pOut, info.frac, &history[info.older],
                         &history[info.newer]);
        }
    } else {
        _Interpolate(pOut, info.frac, &history[info.older],
...
inline int CInterpolatedVarArrayBase<Type, IS_ARRAY>::Interpolate(
    ...
    if (m_bDebug) {
        // "value will hold" means we are either extrapolating, or the samples
        // in GetInterpolationInfo are all the same... In either case there are
        // no more "changes" until we latch a new
        //  value and we can remove this var from the interpolated var list (bit
        //  perf optimization)
        Msg("%s Interpolate at %f%s\n", GetDebugName(), currentTime,
            noMoreChanges ? " [value will hold]" : "");
    }
    ...
    if (memcmp(backupValues, m_pValue, sizeof(Type) * m_nMaxCount) != 0) {
        ...
        // This undoes the work that we do in here so if someone is in the
        // debugger, they can find out which variable changed.
...
void CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetDerivative_SmoothVelocity(
    ...
    if (info.m_bHermite) {
        _Derivative_Hermite_SmoothVelocity(
            pOut, info.frac, &history[info.oldest], &history[info.older],
            &history[info.newer]);
        return;
    } else if (info.newer == info.older &&
        ...
        if (IsValidIndex(realOlder) && history[realOlder].changetime != 0.0) {
            ...
            //
            // So right here, if we see that we haven't gotten any server
            // updates for a whole interpolation interval, then we know the
            // server is choking.
            //
            // The End
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::Copy(
    ...
    if (!pSrc || pSrc->m_nMaxCount != m_nMaxCount) {
        if (pSrc) {
            AssertMsg3(false,
                       "pSrc->m_nMaxCount (%i) != m_nMaxCount (%i) for %s.",
                       pSrc->m_nMaxCount, m_nMaxCount, m_pDebugName);
        } else {
            AssertMsg(false,
                      "pSrc was null in CInterpolatedVarArrayBase<Type, "
                      "IS_ARRAY>::Copy.");
        }
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate(
    ...
    for (int i = 0; i < m_nMaxCount; i++) {
        if (m_bLooping[i]) {
            out[i] =
                LoopingLerp(frac, start->GetValue()[i], end->GetValue()[i]);
        } else {
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Extrapolate(
    ...
    if (fabs(pOld->changetime - pNew->changetime) < 0.001f ||
        ...
        for (int i = 0; i < m_nMaxCount; i++) {
            pOut[i] = ExtrapolateInterpolatedVarType(
                pOld->GetValue()[i], pNew->GetValue()[i], divisor,
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::TimeFixup2_Hermite(
    ...
    if (fabs(dt1 - dt2) > 0.0001f && dt2 > 0.0001f) {
        ...
        for (int i = 0; i < m_nMaxCount; i++) {
            if (m_bLooping[i]) {
                fixup.GetValue()[i] = LoopingLerp(1 - frac, prev->GetValue()[i],
                                                  start->GetValue()[i]);
            } else {
                fixup.GetValue()[i] =
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate_Hermite(
    ...
    // Disable range checks because we can produce weird values here and it's
    // not an error. After interpolation, we will clamp the values.
    ...
    for (int i = 0; i < m_nMaxCount; i++) {
        ...
        if (m_bLooping[i]) {
            out[i] =
                LoopingLerp_Hermite(frac, prev->GetValue()[i],
                                    start->GetValue()[i], end->GetValue()[i]);
        } else {
            out[i] = Lerp_Hermite(frac, prev->GetValue()[i],
                                  start->GetValue()[i], end->GetValue()[i]);
        }

        // Clamp the output from interpolation. There are edge cases where
        // something like m_flCycle can get set to a really high or low value
        // when we set it to zero after a really small time interval (the
        // hermite blender will think it's got a really high velocity and
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Hermite(
    ...
    for (int i = 0; i < m_nMaxCount; i++) {
        Assert(!m_bLooping[i]);
        out[i] = Derivative_Hermite(frac, prev->GetValue()[i],
                                    start->GetValue()[i], end->GetValue()[i]);
        out[i] *= divisor;
    }
}
...
CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Derivative_Hermite_SmoothVelocity(
    ...
    for (int i = 0; i < m_nMaxCount; i++) {
        Type prevVel = (c->GetValue()[i] - b->GetValue()[i]) /
                       (c->changetime - b->changetime);
        Type curVel = (d->GetValue()[i] - c->GetValue()[i]) /
                      (d->changetime - c->changetime);
