...
class CParticleRenderIterator {
   ...
   public:
    ...
    // reflections or monitors). These return const because you should only
    // modify particles during their simulation.
...
class CParticleSimulateIterator {
   ...
   public:
    ...
    // Iterate through the particles, simulate them, and remove them if
    // necessary.
    Particle *GetFirst();
    Particle *GetNext();
...
inline void CParticleRenderIterator::TestFlushBatch() {
    ...
    if (m_nParticlesInCurrentBatch >= NUM_PARTICLES_PER_BATCH) {
        ...
        m_pMeshBuilder->Begin(m_pMesh, MATERIAL_QUADS,
                              NUM_PARTICLES_PER_BATCH * 4);
