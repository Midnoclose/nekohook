...
class CGlowOverlay {
   ...
   public:
    ...
    // If this is set, then the overlay is only visible if the ray to it hits
    // the sky.
    bool m_bInSky;
    float m_skyObstructionScale;
#ifdef PORTAL
    float m_skyObstructionScaleBackups
        [MAX_PORTAL_RECURSIVE_VIEWS];  // used in portal mod during stencil
                                       // rendering to maintain obstructions
                                       // while rendering recursive views
#endif
   ...
   protected:
    ...
    virtual void CalcSpriteColorAndSize(float flDot, CGlowSprite *pSprite,
                                        float *flHorzSize, float *flVertSize,
                                        Vector *vColor);

    virtual void CalcBasis(const Vector &vToGlow, float flHorzSize,
                           float flVertSize, Vector &vBasePt, Vector &vUp,
                           Vector &vRight);
