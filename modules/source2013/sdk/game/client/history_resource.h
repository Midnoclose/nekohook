...
//-----------------------------------------------------------------------------
// Purpose: Used to draw the history of ammo / weapon / item pickups by the
// player
//-----------------------------------------------------------------------------
class CHudHistoryResource : public CHudElement, public vgui::Panel {
   ...
   private:
    struct HIST_ITEM {
        HIST_ITEM() {
            // init this here, because the code that overwrites previous history
            // items will use this to check to see if the item is empty
            DisplayTime = 0.0f;
        }
        int type;
        float DisplayTime;  // the time at which this item should be removed
                            // from the history
        int iCount;
        int iId;
   ...
   public:
    ...
    void AddIconToHistory(int iType, int iId, C_BaseCombatWeapon *weapon,
   ...
   private:
    ...
    CPanelAnimationVarAliasType(float, m_flHistoryGap, "history_gap", "42",
                                "proportional_float");
    CPanelAnimationVarAliasType(float, m_flIconInset, "icon_inset", "28",
                                "proportional_float");
    CPanelAnimationVarAliasType(float, m_flTextInset, "text_inset", "26",
                                "proportional_float");
