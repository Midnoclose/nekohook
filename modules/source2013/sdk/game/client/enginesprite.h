...
//-----------------------------------------------------------------------------
class CEngineSprite {
    // NOTE: don't define a constructor or destructor so that this can be
    // allocated as before.
   public:
    ...
    void DrawFrame(RenderMode_t nRenderMode, int frame, int x, int y,
                   const wrect_t *prcSubRect);
    void DrawFrameOfSize(RenderMode_t nRenderMode, int frame, int x, int y,
                         int iWidth, int iHeight, const wrect_t *prcSubRect);
    bool IsVideo();
    void GetTexCoordRange(float *pMinU, float *pMinV, float *pMaxU,
                          float *pMaxV);
