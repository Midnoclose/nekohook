...
typedef struct {
    int bIsDir; /* If non-zero, element is a directory; if zero, element is a
                   file */
    unsigned int uSizeOrCount; /* If element is a file, this contains size of
                                  file in bytes */
    ...
    long lLastAccessTime; /* Seconds since 1/1/1970 (like time_t) when element
                             was last accessed */
...
typedef struct {
    ...
    /* A ProofOfPurchase token is not necessarily a nul-terminated string; it
       may be binary data (perhaps encrypted). Hence we need a length and an
       array of bytes.							*/
...
typedef enum {
    ...
    /* Note: Mask value 0x2 is reserved for future use. (Some, but not all,
       public accounts already have this set.) */
