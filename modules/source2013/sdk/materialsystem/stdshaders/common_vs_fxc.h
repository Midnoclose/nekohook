...
//	SKIP: defined $LIGHTING_PREVIEW && defined $FASTPATH &&
...
//#define COMPRESSED_NORMALS_TYPE
//COMPRESSED_NORMALS_SEPARATETANGENTS_SHORT2
...
// The g_bLightEnabled registers and g_nLightCountRegister hold the same
// information regarding enabling lights, but callers internal to this file tend
// to use the loops, while external callers will end up using the booleans
...
// It is a binary value used to switch on/off the addition of the flex delta
// stream
...
// We expect this data to come from a signed SHORT2 stream in the range of
// -32768..32767
//
...
void _DecompressShort2Tangent(float2 inputTangent, out float4 outputTangent) {
    float2 ztSigns =
        sign(inputTangent);            // sign bits for z and tangent (+1 or -1)
    float2 xyAbs = abs(inputTangent);  // 1..32767
    outputTangent.xy = (xyAbs - 16384.0f) / 16384.0f;  // x and y
    outputTangent.z =
        ztSigns.x *
        sqrt(saturate(1.0f - dot(outputTangent.xy, outputTangent.xy)));
    outputTangent.w = ztSigns.y;
}

//-----------------------------------------------------------------------------------
// Same code as _DecompressShort2Tangent, just one returns a float4, one a
// float3
...
void _DecompressShort2NormalTangent(float2 inputNormal, float2 inputTangent,
                                    out float3 outputNormal,
                                    out float4 outputTangent) {
    // FIXME: if we end up sticking with the SHORT2 format, pack the normal and
    // tangent into a single SHORT4 element
    //        (that would make unpacking normal+tangent here together much
...
// We expect this data to come from an unsigned UBYTE4 stream in the range of
// 0..255 The final vTangent.w contains the sign to use in the cross product
// when generating a binormal
void _DecompressUByte4NormalTangent(
    float4 inputNormal,
    ...
    out float4 outputTangent)  // {tX, tY, tZ, sign of binormal}
{
    ...
    float4 ztztSignBits =
        (inputNormal - 128.0f) < 0;  // sign bits for zs and binormal (1 or 0)
                                     // set-less-than (slt) asm instruction
    float4 xyxyAbs = abs(inputNormal - 128.0f) - ztztSignBits;  // 0..127
    float4 xyxySignBits =
        (xyxyAbs - 64.0f) < 0;  // sign bits for xs and ys (1 or 0)
    float4 normTan =
        (abs(xyxyAbs - 64.0f) - xyxySignBits) / 63.0f;  // abs({nX, nY, tX, tY})
    ...
    outputNormal.z =
        1.0f - outputNormal.x - outputNormal.y;  // Project onto x+y+z=1
    outputNormal.xyz =
        normalize(outputNormal.xyz);  // Normalize onto unit sphere
    outputNormal.xy *= xyxySigns.xy;  // Restore x and y signs
    outputNormal.z *= ztztSigns.x;    // Restore z sign

    outputTangent.z =
        1.0f - outputTangent.x - outputTangent.y;  // Project onto x+y+z=1
    outputTangent.xyz =
        normalize(outputTangent.xyz);  // Normalize onto unit sphere
...
//-----------------------------------------------------------------------------------
// Decompress just a normal from four-component compressed format (same as
// above) We expect this data to come from an unsigned UBYTE4 stream in the
// range of 0..255 [ When compiled, this works out to approximately 17 asm
// instructions ]
                             ...
                             out float3 outputNormal)  // {nX, nY, nZ}
{
    ...
    float2 ztSigns = (inputNormal.xy - 128.0f) <
                     0;  // sign bits for zs and binormal (1 or 0) set-less-than
                         // (slt) asm instruction
    ...
    outputNormal.xy *=
        lerp(fOne.xx, -fOne.xx, xySigns);  // Restore x and y signs
...
void DecompressVertex_NormalTangent(float4 inputNormal, float4 inputTangent,
                                    out float3 outputNormal,
                                    out float4 outputTangent) {
    if (COMPRESSED_VERTS == 1) {
        if (COMPRESSED_NORMALS_TYPE ==
            COMPRESSED_NORMALS_SEPARATETANGENTS_SHORT2) {
            _DecompressShort2NormalTangent(inputNormal.xy, inputTangent.xy,
                                           outputNormal, outputTangent);
        } else  // ( COMPRESSED_NORMALS_TYPE ==
                // COMPRESSED_NORMALS_COMBINEDTANGENTS_UBYTE4 )
        {
            _DecompressUByte4NormalTangent(inputNormal, outputNormal,
                                           outputTangent);
        }
    } else {
        outputNormal = inputNormal.xyz;
...
// NOTE: vMorphTargetTextureDim.x = width, cVertexTextureDim.y = height,
// cVertexTextureDim.z = # of float4 fields per vertex For position + normal
// morph for example, there will be 2 fields.
//-----------------------------------------------------------------------------
float4 SampleMorphDelta(sampler2D vt, const float3 vMorphTargetTextureDim,
                        const float4 vMorphSubrect, const float flVertexID,
                        const float flField) {
    float flColumn = floor(flVertexID / vMorphSubrect.w);

    float4 t;
    t.x =
        vMorphSubrect.x + vMorphTargetTextureDim.z * flColumn + flField + 0.5f;
...
void SampleMorphDelta2(sampler2D vt, const float3 vMorphTargetTextureDim,
                       const float4 vMorphSubrect, const float flVertexID,
                       out float4 delta1, out float4 delta2) {
    float flColumn = floor(flVertexID / vMorphSubrect.w);
...
bool ApplyMorph(float3 vPosFlex, float3 vNormalFlex, inout float3 vPosition,
...
bool ApplyMorph(float3 vPosFlex, float3 vNormalFlex, inout float3 vPosition,
                inout float3 vNormal, inout float3 vTangent) {
    // Flexes coming in from a separate stream
    float3 vPosDelta = vPosFlex.xyz * cFlexScale.x;
    float3 vNormalDelta = vNormalFlex.xyz * cFlexScale.x;
    vPosition.xyz += vPosDelta;
...
bool ApplyMorph(float4 vPosFlex, float3 vNormalFlex, inout float3 vPosition,
                inout float3 vNormal, inout float3 vTangent,
                out float flWrinkle) {
    // Flexes coming in from a separate stream
    float3 vPosDelta = vPosFlex.xyz * cFlexScale.x;
...
bool ApplyMorph(sampler2D morphSampler, const float3 vMorphTargetTextureDim,
                const float4 vMorphSubrect, const float flVertexID,
                const float3 vMorphTexCoord, inout float3 vPosition) {
#if MORPHING

#if !DECAL
    // Flexes coming in from a separate stream
    float4 vPosDelta = SampleMorphDelta(morphSampler, vMorphTargetTextureDim,
                                        vMorphSubrect, flVertexID, 0);
...
bool ApplyMorph(sampler2D morphSampler, const float3 vMorphTargetTextureDim,
    ...
    SampleMorphDelta2(morphSampler, vMorphTargetTextureDim, vMorphSubrect,
                      flVertexID, vPosDelta, vNormalDelta);
    vPosition += vPosDelta.xyz;
...
float RangeFog(const float3 projPos) {
    return max(cFogMaxDensity,
               (-projPos.z * cOOFogRange + cFogEndOverFogRange));
}
...
float CalcFog(const float3 worldPos, const float3 projPos,
              const bool bWaterFog) {
#if defined(_X360)
    // 360 only does pixel fog
    return 1.0f;
#endif
...
float4 DecompressBoneWeights(const float4 weights) {
    ...
    if (COMPRESSED_VERTS) {
        ...
        // NOTE: we add 1 here so we can divide by 32768 - which is exact
        // (divide by 32767 is not).
        //       This avoids cracking between meshes with different numbers of
        //       bone weights. We use SHORT2 instead of SHORT2N for a similar
        //       reason - the GPU's conversion from [-32768,+32767] to [-1,+1]
        //       is imprecise in the same way.
        result += 1;
        result /= 32768;
    }
...
#endif
    {
        ...
        } else  // skinning - always three bones
        {
            ...
            float4x3 blendMatrix =
                mat1 * weights[0] + mat2 * weights[1] + mat3 * weights[2];
            worldPos = mul4x3(modelPos, blendMatrix);
        }
    }
}

void SkinPositionAndNormal(bool bSkinning, const float4 modelPos,
                           const float3 modelNormal, const float4 boneWeights,
                           float4 fBoneIndices, out float3 worldPos,
...
// Is it worth keeping SkinPosition and SkinPositionAndNormal around since the
// optimizer gets rid of anything that isn't used?
void SkinPositionNormalAndTangentSpace(
    bool bSkinning, const float4 modelPos, const float3 modelNormal,
...
#endif
    {
        if (!bSkinning) {
            ...
            worldNormal = mul3x3(modelNormal, (const float3x3)cModel[0]);
            worldTangentS =
                mul3x3((float3)modelTangentS, (const float3x3)cModel[0]);
        } else  // skinning - always three bones
        {
            float4x3 mat1 = cModel[boneIndices[0]];
            float4x3 mat2 = cModel[boneIndices[1]];
            ...
            worldTangentS =
                mul3x3((float3)modelTangentS, (const float3x3)blendMatrix);
        }
...
// The following "internal" routines are called "privately" by other routines in
// this file which handle the particular flavor of vs20 control flow appropriate
...
float VertexAttenInternal(const float3 worldPos, int lightNum) {
...
#if defined(_X360)
    {
        // X360 dynamic compile hits an internal compiler error using dst(),
        // this is the breakdown of how dst() works from the 360 docs.
        vDist.x = 1;
    ...
    float flSpotAtten = (flCosTheta - cLightInfo[lightNum].spotParams.z) *
                        cLightInfo[lightNum].spotParams.w;
    ...
    float flAtten = lerp(flDistanceAtten, flDistanceAtten * flSpotAtten,
                         cLightInfo[lightNum].dir.w);
...
float CosineTermInternal(const float3 worldPos, const float3 worldNormal,
                         int lightNum, bool bHalfLambert) {
    // Calculate light direction assuming this is a point or spot
    float3 lightDir = normalize(cLightInfo[lightNum].pos - worldPos);

    // Select the above direction or the one in the structure, based upon light
    // type
    lightDir =
        lerp(lightDir, -cLightInfo[lightNum].dir, cLightInfo[lightNum].color.w);
...
// This routine uses booleans to do early-outs and is meant to be called by
// routines OUTSIDE of this file
float GetVertexAttenForLight(const float3 worldPos, int lightNum,
                             bool bUseStaticControlFlow) {
    float result = 0.0f;
...
float3 DoLightInternal(const float3 worldPos, const float3 worldNormal,
                       int lightNum, bool bHalfLambert) {
...
float3 DoLighting(const float3 worldPos, const float3 worldNormal,
    ...
    if (bDynamicLight)  // Dynamic light
    {
        for (int i = 0; i < g_nLightCount; i++) {
            linearColor +=
                DoLightInternal(worldPos, worldNormal, i, bHalfLambert);
        }
    }

    if (bDynamicLight) {
        linearColor +=
            AmbientLight(worldNormal);  // ambient light is already remapped
    }
...
float3 DoLightingUnrolled(const float3 worldPos, const float3 worldNormal,
                          const float3 staticLightingColor,
                          const bool bStaticLight, const bool bDynamicLight,
    ...
    if (bDynamicLight)  // Ambient light
    {
        if (nNumLights >= 1)
            linearColor +=
                DoLightInternal(worldPos, worldNormal, 0, bHalfLambert);
        if (nNumLights >= 2)
            linearColor +=
                DoLightInternal(worldPos, worldNormal, 1, bHalfLambert);
        if (nNumLights >= 3)
            linearColor +=
                DoLightInternal(worldPos, worldNormal, 2, bHalfLambert);
        if (nNumLights >= 4)
            linearColor +=
                DoLightInternal(worldPos, worldNormal, 3, bHalfLambert);
    }
...
float2 ComputeSphereMapTexCoords(in float3 reflectionVector) {
    ...
    // generate <rx ry rz+1>
    float3 tmp = float3(reflectionVector.x, reflectionVector.y,
...
float3 ApplyDeformation(float3 worldpos, int deftype, float4 defparms0,
                        float4 defparms1, float4 defparms2, float4 defparms3) {
    float3 ret = worldpos;
