...
//  SKIP: $BUMPMASK && ( $SEAMLESS || $DETAILTEXTURE || $SELFILLUM ||
//  $BASETEXTURENOENVMAP || $BASETEXTURE2 )
...
// 360 compiler craps out on some combo in this family.  Content doesn't use
// blendmode 10 anyway
//  SKIP: $FASTPATH && $PIXELFOGTYPE && $BASETEXTURE2 && $DETAILTEXTURE &&
//  $CUBEMAP && ($DETAIL_BLEND_MODE == 10 ) [XBOX]
...
#define USE_32BIT_LIGHTMAPS_ON_360  // uncomment to use 32bit lightmaps, be sure
                                    // to keep this in sync with the same #define
                                    // in materialsystem/cmatlightmaps.cpp
...
struct PS_INPUT {
    ...
    // detail textures and bumpmaps are mutually exclusive so that we have
    // enough texcoords.
#if (RELIEF_MAPPING == 0)
...
#endif
{
    ...
    GetBaseTextureAndNormal(
        BaseTextureSampler, BaseTextureSampler2, BumpmapSampler, bBaseTexture2,
        bBumpmap || bNormalMapAlphaEnvmapMask, baseTexCoords, i.vertexColor.rgb,
        baseColor, baseColor2, vNormal);
    ...
    if (bBumpmap && bDiffuseBumpmap) {
        ...
        HALF2 bumpCoord1 = ComputeLightmapCoordinates(i.lightmapTexCoord1And2,
...
#else
    detailColor =
        float4(g_DetailTint, 1.0f) * tex2D(DetailSampler, detailTexCoord);
#endif
    ...
    if ((distAlphaMask >= OUTLINE_MIN_VALUE0) &&
        ...
        if (distAlphaMask <= OUTLINE_MIN_VALUE1) {
            ...
            oFactor = smoothstep(OUTLINE_MAX_VALUE1, OUTLINE_MAX_VALUE0,
                                 distAlphaMask);
        }
    ...
    if (bBaseTexture2) {
#if (SELFILLUM == 0) && (PIXELFOGTYPE != PIXEL_FOG_TYPE_HEIGHT) && \
    (FANCY_BLENDING)
    ...
    if (bBumpmap) {
...
#if (BUMPMAP2 == 1)
        {
            ...
            if (BUMPMAP == 2)
                ...
                vNormal2 = DecompressNormal(BumpmapSampler2, b2TexCoord,
                                            NORMAL_DECODE_MODE,
...
#if (BUMPMASK == 1)
            float3 vNormal1 = DecompressNormal(
                BumpmapSampler, i.detailOrBumpAndEnvmapMaskTexCoord.xy,
            ...
            vNormalMask =
                DecompressNormal(BumpMaskSampler, i.baseTexCoord.xy,
                                 NORMALMASK_DECODE_MODE, AlphaMaskSampler);

            vNormal.xyz = lerp(vNormalMask.xyz, vNormal.xyz,
                               vNormalMask.a);  // Mask out normals from vNormal
    ...
    if (bDetailTexture) {
        albedo = TextureCombine(albedo, detailColor, DETAIL_BLEND_MODE,
                                g_DetailBlendFactor);
    }
    ...
    // Save this off for single-pass flashlight, since we'll still need the
    // SSBump vector, not a real normal
...
// ssbump
        ...
        // now, calculate vNormal for reflection purposes. if vNormal isn't
        // needed, hopefully the compiler will eliminate these calculations
        vNormal.xyz =
            normalize(bumpBasis[0] * vNormal.x + bumpBasis[1] * vNormal.y +
        ...
        dp *= 2 * detailColor;
#endif
        diffuseLighting = dp.x * lightmapColor1 + dp.y * lightmapColor2 +
                          dp.z * lightmapColor3;
    ...
    // FIXME: 8-bit lookup textures like this need a "nice filtering" VTF
    // option, which converts
    //        them to 16-bit on load or does filtering in the shader (since most
    //        hardware - 360 included - interpolates 8-bit textures at 8-bit
    //        precision, which causes banding)
    ...
    // ssbump doesn't pass a normal to the flashlight...it computes shadowing a
    // different way
#if (BUMPMAP == 2)
    ...
    tangentPosToLightVector.x =
        dot(worldPosToLightVector, i.tangentSpaceTranspose[0]);
    tangentPosToLightVector.y =
        dot(worldPosToLightVector, i.tangentSpaceTranspose[1]);
    tangentPosToLightVector.z =
        dot(worldPosToLightVector, i.tangentSpaceTranspose[2]);

    tangentPosToLightVector = normalize(tangentPosToLightVector);

    float nDotL =
        saturate(vSSBumpVector.x * dot(tangentPosToLightVector, bumpBasis[0]) +
    ...
    float fFlashlight = DoFlashlight(
        g_FlashlightPos, i.worldPos_projPosZ.xyz, i.flashlightSpacePos,
        worldSpaceNormal, g_FlashlightAttenuationFactors.xyz,
        g_FlashlightAttenuationFactors.w, FlashlightSampler, ShadowDepthSampler,
        RandRotSampler, 0, true, false, i.vProjPos.xy / i.vProjPos.w, false,
        g_ShadowTweaks, bHasNormal);
    ...
    if (bSelfIllum) {
        ...
        diffuseComponent =
            lerp(diffuseComponent, selfIllumComponent, baseColor.a);
    }
    ...
    if (bCubemap) {
        ...
        float3 reflectVect = CalcReflectionVectorUnnormalized(
            worldSpaceNormal, worldVertToEyeVector);
        ...
        specularLighting =
            lerp(specularLighting, specularLightingSquared, g_EnvmapContrast);
        HALF3 greyScale = dot(specularLighting, HALF3(0.299f, 0.587f, 0.114f));
        specularLighting =
            lerp(greyScale, specularLighting, g_EnvmapSaturation);
#endif
    ...
    float dotprod =
        0.7 + 0.25 * dot(worldSpaceNormal, normalize(float3(1, 2, -.5)));
    return FinalOutput(HALF4(dotprod * albedo.xyz, alpha), 0,
    ...
    bWriteDepthToAlpha =
        (WRITE_DEPTH_TO_DESTALPHA != 0) && (WRITEWATERFOGTODESTALPHA == 0);
#endif

    float fogFactor =
        CalcPixelFogFactor(PIXELFOGTYPE, g_FogParams, g_EyePos.z,
                           i.worldPos_projPosZ.z, i.worldPos_projPosZ.w);

#if WRITEWATERFOGTODESTALPHA && (PIXELFOGTYPE == PIXEL_FOG_TYPE_HEIGHT)
    alpha = fogFactor;
#endif

    return FinalOutput(float4(result.rgb, alpha), fogFactor, PIXELFOGTYPE,
                       TONEMAP_SCALE_LINEAR, bWriteDepthToAlpha,
