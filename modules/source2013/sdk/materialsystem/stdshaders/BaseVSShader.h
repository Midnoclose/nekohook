...
class CBaseVSShader : public CBaseShader {
   public:
    ...
    // Loads a scale/offset version of the viewport transform into the specified
    // constant.
    ...
    void SetModulationPixelShaderDynamicState_LinearColorSpace(
        int modulationVar);
    ...
    void SetColorVertexShaderConstant(int nVertexReg, int colorVar,
                                      int alphaVar);
    ...
    void DrawWorldBumpedUsingVertexShader(
        ...
        int envmapMaskFrame, int envmapVar, int envmapFrameVar,
        int envmapTintVar, int colorVar, int alphaVar, int envmapContrastVar,
    ...
    // Alpha tested materials can end up leaving garbage in the dest alpha
    // buffer if they write depth. This pass fills in the areas that passed the
    // alpha test with depth in dest alpha by writing only equal depth pixels and
    // only if we should be writing depth to dest alpha
