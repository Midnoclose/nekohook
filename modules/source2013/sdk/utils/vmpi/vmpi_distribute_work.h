...
class IWorkUnitDistributorCallbacks {
   public:
    ...
    // because when wu0 is received we already have { wu0, wu1, wu2 } so we
    // signal that 3 subsequent work units completed, like wise by the time when
    // wu3 is received we already have a full set { wu0, wu1, wu2, wu3, wu4,
    // wu5, wu6 } and signal that 7 work units completed.
...
typedef void (*ProcessWorkUnitFn)(int iThread, uint64 iWorkUnit,
                                  MessageBuffer *pBuf);

// pBuf is ready to read the results written to the buffer in ProcessWorkUnitFn.
typedef void (*ReceiveWorkUnitFn)(uint64 iWorkUnit, MessageBuffer *pBuf,
                                  int iWorker);
