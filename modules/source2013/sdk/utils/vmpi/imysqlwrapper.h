...
class IMySQLRowSet {
   public:
    ...
    // again. Note: after calling this, you have to call NextRow() to actually
    // get the first row's value ready.
...
class IMySQL : public IMySQLRowSet {
   public:
    ...
    // This reads in all of the data in the last row set you queried with
    // Execute and builds a separate copy. This is useful in some of the VMPI
    // tools to have a thread repeatedly execute a slow query, then store off
    // the results for the main thread to parse.
