...
class IChannel {
   public:
    ...
    // reliable one, it will automatically release the unreliable one it
    // contains.
    ...
    //
    // Note: this is most efficient if you keep the buffer around between calls
    // so it only reallocates it when it needs more space.
    virtual bool Recv(CUtlVector<unsigned char> &data,
                      double flTimeout = 0) = 0;

    // Returns false if the connection has been broken.
    virtual bool IsConnected() = 0;

    // If IsConnected returns false, you can call this to find out why the
