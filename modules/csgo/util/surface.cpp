...
static inline Color convert(CatColor _color) {
...
static unsigned long long surface_fonts[FONT_COUNT][12];

// Retrieves one of the fonts generated and saved into
static const unsigned long long& GetSurfaceFont(int font, int size) {
...
static void String3(const char* text, int x, int y, int font, int size,
    ...
    // Draw
    g_ISurface->DrawPrintText(buf, strlen(text));
}
// Shadow processor
#define shadow        \
    CatColor(0, 0, 0, \
