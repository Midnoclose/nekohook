...
class ClassDumper : private CatLogger {
   public:
    ...
    void SaveDump() {
        ...
        while (cc) {
            log("[%d] %s", cc->m_ClassID, cc->m_pNetworkName);
            cc = cc->m_pNext;
        }
    }
};
...
private:
        void DumpTable(RecvTable* pTable, int recurstions = 0, int parent_offset
= 0) { if (pTable == nullptr) return;

                // Print tabs for how far we are
                for (int i = 0; i < recurstions; i++)
                        log("%s", "  ");
...
TypeToString(pProp->GetType()), "]\n");
                        ...
                        if (pProp->GetDataTable()) {
                                ...
                                // Recurse into child
                                DumpTable(pProp->GetDataTable(), recurstions +
1, pProp->GetOffset())
                        }
                }
        }
