...
typedef enum {
    LEGANIM_9WAY,    // Legs use a 9-way blend, with "move_x" and "move_y" pose
                     // parameters.
    LEGANIM_8WAY,    // Legs use an 8-way blend with "move_yaw" pose param.
    LEGANIM_GOLDSRC  // Legs always point in the direction he's running and the
                     // torso rotates.
} LegAnimType_t;
...
class CSequenceTransitioner {
   public:
    void CheckForSequenceChange(
        ...
        studiohdr_t *hdr, int nCurSequence,
    ...
    void UpdateCurrent(
        // Describe the current animation state with these parameters.
        studiohdr_t *hdr, int nCurSequence, float flCurCycle,
...
#define MAIN_IDLE_SEQUENCE_LAYER \
    0  // For 8-way blended models, this layer blends an idle on top of the
       // run/walk animation to simulate a 9-way blend.
...
class CBasePlayerAnimState : virtual public IPlayerAnimState {
   ...
   public:
    ...
    // class overrides this to handle events like jumping, firing, etc. virtual
    // Activity CalcMainActivity() = 0; ye should get this activity shit but no
    // time
    ...
    // ... );   PRINTF_FORMAT_STRING 404/ void AnimStateLog( PRINTF_FORMAT_STRING
    // const char *pMsg, ... );
    ...
    // virtual Activity TranslateActivity( Activity actDesired ) { return
    // actDesired; }   404 activity

    // Allow inheriting classes to override SelectWeightedSequence
    // virtual int SelectWeightedSequence( Activity activity );

   public:
    // void				GetPoseParameters( studiohdr_t *pStudioHdr, float
    // poseParameter[MAXSTUDIOPOSEPARAM] ); <- got this in ivmodelinfo.h
   ...
   protected:
    ...
    // virtual int CalcSequenceIndex( PRINTF_FORMAT_STRING const char
    // *pBaseName, ... );
   ...
   private:
    ...
    void UpdateAimSequenceLayers(float flCycle, int iFirstLayer,
                                 bool bForceIdle,
